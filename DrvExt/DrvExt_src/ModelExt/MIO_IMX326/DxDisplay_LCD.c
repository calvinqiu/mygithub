...
#define DRVLCD_BRT_LEVEL_MAX     (16)

#define DRVLCD_BRT_LEVEL_INIT    0//8//(DRVLCD_BRT_LEVEL_MAX/2)
#define DRVLCD_BRT_LEVEL_OFF     (DRVLCD_BRT_LEVEL_MAX-1)
...
void PWMTurnOnLCDBacklightNormal(void);
void PWMTurnOffLCDBacklightNormal(void);


//void CloseLcdPWM(void);
...
static UINT32 DrvLCDOpen(void) // Common Constructor
{
    ...
    if(g_LCDDout == 0)
    {
		...
		//if(GetLcdBlCtrlType() == LCD_BL_CTRL_GPIO) {
...
static BOOL lcd_display_off = FALSE;////TRUE;
		    ...
		    case DRVDISP_STATE_BRIGHTLVL:
		        {
		            ...
		            GPIOMap_SetLCDBacklightBrightLevel( Value );
		        }
		        break;
...
//#define PRD_VALUE					128
//#define PRD_VALUE					512
#define PRD_VALUE					240
//#define PRD_VALUE					160


#define PRD_BASE					(PRD_VALUE/DRVLCD_BRT_LEVEL_MAX)


//static PWM_CFG g_LCDBacklightPWMInfo = {255, PRD_BASE*9, 127, 0, 0};
static PWM_CFG g_LCDBacklightPWMInfo = {PRD_VALUE, PRD_BASE*DRVLCD_BRT_LEVEL_INIT, PRD_VALUE-1, 0, 0};
...
static BOOL PWM_LCD_OFF = FALSE;
static BOOL PWM_LCD_END = FALSE;
void PWMTurnOnLCDBacklightNormal(void)
{
    if(g_LCDBacklightEn == TRUE)
    {
		if(GetLcdBlCtrlType() == LCD_BL_CTRL_PWM) {

			DrvLCDState(DXSET|DRVDISP_STATE_BRIGHTLVL, DRVLCD_BRT_LEVEL_INIT);
		}
		PWM_LCD_OFF = TRUE;
    }
}
...
void PWMTurnOffLCDBacklightNormal(void)
{
    if(g_LCDBacklightEn == TRUE)
    {
		if(GetLcdBlCtrlType() == LCD_BL_CTRL_PWM) {
			...
			//g_LCDBacklightPWMInfo.uiRise = PRD_BASE*16;//g_LCDBacklightLvlPWMDuty[g_LCDBacklightLvl];
			g_LCDBacklightPWMInfo.uiRise = g_LCDBacklightLvlPWMDuty[ DRVLCD_BRT_LEVEL_OFF ];//PRD_BASE*16;

			pwm_pwmConfig(PWMID_LCD_BLG_PCTL, &g_LCDBacklightPWMInfo);

			pwm_pwmReloadConfig(PWMID_LCD_BLG_PCTL, g_LCDBacklightPWMInfo.uiRise, g_LCDBacklightPWMInfo.uiFall, g_LCDBacklightPWMInfo.uiPrd);
...
void CloseLcdPWM(void)
{
	if(GetLcdBlCtrlType() == LCD_BL_CTRL_PWM) {
		...
		if (g_LCDBacklightEn == TRUE) {
			...
			DrvLCDState(DXSET|DRVDISP_STATE_BRIGHTLVL, DRVLCD_BRT_LEVEL_OFF);
...
static void GPIOMap_TurnOnLCDBacklight(void)
{
	...
	static BOOL _init_lcd_pwm = FALSE;
	...
	if(g_LCDBacklightEn == FALSE) {
		...
		if(GetLcdBlCtrlType() == LCD_BL_CTRL_PWM) {
			...
			if(FALSE == _init_lcd_pwm) {

				_init_lcd_pwm = TRUE;

		        pwm_open(PWMID_LCD_BLG_PCTL);

				pwm_pwmConfigClockDiv(PWM13_CLKDIV, 10);//1199

		        pwm_pwmConfig(PWMID_LCD_BLG_PCTL, &g_LCDBacklightPWMInfo);

				if (pwm_pwmDisable(PWMID_LCD_BLG_PCTL) != E_OK)
				{
