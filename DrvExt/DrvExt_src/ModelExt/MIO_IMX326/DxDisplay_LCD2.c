...
static BOOL GPIOMap_DetLCD2(void);

//dx object
DX_OBJECT gDevLCD2 =
{
    DXFLAG_SIGN,
    DX_CLASS_DISPLAY_EXT|DX_TYPE_LCD2,
    DISPLAY_VER,
    "Display_LCD2",
    0,0,0,0,
    DrvLCDGetcaps,
...
static int _DrvLCDHookDout(UINT32 id)
{
    ...
    if(id == 0)
    {
        ...
        DBG_ERR("LCD2 not support DOUT1!\r\n");
        return 0;
    }
    if(id == 1)
...
static UINT32 DrvLCDOpen(void) // Common Constructor
{
    ...
    pDev->SEL.SET_REG_IF.uiGpioSen   = GPIO_LCD2_SIF_SEN;
    pDev->SEL.SET_REG_IF.uiGpioClk   = GPIO_LCD2_SIF_SCK;
    pDev->SEL.SET_REG_IF.uiGpioData  = GPIO_LCD2_SIF_SDA;
    pObj->devCtrl(DISPDEV_SET_REG_IF,pDev);
#endif
#if (LCD2_COMM_CTRL == LCD_COMM_BY_SIF)
    pDev->SEL.SET_REG_IF.LcdCtrl     = DISPDEV_LCDCTRL_SIF;
    pDev->SEL.SET_REG_IF.uiSifCh     = SIF_LCD2_CH;
    ...
    pad_setPullUpDown(PAD_LCD2_SIF_SEN, PAD_PULLUP);
    pad_setPullUpDown(PAD_LCD2_SIF_SCK, PAD_PULLDOWN);
    pad_setPullUpDown(PAD_LCD2_SIF_SDA, PAD_PULLDOWN);
#endif
...
/**
  Detect LCD2 is plugging in or unplugged

  Detect LCD2 is plugging in or unplugged.
  Return TRUE if LCD2 is plugging in, FALSE if LCD2 is unplugged.

  @param void
  @return BOOL: TRUE -> LCD2 is plugging in, FALSE -> LCD2 is unplugged
*/
static BOOL GPIOMap_DetLCD2(void)
{
#if ((LCD2_DET_CTRL == LCD2_DET_BY_VIO) && (USE_VIO == ENABLE))
    return (vio_getpin(VIO_LCD2_DET) != 0);
#else
    return 0;
#endif
}
