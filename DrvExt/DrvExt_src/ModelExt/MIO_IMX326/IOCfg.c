...
GPIO_INIT_OBJ uiGPIOMapInitTab[] = {
     ...
     {  GPIO_CARD_WP,      		GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST      },
     	
			
	 //wifi
     {  GPIO_WIFI_SHDN,         GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_HI,   PAD_PIN_NOT_EXIST      },
     {  GPIO_WIFI_CS,           GPIO_DIR_OUTPUT,     GPIO_SET_OUTPUT_HI,  GPIO_SET_NONE       },
...
//#endif
	 ...
	 //I2C KEY
     {  P_GPIO_31,         GPIO_DIR_OUTPUT,     GPIO_SET_OUTPUT_HI,  GPIO_SET_NONE       },
     {  P_GPIO_32,         GPIO_DIR_OUTPUT,     GPIO_SET_OUTPUT_HI,  GPIO_SET_NONE       },

     {  P_GPIO_12,         GPIO_DIR_OUTPUT,     GPIO_SET_OUTPUT_HI,  GPIO_SET_NONE       },
     {  P_GPIO_15,         GPIO_DIR_OUTPUT,     GPIO_SET_OUTPUT_HI,  GPIO_SET_NONE       },	 

	 {  P_GPIO_26,         GPIO_DIR_OUTPUT,     GPIO_SET_OUTPUT_HI,  GPIO_SET_NONE       },	 

	 { GPIO_CHARGE_FULL,   GPIO_DIR_INPUT, 		PAD_PULLUP, 			PAD_CHARGE_FULL},
	 { GPIO_CHARGE_EN, 	   GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,    PAD_PIN_NOT_EXIST},

	 { GPIO_DET_HW_VER,	   GPIO_DIR_INPUT,	   PAD_PULLDOWN,		   PAD_DET_HW_VER	},
...
void IO_InitGPIO(void)
{
	...
	//@liangfu 20161007. diff the lcd backlight gpio type.
...
BOOL IsChargeFull(void) {
	...
	UINT32 ret = gpio_getPin(GPIO_CHARGE_FULL) ;
	if(ret) {

		gpio_setPin( GPIO_CHARGE_EN );
		return TRUE;
	} else {
...
//获取硬件版本号，新的硬件版本，IO口拉高；旧软件版本，IO口拉低.
BOOL IsNew_HW_VER(void) {

	UINT32 ret = gpio_getPin(GPIO_DET_HW_VER) ;
	if(ret) {
...
static unsigned char lcd_bl_type = LCD_BL_CTRL_PWM;//LCD_BL_CTRL_GPIO;//LCD_BL_CTRL_PWM;
void SetLcdBlCtrlType(unsigned char en) {

	lcd_bl_type = en;
}

unsigned char GetLcdBlCtrlType(void) {

	return LCD_BL_CTRL_PWM;
	//return lcd_bl_type;
}
