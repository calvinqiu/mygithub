...
#if 0
static SENSOR_INIT_OBJ Sensor_GetObjFlip(void)
{
    SENSOR_INIT_OBJ InitObj = {0};
    InitObj.CmdInfo.CmdType = SENSOR_CMD_I2C;
    InitObj.CmdInfo.INFO.I2C.I2c = SENSOR_I2C1_2ND;
...
debug_msg("====== ov4689 SenPowerOn! \n");
	...
	//reset key
	gpio_setDir(P_GPIO_29, GPIO_DIR_OUTPUT);
    gpio_setPin(P_GPIO_29);

	Delay_DelayMs( 100 );

	gpio_setDir(P_GPIO_29, GPIO_DIR_OUTPUT);
	gpio_clearPin(P_GPIO_29);

}

static void SenPowerOff(void)
{
debug_msg("====== ov4689 SenPowerOff! \n");
...
#if 0
static SEN_GET_INI_OBJ SenObjTabFlip[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetObjFlip,   NULL,   NULL,   NULL,
};
#endif
...
static SENSOR_INIT_OBJ Sensor_GetObj(void)
{
    ...
    */
    SENSOR_INIT_OBJ InitObj = {0};
    InitObj.CmdInfo.CmdType = SENSOR_CMD_IO;
    InitObj.CmdInfo.INFO.IO.fpIO = SENSOR_IO;
...
#elif ((_SENSORLIB_ == _SENSORLIB_CMOS_IMX206CQC_) &&      \
       (_SENSORLIB2_ == _SENSORLIB2_CMOS_IMX206CQC_) &&            \
       (_SENSORLIB3_ == _SENSORLIB3_OFF_) &&            \
       (_SENSORLIB4_ == _SENSORLIB4_OFF_))
...
static ER SENSOR_IO(SENSOR_ID Id, UINT32 uiData0)
{
    UINT32 uiData0_bit;
    UINT32 BitCount;
    wai_sem(SENSOR_IO_ID);

    if (CHK_IO == 1)
        DBG_WRN("Overlap into SENSOR_IO\r\n");
    CHK_IO = 1;
...
static SENSOR_INIT_OBJ Sensor_GetObj2(void)
{
    ...
    */
    ...
    InitObj.Sen2LVDSPinMap[0] = 5;
    InitObj.Sen2LVDSPinMap[1] = 6;
    InitObj.Sen2LVDSPinMap[2] = 7;
    InitObj.Sen2LVDSPinMap[3] = 8;
    InitObj.Sen2LVDSPinMap[4] = SEN_IGNORE;
    InitObj.Sen2LVDSPinMap[5] = SEN_IGNORE;
    InitObj.Sen2LVDSPinMap[6] = SEN_IGNORE;
    InitObj.Sen2LVDSPinMap[7] = SEN_IGNORE;
...
static BOOL sen1IsPowerOn = FALSE;
static void SenPowerOn(void)
{
    if (pll_isClockEnabled(SIE_MCLK2) == DISABLE)//Confirm sensor 2 not open
    {
        gpio_setDir(P_GPIO_40, GPIO_DIR_OUTPUT);
        gpio_setDir(P_GPIO_37, GPIO_DIR_OUTPUT);
        gpio_setDir(P_GPIO_36, GPIO_DIR_OUTPUT);
        ...
        pll_selectClkSrc(PLL_CLK_SIEMCLK2, PLL_CLKSRC_PLL5);
        pll_setClkFreq(PLL_CLK_SIEMCLK2, 72000000);//72MHz
        pll_setClkEn(PLL_CLK_SIEMCLK2, ENABLE);
    }
...
static void SenPowerOn2(void)
{
    if (pll_isClockEnabled(SIE_MCLK) == DISABLE)//Confirm sensor 1 not open
    {
        gpio_setDir(P_GPIO_40, GPIO_DIR_OUTPUT);
        gpio_setDir(P_GPIO_37, GPIO_DIR_OUTPUT);
        gpio_setDir(P_GPIO_36, GPIO_DIR_OUTPUT);
...
static void SenPowerOff(void)
{
    if (pll_isClockEnabled(SIE_MCLK2) == DISABLE)// confirm sensor 2 has been closed
    {
        pll_setClkEn(PLL_CLK_SIEMCLK, DISABLE);
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_IMX206CQC,   Sensor_GetDrvTab_IMX206CQC,   NULL,   NULL,
};
...
#elif ((_SENSORLIB_ == _SENSORLIB_CMOS_IMX326LQC_) &&   \
       (_SENSORLIB2_ == _SENSORLIB2_OFF_) &&            \
       (_SENSORLIB3_ == _SENSORLIB3_OFF_) &&            \
       (_SENSORLIB4_ == _SENSORLIB4_OFF_))
static SENSOR_INIT_OBJ Sensor_GetObj(void)
{
    ...
    InitObj.CmdInfo.INFO.I2C.I2c = SENSOR_I2C1_2ND;
    InitObj.CmdInfo.INFO.I2C.extSlaveAddrEnable = DISABLE;
	//#NT#2016/11/25#Silvia Wu -begin
    //#NT# test
    InitObj.ChgMclkEn = ENABLE;
    //#NT#2016/11/25#Silvia Wu -end
...
{
	// power on sequence
	gpio_setDir(GPIO_SENSOR_12EN, GPIO_DIR_OUTPUT);
	gpio_setDir(GPIO_SENSOR_18EN, GPIO_DIR_OUTPUT);
	gpio_setDir(GPIO_SENSOR_28EN, GPIO_DIR_OUTPUT);
	gpio_setDir(GPIO_SENSOR_RESET, GPIO_DIR_OUTPUT);

	gpio_clearPin(GPIO_SENSOR_12EN);
	gpio_clearPin(GPIO_SENSOR_18EN);
	gpio_clearPin(GPIO_SENSOR_28EN);
	gpio_clearPin(GPIO_SENSOR_RESET);


    gpio_setPin(GPIO_SENSOR_12EN);
    Delay_DelayMs(20);
    gpio_setPin(GPIO_SENSOR_18EN);
    Delay_DelayMs(20);
    gpio_setPin(GPIO_SENSOR_28EN);
    Delay_DelayMs(20);
    gpio_setPin(GPIO_SENSOR_RESET);
	Delay_DelayMs(20);
    ...
    if (pll_getPLLEn(PLL_ID_5) == DISABLE)
    {
        ...
        //pll_setPLL(PLL_ID_5, 0x318000);//297MHz
        pll_setPLLEn(PLL_ID_5, ENABLE);
    }
	if (pll_getPLLEn(PLL_CLKSRC_480) == DISABLE)
    {
        pll_setPLLEn(PLL_CLKSRC_480, ENABLE);
    }

    pll_selectClkSrc(PLL_CLK_SIEMCLK, PLL_CLKSRC_480);
	//pll_selectClkSrc(PLL_CLK_SIEMCLK, PLL_ID_5);
    pll_setClkFreq(PLL_CLK_SIEMCLK, 24000000);//24MHz
    //pll_setClkFreq(PLL_CLK_SIEMCLK, 12000000);//12MHz
    pll_setClkEn(PLL_CLK_SIEMCLK, ENABLE);
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_IMX326LQC,   NULL,   NULL,   NULL,
};
