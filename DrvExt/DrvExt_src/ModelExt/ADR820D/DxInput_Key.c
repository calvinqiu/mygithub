...
#define VOLDET_KEY_ADC_LVL0         (483-VOLDET_KEY_ADC_RANGE)
#define VOLDET_KEY_ADC_LVL1         (637-VOLDET_KEY_ADC_RANGE)
#define VOLDET_KEY_ADC_LVL2         (748-VOLDET_KEY_ADC_RANGE)
#define VOLDET_KEY_ADC_TH           (800+VOLDET_KEY_ADC_RANGE)

#define VOLDET_KEY_LVL_UNKNOWN      0xFFFFFFFF
#define VOLDET_KEY_LVL_UP           0
#define VOLDET_KEY_LVL_DOWN         1
#define VOLDET_KEY_LVL_ENTER        2
#endif
...
static UINT32 VolDet_GetKey2Level(void)
{
    ...
    if (uiKey1ADC < VOLDET_KEY_ADC_TH)
    {
		if (uiKey1ADC >= VOLDET_KEY_ADC_LVL1)
        {
            uiCurKey2Lvl = VOLDET_KEY_LVL_DOWN;
        }
        else //if (uiKey1ADC >= VOLDET_KEY_ADC_LVL0)
...
UINT32 DrvKey_DetNormalKey(void)
{
	...
	switch(uiKey1Lvl)
	{
		...
		default:
			break;
		case VOLDET_KEY_LVL_UP:  //SW1
			uiKeyCode |= FLGKEY_UP;
			break;
		case VOLDET_KEY_LVL_DOWN:	//SW2
			uiKeyCode |= FLGKEY_DOWN;
			break;
		case VOLDET_KEY_LVL_ENTER:	//SW3
			uiKeyCode |= FLGKEY_ENTER;
			break;
	}
#endif

	if(!gpio_getPin(GPIO_KEY_MENU))  //SW4
	{
