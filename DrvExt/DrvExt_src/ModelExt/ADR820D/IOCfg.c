...
GPIO_INIT_OBJ uiGPIOMapInitTab[] = {
	 ...
	 {	GPIO_SOC_ON, 	GPIO_DIR_OUTPUT,	GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST	  }, //WIFI SHDN, ShutDown
     {  GPIO_LCD_CTRL,      GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_HI,  PAD_PIN_NOT_EXIST      },
     {  GPIO_LCD_BLG_PCTL,      GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,  PAD_PIN_NOT_EXIST      },
	 {	GPIO_CARD_POWER,		GPIO_DIR_OUTPUT,	GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST 	 },
	 ...
	 {	GPIO_SENSOR2_POWER_MODULE,	 GPIO_DIR_OUTPUT,	 GPIO_SET_OUTPUT_HI,   PAD_PIN_NOT_EXIST	 },
...
#if 0//(_SENSORLIB2_ == _SENSORLIB_CMOS_IMX322LQJVX1_||_SENSORLIB2_ == _SENSORLIB2_CMOS_SC2033CVX1_)
	 {	GPIO_STATE_DET, 	 GPIO_DIR_INPUT,	 PAD_PULLDOWN,		   PAD_STATE_DET	   },
#endif
#endif
...
#if 0
static void GPIO_Sensor2PowerOn(void)
{
	gpio_setDir(GPIO_SENSOR2_PWR, GPIO_DIR_OUTPUT);
	gpio_setPin(GPIO_SENSOR2_PWR);
}

static void GPIO_Sensor2PowerOff(void)
{
	gpio_setDir(GPIO_SENSOR2_PWR, GPIO_DIR_OUTPUT);
	gpio_clearPin(GPIO_SENSOR2_PWR);
}

static BOOL GPIO_IsSensor2PowerOn(void)
{
	return (gpio_getPin(GPIO_SENSOR2_PWR) == 1 ? TRUE : FALSE);
}

static BOOL GPIO_DetWaveGuard1State(void)
{
	if(HwPower_GetPowerKey(POWER_ID_PSW1) == TRUE)
		debug_msg("GPIO_DetWaveGuard1State = %d \r\n",HwPower_GetPowerKey(POWER_ID_PSW1));
	return (HwPower_GetPowerKey(POWER_ID_PSW1) == 1 ? TRUE : FALSE);
}

static BOOL GPIO_DetWaveGuard2State(void)
{
	if(gpio_getPin(GPIO_STATE_DET) == TRUE)
		debug_msg("GPIO_DetWaveGuard2State = %d \r\n",gpio_getPin(GPIO_STATE_DET));

	return (gpio_getPin(GPIO_STATE_DET) == 1 ? TRUE : FALSE);
}
#endif