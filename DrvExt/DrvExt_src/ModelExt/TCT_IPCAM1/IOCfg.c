...
GPIO_INIT_OBJ uiGPIOMapInitTab[] = {
    ...
    || _MODEL_DSC_ == _MODEL_IPCAM2_EVB_ )
     ...
     //#NT#2016/09/29#Niven Cho -begin
     //#NT#FIX PHI Power Sequence
          {  GPIO_ETH_GMAC,          GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_HI,  PAD_PIN_NOT_EXIST      },
     {  L_GPIO_21,              GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_HI,  PAD_PIN_NOT_EXIST      },
     //#NT#2016/09/29#Niven Cho -end
#endif
...
#if (_NETWORK_ == _NETWORK_GMAC_ETHERNET_)
    //PGPIO[20] 25MHz
    pll_setPLL(PLL_ID_2, 3276800);
    pll_setPLLEn(PLL_ID_2, TRUE);
    pll_selectClkSrc(PLL_CLK_SPCLK, PLL_CLKSRC_PLL2);
    pll_setClkFreq(PLL_CLK_SPCLK, 25000000);
    pll_setClkEn(PLL_CLK_SPCLK, TRUE);
    //RESET
    Delay_DelayMs(20);
