...
#define LCD_ASPECT_W                    4
#define LCD_ASPECT_H                    3
#elif (_LCDTYPE_ == _LCDTYPE_ILI9341_IF8B_)
#define LCD_MAX_W                       ALIGN_CEIL_16(320)  //device w of DISP_LCDMODE_xxx, to support rotate, it must align to 16
...
#define LCD_MAX_W                       ALIGN_CEIL_16(1280) //device w of DISP_LCDMODE_xxx, to support rotate, it must align to 16
#define LCD_MAX_H                       ALIGN_CEIL_16(400)  //device h of DISP_LCDMODE_xxx, to support rotate, it must align to 16
#define LCD_ASPECT_W                    16
#define LCD_ASPECT_H                    5
#elif (_LCDTYPE_ == _LCDTYPE_M935UX01M_SN75LVDS83B_)
...
#define DRVLCD_BRT_LEVEL_INIT     9//5
...
//static PWM_CFG g_LCDBacklightPWMInfo = {100, 0, 10, 0, 0};
static PWM_CFG g_LCDBacklightPWMInfo = {100, 0, 90, 0, 0};
...
static void GPIOMap_TurnOnLCDBacklight(void)
{
    ...
    {
#if (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)
        g_LCDBacklightPWMInfo.uiRise = 0;//g_LCDBacklightLvlPWMDuty[g_LCDBacklightLvl];
        DBG_IND("PWM: rise=%d\r\n", g_LCDBacklightPWMInfo.uiRise);
        ER result= pwm_open(PWMID_LCD_BLG_PCTL);
        //DBG_IND("pwm_open: result=%d\r\n", result);

        result=pwm_pwmConfig(PWMID_LCD_BLG_PCTL, &g_LCDBacklightPWMInfo);
        //DBG_IND("pwm_pwmConfig: result=%d\r\n", result);

        //Output Freq=120M/(Div+1)/base period=120M/24=5M
        pwm_pwmConfigClockDiv(PWM0_3_CLKDIV,23);//5M

        result=pwm_pwmEnable(PWMID_LCD_BLG_PCTL);
        //DBG_IND("pwm_pwmEnable: result=%d\r\n", result);
...
static BOOL GPIOMap_IsLCDBacklightOn(void)
{
    DBG_IND("GPIOMap_IsLCDBacklightOn: g_LCDBacklightEn=%d\r\n", g_LCDBacklightEn);
