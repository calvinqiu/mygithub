...
{
    ChargeIC_Open();
    ChargeIC_DoCmd(cmd_ChargeIC_init, 0, 0);
    debug_msg("Charge init\r\n");

    gpio_setDir(P_GPIO_37, GPIO_DIR_OUTPUT);  //  Enable CMOS  OSC OUTPUT
    gpio_setDir(P_GPIO_36, GPIO_DIR_OUTPUT);  //  SN_2.8EN
    gpio_setDir(GPIO_SENSOR_RESET, GPIO_DIR_OUTPUT);

    ChargeIC_DoCmd(cmd_Charge_en_1_2V, 0x00, 0);  //  SN_1.2 Disable
    ChargeIC_DoCmd(cmd_Charge_en_1_8V, 0x00, 0);  //  SN_1.8 Disable
    gpio_clearPin(P_GPIO_36);  //  SN_2.8EN
    gpio_clearPin(GPIO_SENSOR_RESET);
    Delay_DelayMs(5);

    ChargeIC_DoCmd(cmd_Charge_en_1_2V, 0x80, 0);  //  SN_1.2EN
    Delay_DelayMs(5);
    ChargeIC_DoCmd(cmd_Charge_en_1_8V, 0x60, 0);  //  SN_1.8EN
    Delay_DelayMs(5);
    gpio_setPin(P_GPIO_36);  //  SN_2.8EN
    Delay_DelayMs(5);
    gpio_setPin(GPIO_SENSOR_RESET);
