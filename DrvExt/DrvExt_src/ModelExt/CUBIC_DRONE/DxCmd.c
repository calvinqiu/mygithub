...
static BOOL Cmd_dx_chicw(CHAR* strCmd)
{
    UINT32  addr;
    UINT32  value;
    INT32   arg_num;
    arg_num=sscanf_s(strCmd, "%x %x\n", &addr, &value);
    if (arg_num<2)
        return TRUE;
    DBG_DUMP("[CHARGEIC] - chargeic write\r\n");
    DBG_DUMP("[CHARGEIC] - reg[%02x] <= %02x\r\n", addr, value);
    ChargeIC_DoCmd(cmd_ChargeIC_write, addr, &value);
    return TRUE;
}
static BOOL Cmd_dx_chicr(CHAR* strCmd)
{
    UINT32  addr;
    UINT32  value;
    INT32   arg_num;
    arg_num=sscanf_s(strCmd, "%x\n", &addr);
    if (arg_num<1)
        return TRUE;
    DBG_DUMP("[CHARGEIC] - chargeic read\r\n");
    ChargeIC_DoCmd(cmd_ChargeIC_read, addr, &value);
    DBG_DUMP("[CHARGEIC] - reg[%02x] => %02x\r\n", addr, value);
    return TRUE;
}
static BOOL Cmd_dx_chic_dump_reg(CHAR* strCmd)
{
    CHKPNT;
    ChargeIC_DumpAll();
...
SXCMD_ITEM("chicw %", Cmd_dx_chicw, "write register of charger IC")
SXCMD_ITEM("chicr %", Cmd_dx_chicr, "read register of charger IC")
SXCMD_ITEM("chic_dump_all_reg", Cmd_dx_chic_dump_reg,  "dump all registers of charger IC")
SXCMD_END()
