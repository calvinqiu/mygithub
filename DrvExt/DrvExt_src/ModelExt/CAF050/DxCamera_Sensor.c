...
#elif ((_SENSORLIB_ == _SENSORLIB_CMOS_IMX258_) &&      \
       (_SENSORLIB2_ == _SENSORLIB2_CMOS_OV7251M_) &&            \
       (_SENSORLIB3_ == _SENSORLIB3_OFF_) &&            \
       (_SENSORLIB4_ == _SENSORLIB_OFF_))
...
static SENSOR_INIT_OBJ Sensor_GetObj2(void)
{
    ...
    InitObj.CmdInfo.INFO.I2C.extSlaveAddrEnable = ENABLE;
    InitObj.CmdInfo.INFO.I2C.extSlaveRAddr = 0xC1;
    InitObj.CmdInfo.INFO.I2C.extSlaveWAddr = 0xC0;
    InitObj.ChgMclkEn = ENABLE;
    return InitObj;
}
...
static void SenPowerOn(void)
{
    ...
    gpio_setDir(PGPIO_SENSOR_EN1, GPIO_DIR_OUTPUT);
    gpio_setPin(PGPIO_SENSOR_EN1);
    Delay_DelayMs(10);
    gpio_setDir(PGPIO_SENSOR_EN2, GPIO_DIR_OUTPUT);
    gpio_setPin(PGPIO_SENSOR_EN2);
    Delay_DelayMs(10);
    gpio_setDir(PGPIO_SENSOR_EN3, GPIO_DIR_OUTPUT);
    gpio_setPin(PGPIO_SENSOR_EN3);
    Delay_DelayMs(10);

    // PLL frequency will be 12MHz * uiSetting / 131072
    pll_setPLLEn(PLL_ID_5, DISABLE);
    ...
    //pll_selectClkSrc(PLL_CLK_SIEMCLK, PLL_CLKSRC_PLL5);
    pll_selectClkSrc(PLL_CLK_SIEMCLK, PLL_CLKSRC_480);
    pll_setClkFreq(PLL_CLK_SIEMCLK, 12000000);
    pll_setClkEn(PLL_CLK_SIEMCLK, ENABLE);
    Delay_DelayMs(10);
...
static void SenPowerOn2(void)
{
    gpio_setDir(PGPIO_SENSOR2_PWR, GPIO_DIR_OUTPUT);
    gpio_setPin(PGPIO_SENSOR2_PWR);
    Delay_DelayMs(2);

    gpio_setDir(SGPIO_SENSOR2_EXTCLK, GPIO_DIR_OUTPUT);
//    gpio_setPin(SGPIO_SENSOR2_EXTCLK);

    // for i2c
    /*
    if (pll_getPLLEn(PLL_ID_5) == DISABLE)
    {
        pll_setPLL(PLL_ID_5, 0x20000);//12MHz
        pll_setPLLEn(PLL_ID_5, ENABLE);
    }
    */
    ...
    //pll_selectClkSrc(PLL_CLK_SIEMCLK2, PLL_CLKSRC_PLL5);
    pll_selectClkSrc(PLL_CLK_SIEMCLK, PLL_CLKSRC_480);
    pll_setClkFreq(PLL_CLK_SIEMCLK2, 24000000);  //24MHz
    pll_setClkEn(PLL_CLK_SIEMCLK2, ENABLE);
    Delay_DelayMs(10);
}
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_IMX258,   Sensor_GetDrvTab_OV7251M,   NULL,   NULL,
};
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_IMX258,   NULL,   NULL,   NULL,
};
