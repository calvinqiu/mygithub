...
static PWM_CFG g_IRLEDPWMInfo = {100, 0, 10, 0, 0};
...
void LED_TurnOnLED(UINT32 uiLED)
{
    ...
    #if (LED_IR_CTRL == LED_IR_BY_PWM)
    {
        ...
        //g_IRLEDPWMInfo.uiRise = 0;//FcsLedPrdVal[FcsLedLvl];
        pwm_open(PWMID_IR_LED);
        pwm_pwmConfig(PWMID_IR_LED, &g_IRLEDPWMInfo);
        //Output Freq=120M/(Div+1)/base period=120M/24/100=50K
        pwm_pwmConfigClockDiv(PWM0_3_CLKDIV,23);//1199
        pwm_pwmEnable(PWMID_IR_LED);
...
void LED_TurnOffLED(UINT32 uiLED)
{
    ...
    #if (LED_IR_CTRL == LED_IR_BY_PWM)
    {
        pwm_pwmDisable(PWMID_IR_LED);
        pwm_close(PWMID_IR_LED, TRUE);
    }
    #endif
    break;
    case GPIOMAP_LED_FCS:
