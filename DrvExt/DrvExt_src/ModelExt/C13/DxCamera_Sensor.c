...
static SEN_PLUG_IN SenDetPlugInOnTab[SENSOR_ID_MAX_NUM] =
{
    NULL,   NULL,   NULL,   NULL,
       ...
       (_SENSORLIB2_ == _SENSORLIB2_OFF_) &&            \
       (_SENSORLIB3_ == _SENSORLIB3_CMOS_GM7150_) &&     \
       (_SENSORLIB4_ == _SENSORLIB4_OFF_))
...
static SENSOR_INIT_OBJ Sensor_GetObj(void)
{
    ...
    InitObj.CmdInfo.INFO.I2C.extSlaveAddrEnable = FALSE;
    InitObj.ChgMclkEn = ENABLE;
    InitObj.Option = SENSOR_OPTION_MIRROR;//SENSOR_OPTION_FLIP
    return InitObj;
}

static SENSOR_INIT_OBJ Sensor_GetObj3(void)
{
    SENSOR_INIT_OBJ InitObj = {0};
    InitObj.CmdInfo.CmdType = SENSOR_CMD_I2C;

    InitObj.CmdInfo.INFO.I2C.I2c = SENSOR_I2C1_1ST; // need check
    InitObj.CmdInfo.INFO.I2C.extSlaveAddrEnable = ENABLE;
    InitObj.CmdInfo.INFO.I2C.extSlaveRAddr = 0xBB;
    InitObj.CmdInfo.INFO.I2C.extSlaveWAddr = 0xBA;
    InitObj.ChgMclkEn = ENABLE;
    return InitObj;
}
...
static void SenPowerOn(void)
{
    gpio_setDir(GPIO_SENSOR_PWR_VDD_A_V28, GPIO_DIR_OUTPUT);
    gpio_setPin(GPIO_SENSOR_PWR_VDD_A_V28);

    gpio_setDir(GPIO_SENSOR_PWR_VDD_IO_V18, GPIO_DIR_OUTPUT);
    gpio_clearPin(GPIO_SENSOR_PWR_VDD_IO_V18);

    gpio_setDir(GPIO_SENSOR_STANDBY, GPIO_DIR_OUTPUT);
    gpio_setPin(GPIO_SENSOR_STANDBY);
    Delay_DelayMs(20);

    gpio_setDir(GPIO_SENSOR_PWR_VDD_D_V12, GPIO_DIR_OUTPUT);
    gpio_setPin(GPIO_SENSOR_PWR_VDD_D_V12);
    Delay_DelayMs(20);
...
static void SenPowerOn3(void)
{
    gpio_setPin(GPIO_SENSOR3_POWER_EN);
    Delay_DelayMs(500);
    gpio_clearPin(GPIO_SENSOR3_PDN);
    Delay_DelayMs(10);
    gpio_setPin(GPIO_SENSOR3_PDN);
    Delay_DelayMs(10);
    gpio_clearPin(GPIO_SENSOR3_RESET);
    Delay_DelayMs(20);
    gpio_setPin(GPIO_SENSOR3_RESET);
    Delay_DelayMs(20);
}
...
static void SenPowerSaveOff3(void)
{
}
...
static void SenPowerSaveOn3(void)
{
}

static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_OV4689,	NULL,   Sensor_GetDrvTab_GM7150,    NULL,
};

static SEN_GET_INI_OBJ SenObjTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetObj,	NULL,   Sensor_GetObj3,    NULL,
};

static FP SenPowerOnTab[SENSOR_ID_MAX_NUM] =
{
    SenPowerOn,		NULL,   SenPowerOn3,   NULL,
};

static FP SenPowerOffTab[SENSOR_ID_MAX_NUM] =
{
    SenPowerOff,	NULL,   SenPowerOff3,   NULL,
};

static FP SenPowerSaveOffTab[SENSOR_ID_MAX_NUM] =
{
    SenPowerSaveOff,	NULL,   SenPowerSaveOff3,    NULL,
};

static FP SenPowerSaveOnTab[SENSOR_ID_MAX_NUM] =
{
    SenPowerSaveOn,	NULL,   SenPowerSaveOn3,    NULL,
};

static BOOL SenDetPlugIn3(void)
{
#if (SENSOR_DET_CTRL == SENSOR_DET_DUMMY)
    return FALSE;
#endif

#if (SENSOR_DET_CTRL == SENSOR_DET_BY_GPIO)
    if (gpio_getPin(GPIO_SENSOR3_CVBS_PLUG) == TRUE && gpio_getPin(GPIO_SENSOR3_PART_DTC) == TRUE)
        return TRUE;
    else
        return FALSE;
#endif
...
void DrvSensor_PowerTurnOn(SENSOR_ID Id)
{
    ...
    DBG_IND("[PWRIC] - sensor open\r\n");
    //PwrIC_DoCmd(cmd_LCD_getbl, 0, &bl); //read LCD bl
    PwrIC_DoCmd(cmd_Sensor_open, 0xE0, 0);  // sensor open by PowerIC
#endif
}
...
void DrvSensor_PowerTurnOff(SENSOR_ID Id)
{
    ...
    //PwrIC_DoCmd(cmd_Sensor_close, 0, 0);  // sensor close by PowerIC
}
