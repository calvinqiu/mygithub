...
#define LCD_MAX_W                       ALIGN_CEIL_16(856) //device w of DISP_LCDMODE_xxx, to support rotate, it must align to 16
...
#define DRVLCD_BRT_LEVEL_INIT     9
...
static void GPIOMap_LCDReset(void)
{
    ...
    gpio_clearPin(GPIO_LCD_RESET_LEGACY);
    SwTimer_DelayMs(10);
    gpio_setPin(GPIO_LCD_RESET);
    gpio_setPin(GPIO_LCD_RESET_LEGACY);
#endif
}
...
static void GPIOMap_TurnOnLCDBacklight(void)
{
    ...
    {
#if (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)
        g_LCDBacklightPWMInfo.uiFall= g_LCDBacklightLvlPWMDuty[g_LCDBacklightLvl];
        DBG_IND("PWM: uiFall=%d\r\n", g_LCDBacklightPWMInfo.uiFall);
        pwm_open(PWMID_LCD_BLG_PCTL);
        pwm_pwmConfig(PWMID_LCD_BLG_PCTL, &g_LCDBacklightPWMInfo);
        //Output Freq=120M/(Div+1)/base period=120M/24/100=50K
        pwm_pwmConfigClockDiv(PWM19_CLKDIV,19);//1199  //23
        pwm_pwmEnable(PWMID_LCD_BLG_PCTL);

		// To backward compatible for old hardware
		gpio_setDir(P_GPIO_30, GPIO_DIR_OUTPUT);
        gpio_setPin(P_GPIO_30);

#elif (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_GPIO)
        gpio_setPin(GPIO_LCD_BLG_PCTL);
#elif (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWRIC)
        PwrIC_DoCmd(cmd_LCD_setbl, 0xF, 0); //read LCD bl
#endif
...
static void GPIOMap_TurnOffLCDBacklight(void)
{
    ...
    {
#if (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)
        g_LCDBacklightPWMInfo.uiFall= 0;
        pwm_pwmConfig(PWMID_LCD_BLG_PCTL, &g_LCDBacklightPWMInfo);
		...
		// To backward compatible for old hardware
		gpio_setDir(P_GPIO_30, GPIO_DIR_OUTPUT);
        gpio_clearPin(P_GPIO_30);

#elif (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_GPIO)
        gpio_clearPin(GPIO_LCD_BLG_PCTL);
#elif (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWRIC)
        PwrIC_DoCmd(cmd_LCD_setbl, 0x0, 0); //read LCD bl
#endif
...
static void GPIOMap_AdjustLCDBacklight(UINT32 uiAdjValue)
{
    ...
    if(g_LCDBacklightEn)
    {
        ...
        DBG_IND("PWM: uiFall=%d\r\n", g_LCDBacklightPWMInfo.uiFall);
        //Output Freq=120M/(Div+1)/base period=120M/24/100=50K
        //pwm_pwmConfigClockDiv(PWM17_CLKDIV,23);//1199
        pwm_pwmEnable(PWMID_LCD_BLG_PCTL);
