...
static UINT32 VolDet_GetKey2Level(void)
{
    ...
    DBG_IND("uiKey2ADC %d \r\n", uiKey2ADC);
    if (uiKey2ADC < VOLDET_KEY_ADC_TH) //768+30
    {
        if (uiKey2ADC > VOLDET_KEY_ADC_LVL3) //768-30
        {
            uiCurKey2Lvl = VOLDET_KEY_LVL_4;
        }
        else if (uiKey2ADC > VOLDET_KEY_ADC_LVL2) //512-30
        {
            uiCurKey2Lvl = VOLDET_KEY_LVL_3;
        }
        else if (uiKey2ADC > VOLDET_KEY_ADC_LVL1) //256-30
        {
            uiCurKey2Lvl = VOLDET_KEY_LVL_2;
        }
        else if (uiKey2ADC > VOLDET_KEY_ADC_LVL0) //<30
        {
            uiCurKey2Lvl = VOLDET_KEY_LVL_1;
        }
        else
...
#if defined(YQCONFIG_POWER_KEY_SWITCH_LCD_TV_SUPPORT)
...
UINT32 DrvKey_DetPowerKey(void)
{
    ...
    UINT32 uiKeyCode = 0;
    BOOL pwr_key_status = 0;
#if defined(YQCONFIG_PWRKEY_LONGPRESS_PWROFF_SUPPORT)
    pwr_key_status = GPIO_GetPowerKey_Status();
#endif
