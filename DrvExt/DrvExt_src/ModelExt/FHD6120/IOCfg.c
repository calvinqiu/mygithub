...
GPIO_INIT_OBJ uiGPIOMapInitTab[] = {
     ...
     {  GPIO_WATCHDOG_EN,       GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST      },
     //SD card host plug det
     //{  GPIO_CARD_HOST_PLUG_DET, GPIO_DIR_INPUT,    PAD_PULLUP,   PAD_CARD_HOST_PLUG_DET},
     ...
     {  GPIO_WIFI_CLK,         GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST      }, //WIFI SHDN, ShutDown
     {  GPIO_WIFI_DAT,         GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST      }, //WIFI SHDN, ShutDown
     {  GPIO_WIFI_DAT0,         GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST      }, //WIFI SHDN, ShutDown
     {  GPIO_WIFI_DAT1,         GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST      }, //WIFI SHDN, ShutDown
     {  GPIO_WIFI_DAT2,         GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST      }, //WIFI SHDN, ShutDown
     {  GPIO_WIFI_DAT3,         GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_PIN_NOT_EXIST      }, //WIFI SHDN, ShutDown
};
...
void IO_InitADC(void)
{
    ...
    if (adc_open(ADC_CH_CARDV_BATTERY) != E_OK)
    {
        DBG_ERR("Can't open ADC channel for battery voltage detection\r\n");
    }
    else
    {
        adc_setChConfig(ADC_CH_CARDV_BATTERY, ADC_CH_CONFIG_ID_SAMPLE_FREQ, 10000); //10K Hz, sample once about 100 us for CONTINUOUS mode
        adc_setChConfig(ADC_CH_CARDV_BATTERY, ADC_CH_CONFIG_ID_SAMPLE_MODE, (CARDV_BATTERY_ADC_MODE) ? ADC_CH_SAMPLEMODE_CONTINUOUS : ADC_CH_SAMPLEMODE_ONESHOT);
        adc_setChConfig(ADC_CH_CARDV_BATTERY, ADC_CH_CONFIG_ID_INTEN, FALSE);
    }
...
BOOL GPIOMap_GPS_Det(void)
{
    return (BOOL)(gpio_getPin(GPIO_GPS_DET)?FALSE:TRUE);
}


BOOL GPIOMap_CardProtect_Det(void)
{
    return (BOOL)(gpio_getPin(GPIO_CARD_WP_DET)?TRUE:FALSE);
}
