...
UINT32 DrvEmbMemInit(void* pInitParam) // Set Init Parameters
{
...

    {
        ...
        {
            if(p->uiPhyAddr != p->uiResvSize)
            {
                //p->uiResvSize given by loader for loader size
                //if your board have no any nor / nand, it gets zero. (you must really mount a nor or nand)
                DBG_ERR("Fw area have to start at 0x%08X.\r\n",p->uiResvSize);
                return E_PAR;
            }

            UINT32 uiPhyAddr = p->uiPhyAddr - p->uiResvSize;
            UINT32 uiPhySize = p->uiPhySize + p->uiResvSize;

            pStrg->SetParam(STRG_SET_MEMORY_REGION, g_pInit->buf.Addr, g_pInit->buf.Size);
            pStrg->SetParam(STRG_SET_PARTITION_SIZE,uiPhyAddr,uiPhySize);
