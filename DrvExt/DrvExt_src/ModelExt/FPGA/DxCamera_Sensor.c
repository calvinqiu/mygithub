...
static void SenPowerOn(void)
{
    ...
    pll_setClkFreq(PLL_CLK_SIEMCLK, 8000000);// for FPGA
#else
    pll_setClkFreq(PLL_CLK_SIEMCLK, 10800000);// for RealChip emulation
#endif
#else
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_NT99141,   Sensor_GetDrvTab_NT99141,   NULL,   NULL,
};
...
static void SenPowerOn(void)
{
    ...
    //pll_setClkFreq(PLL_CLK_SIEMCLK, 5400000);//5.4MHz
    pll_setClkFreq(PLL_CLK_SIEMCLK, 8000000);//8MHz
    pll_setClkEn(PLL_CLK_SIEMCLK, ENABLE);
}
