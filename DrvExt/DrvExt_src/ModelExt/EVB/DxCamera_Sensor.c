...
#elif ((_SENSORLIB_ == _SENSORLIB_CMOS_AR0237CSP_) &&      \
       (_SENSORLIB2_ == _SENSORLIB2_OFF_) &&            \
       (_SENSORLIB3_ == _SENSORLIB3_OFF_) &&            \
       (_SENSORLIB4_ == _SENSORLIB4_OFF_))
extern void SetSensorStreamOn(SENSOR_ID Id);
...
static void SenPowerOn(void)
{
    if (pll_getPLLEn(PLL_ID_5) == DISABLE)
    {
        pll_setPLL(PLL_ID_5, 0x168000);//54MHz
        pll_setPLLEn(PLL_ID_5, ENABLE);
    }
...
//Step2. (Enable MCLK) Wait 1ms, enable SN_MCLK(24MHz)
    ...
    pll_selectClkSrc(PLL_CLK_SIEMCLK, PLL_CLKSRC_PLL5);
    pll_setClkFreq(PLL_CLK_SIEMCLK, 22500000);
    pll_setClkEn(PLL_CLK_SIEMCLK, ENABLE);

//Step3. (Reset sensor) Wait 1ms, S_GPIO7=0. Wait 2ms, S_GPIO7=1.
    Delay_DelayMs(10);
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_AR0237CSP,  NULL,   NULL,   NULL,
};
...
#elif ((_SENSORLIB_ == _SENSORLIB_OFF_) &&      \
       (_SENSORLIB2_ == _SENSORLIB2_CMOS_CCIR601A_) &&            \
       (_SENSORLIB3_ == _SENSORLIB3_OFF_) &&            \
       (_SENSORLIB4_ == _SENSORLIB4_OFF_))
...
static void SenPowerOn2(void)
{
    ...
    gpio_setDir(GPIO_SENSOR_RESET_3V, GPIO_DIR_OUTPUT);
    gpio_clearPin(GPIO_SENSOR_RESET_3V);

    gpio_setDir(GPIO_SENSOR_1V, GPIO_DIR_OUTPUT);
    gpio_clearPin(GPIO_SENSOR_1V);

    gpio_setDir(GPIO_SENSOR_18V, GPIO_DIR_OUTPUT);
    gpio_clearPin(GPIO_SENSOR_18V);

    gpio_setDir(GPIO_SENSOR_29V, GPIO_DIR_OUTPUT);
    gpio_clearPin(GPIO_SENSOR_29V);

    // power on sequence
    gpio_setPin(GPIO_SENSOR_1V);
    Delay_DelayMs(10);

    gpio_setPin(GPIO_SENSOR_18V);
    Delay_DelayMs(10);

    gpio_setPin(GPIO_SENSOR_29V);
    Delay_DelayMs(10);

    gpio_setPin(GPIO_SENSOR_RESET_3V);
    gpio_setPin(GPIO_SENSOR_STANDBY);
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    NULL,   Sensor_GetDrvTab_CCIR601A,   NULL,   NULL,
};
...
#elif ((_SENSORLIB_ == _SENSORLIB_CMOS_IMX317CQC_) &&      \
       (_SENSORLIB2_ == _SENSORLIB2_OFF_) &&            \
       (_SENSORLIB3_ == _SENSORLIB3_OFF_) &&            \
       (_SENSORLIB4_ == _SENSORLIB4_OFF_))
...
static void SenPowerOn(void)
{
    pll_setPLLEn(PLL_ID_5, DISABLE);
    //pll_setPLL(PLL_ID_5, 0xC0000);//72MHz
    pll_setPLL(PLL_ID_5, 0x300000);//288MHz
    pll_setPLLEn(PLL_ID_5, ENABLE);
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_IMX317CQC,   NULL,   NULL,   NULL,
};
