...
//#NT#2016/11/16#Niven Cho -begin
//#NT#EMMC_FLOW
#define CFG_FAKE_EMMC_ON_SD2 DISABLE
#include "sdio.h"
#if (CFG_FAKE_EMMC_ON_SD2)
#define EMB_GETSTRGOBJ(x) sdio2_getStorageObject(x)
#else
#define EMB_GETSTRGOBJ(x) sdio3_getStorageObject(x)
#endif
//#NT#2016/11/16#Niven Cho -end
...
UINT32 DrvEmbMemInit(void* pInitParam) // Set Init Parameters
{
...

    {
        ...
        if(p->uiDxClassType ==(DX_CLASS_STORAGE_EXT|DX_TYPE_EMBMEM0))
        {
			...
			#if (_INTERSTORAGE_ != _INTERSTORAGE_EMMC_) //EMMC not support STRG_GET_DEVICE_PHY_SIZE
            UINT32 PhySize;
...
//#NT#EMMC_FLOW
#if (CFG_FAKE_EMMC_ON_SD2)
		sdio2_setCallBack(SDIO_CALLBACK_CARD_DETECT, (SDIO_CALLBACK_HDL)xSdioDet);
        sdio2_setCallBack(SDIO_CALLBACK_WRITE_PROTECT, (SDIO_CALLBACK_HDL)xSdioWp);
#else
