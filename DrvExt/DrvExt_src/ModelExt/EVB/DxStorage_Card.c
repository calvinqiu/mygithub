...
{
    DXFLAG_SIGN,
    DX_CLASS_STORAGE_EXT|DX_TYPE_CARD2,
    STORAGE_VER,
    "Storage_Card2",
    0,0,0,0,
    DrvCARDGetcaps2,
...
UINT32 DrvCARDInit(void* pInitParam) // Set Init Parameters
{
    ...
    */
...
#endif
#if 0 //config to 96MHz that cause the worst compatibility
	// Support UHSI
    ...
    sdio_setConfig(SDIO_CONFIG_ID_BUS_SPEED_MODE, SDIO_BUS_SPEED_MODE_DS | SDIO_BUS_SPEED_MODE_HS | SDIO_BUS_SPEED_MODE_SDR50);
#endif
    //if(STRG_OBJ_FAT1)
    {
...
UINT32 DrvCARDInit2(void* pInitParam) // Set Init Parameters
{
    ...
    sdio2_setCallBack(SDIO_CALLBACK_CARD_DETECT, (SDIO_CALLBACK_HDL)DrvCARD_DetStrgCard_TRUE);
    sdio2_setCallBack(SDIO_CALLBACK_WRITE_PROTECT, (SDIO_CALLBACK_HDL)DrvCARD_DetStrgCardWP_FALSE);
