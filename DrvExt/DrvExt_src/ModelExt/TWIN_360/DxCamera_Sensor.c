...
#elif ((_SENSORLIB_ == _SENSORLIB_CMOS_OV4689_) && \
       (_SENSORLIB2_ == _SENSORLIB2_CMOS_OV4689_) && \
       (_SENSORLIB3_ == _SENSORLIB3_OFF_) && \
       (_SENSORLIB4_ == _SENSORLIB4_OFF_))
#if _FPGA_EMULATION_
#include "pll_protected.h"
...
static void SenPowerOn(void)
{
    ...
    gpio_setDir(S_GPIO_2, GPIO_DIR_OUTPUT); //standby for sensor 1
    gpio_setPin(S_GPIO_2);
    gpio_setDir(S_GPIO_3, GPIO_DIR_OUTPUT); //standby for sensor 2
    gpio_setPin(S_GPIO_3);
...
{
    gpio_clearPin(GPIO_SENSOR2_RESET);
    gpio_clearPin(GPIO_SENSOR2_STANDBY);
    pll_setClkEn(PLL_CLK_SIEMCLK2, DISABLE);
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_OV4689,   Sensor_GetDrvTab_OV4689,   NULL,   NULL,
};
