...
#elif ((_SENSORLIB_ == _SENSORLIB_CMOS_IMX322LQJ_THCV236_) &&      \
...
UINT32 SenPowerOn_flag = FALSE;

static void SenPowerOn(void)
{
    if (SenPowerOn_flag == TRUE)
    {
        return;
    }
    SenPowerOn_flag = TRUE;
...
static void SenPowerOff(void)
{
    if (SenPowerOn_flag == FALSE)
    {
        return;
    }
    SenPowerOn_flag = FALSE;
...
static SEN_GET_DRV SenDrvTab[SENSOR_ID_MAX_NUM] =
{
    Sensor_GetDrvTab_IMX322LQJ_THCV236,   Sensor_GetDrvTab_IMX322LQJVX1,   NULL,   NULL,
};
...
static BOOL SenDetPlugIn(void)
{
	...
	SENSOR_CMD detect_reg = {0};
	detect_reg.uiAddr = 0x50;
	detect_reg.uiDataLen = 2;
	detect_reg.uiData[1] = 0x654321; //MAGIC!
	SENSOR_INIT_OBJ SenObj;
    SENSOR_DRVTAB *SenTab;

	    SenPowerOn();

	    SenObj = DrvSensor_GetObj(SENSOR_ID_1);
		SenTab = DrvSensor_GetTab(SENSOR_ID_1);
		Sensor_Open(SENSOR_ID_1, &SenObj, SenTab);
    read_rt = Sensor_ReadReg(SENSOR_ID_1, &detect_reg);
...
{
#if (SENSOR_DET_CTRL != SENSOR_DET_DUMMY)
    SenDetPlugIn,   SenDetPlugIn2,   NULL,   NULL,
#else
    NULL,   NULL,   NULL,   NULL,
#endif
};
