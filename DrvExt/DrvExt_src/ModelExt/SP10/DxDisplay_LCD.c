...
extern void DrvIDEEffectControl(PDISPCTRL_PARAM pCtrl, PDISP_OBJ pObj,BOOL ide_en);
...
static void DrvLCD_TurnOn(void)
{
   ...
   DrvIDEEffectControl(pCtrl,pObj,TRUE);//will
...
static void DrvLCD_SleepLeave(void)
{
    ...
    pDev->SEL.OPEN_DEVICE.DevID = DISPDEV_ID_PANEL;
    pObj->devCtrl(DISPDEV_OPEN_DEVICE, pDev);
	#ifdef CUST_MENU_SENSOR_FLIP
	//LCDRotate(SensorGetFlipFlag());
	#endif
    //////////////////////////////////////////////////////////////////////
    // special operation for this project!
    //
...
*/
#ifdef CUST_MENU_SENSOR_FLIP
extern BOOL panelFlipActiveFlag;
#endif
static void GPIOMap_TurnOnLCDBacklight(void)
{
...
#if 0//will close def CUST_MENU_SENSOR_FLIP
	debug_msg("LcdBackLight_TurnOn GPIOMap panelFlipActiveFlag=%d \r\n",panelFlipActiveFlag);
       #ifdef ORDER_SHOW_LOGO_TIME
	   	gpio_setPin(GPIO_LCD_BLG_PCTL);
	#else
		if(panelFlipActiveFlag==TRUE)
			gpio_setPin(GPIO_LCD_BLG_PCTL);
	#endif
#else
        gpio_setPin(GPIO_LCD_BLG_PCTL);
