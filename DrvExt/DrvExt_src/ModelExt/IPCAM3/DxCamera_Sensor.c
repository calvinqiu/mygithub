...
//for 0230 ipcam
    ...
    //Step1. (Set default) S_GPIO7=1, P_GPIO36/P_GPIO37=0
    ...
    gpio_clearPin(GPIO_SENSOR_28V);

    //Step1. (Enable sensor power) Wait 5ms, P_GPIO36=1, wait 5ms, P_GPIO37=1
    Delay_DelayMs(5);
    gpio_setDir(GPIO_SENSOR_18V,GPIO_DIR_OUTPUT);
    ...
    //Step2. (Master clock) Wait 30ms, enable SN_MCLK.
    Delay_DelayMs(30);
    pll_selectClkSrc(PLL_CLK_SIEMCLK, PLL_CLKSRC_PLL5);
    pll_setClkFreq(PLL_CLK_SIEMCLK, 27000000);
    pll_setClkEn(PLL_CLK_SIEMCLK, ENABLE);
    //Step3. (Reset sensor) Wait 5ms, S_GPIO7=0, wait 5ms, S_GPIO7=1.
    Delay_DelayMs(5);
    gpio_setDir(GPIO_SENSOR_RESET, GPIO_DIR_OUTPUT);
    gpio_clearPin(GPIO_SENSOR_RESET);
    Delay_DelayMs(5);
    gpio_setDir(GPIO_SENSOR_RESET, GPIO_DIR_OUTPUT);
    gpio_setPin(GPIO_SENSOR_RESET);
    //Step4. (Configure sensor) Wait 1s, configure sensor by I2C (S_GPIO5, S_GPIO6) interface.
    Delay_DelayMs(1000);

    //The sensor I2C slave address is 0x20. Please make sure bit R0x306E[9]=1 in the configurations.
