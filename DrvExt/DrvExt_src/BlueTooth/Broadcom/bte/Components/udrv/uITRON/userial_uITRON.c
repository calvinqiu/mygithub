...
{
    uint32_t c = uart2_write((uint8_t*)data_out, size);
...
{
    int32_t c = uart2_read(data_in, size);
...
{
    if(v)
        uart2_setConfig(UART_CONFIG_ID_BAUDRATE, v);
}
/*******************************************************************************
 **
...
#if 0
static void uart2_lockRts(void)
{
    UINT32 uiReg;
...
static void uart2_unlockRts(void)
{
    UINT32 uiReg;
    uiReg = *(UINT32 *)(0xC0010000 + 0xAC);
    uiReg &= ~(0x00000002);
...
static void set_rts_enable(int enable)
{
    ...
    enable ? uart2_unlockRts() : uart2_lockRts();
    rts_state = !!enable;
    #endif
}
...
void test_btuart_hw(void)
{
    ...
    UINT32 r = uart2_write(cmd_reset, sizeof(cmd_reset));
    debug_trace("status %d\r\n", r);

    GKI_delay(1000);

    debug_trace("<- hci reset evt\r\n");
    //r = uart2_read(evt_reset, 7);
    r = uart2_read(evt_reset, 1);
    r = uart2_read(evt_reset + 1, 2);
    r = uart2_read(evt_reset + 3, 4);
    debug_trace("status %d\r\n", r);
...
{    
    ...
    UINT32 r = uart2_write(cmd + 1, length); 
     debug_trace("status %d\r\n", r);
    
    GKI_delay(delay);   
    r = uart2_read(evt_reset, 1);       debug_trace("read status %d\r\n", r);
    r = uart2_read(evt_reset + 1, 2);   debug_trace("read status %d\r\n", r);
    length = 0x0ff & evt_reset[2];
    r = uart2_read(evt_reset + 3, length);    
