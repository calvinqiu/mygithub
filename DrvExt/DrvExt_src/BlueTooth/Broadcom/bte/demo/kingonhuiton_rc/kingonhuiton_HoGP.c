...
kingonhuiton_rc_cb_t kingonhuiton_rc_cb;
...
static void load_stored_rc_info(void)
{
    ...
    app_rc_store.rclist[0][0] = 0x11;
    app_rc_store.rclist[0][1] = 0x00;
    app_rc_store.rclist[0][2] = 0x54;
    app_rc_store.rclist[0][3] = 0x45;
    app_rc_store.rclist[0][4] = 0x45;
    app_rc_store.rclist[0][5] = 0x4B;
}
...
void application_start(void)
{
    ...
    kingonhuiton_rc_init(&kingonhuiton_rc_cb);
    load_stored_rc_info();
...
static BOOLEAN bt_le_enable_hogp_report(void)
{
    ...
    for (i = 0; i < MAX_HOGP_REPORT; i++)
    {
        ...
        if ((kingonhuiton_rc_cb.hogp_report[i].report_ccc_value != 0x01) && (kingonhuiton_rc_cb.hogp_report[i].report_ccc_handle != 0))
        {
            result = TRUE;
            hid_ccc_handle = kingonhuiton_rc_cb.hogp_report[i].report_ccc_handle;
            kingonhuiton_rc_cb.hogp_report[i].report_ccc_value = 0x01;
            break;
        }
    }
...
static void bt_le_disable_hogp_report(void)
{
    ...
    for (i = 0; i < MAX_HOGP_REPORT; i++)
    {
        if ((kingonhuiton_rc_cb.hogp_report[i].report_ccc_value != 0) && (kingonhuiton_rc_cb.hogp_report[i].report_ccc_handle != 0))
        {
            kingonhuiton_rc_cb.hogp_report[i].report_ccc_value = 0;
        }
    }
}
...
static void bt_le_proximity_handle_discovery_result(wiced_bt_gatt_event_data_t *p_event_data)
{
    ...
    if (proximity_monitor_state == HOGP_HOST_STATE_DISCOVERING_SERVICES)
    {
        if (GATT_DISCOVERY_RESULT_SERVICE_UUID_LEN(p_event_data) == LEN_UUID_16)
        {
            ...
            if (service_uuid == kingonhuiton_rc_cb.hid_service.uuid)
            {
                WPRINT_BT_APP_INFO(("HID service found\r\n"));
                kingonhuiton_rc_cb.hid_service.shandle = GATT_DISCOVERY_RESULT_SERVICE_START_HANDLE(p_event_data);
                kingonhuiton_rc_cb.hid_service.ehandle = GATT_DISCOVERY_RESULT_SERVICE_END_HANDLE(p_event_data);
            }
        }
    }
    ...
    else if (proximity_monitor_state == HOGP_HOST_STATE_DISCOVERING_DESCRIPTERS)
    {
        ...
        if (GATT_DISCOVERY_RESULT_CHARACTERISTIC_DESCRIPTOR_UUID_LEN(p_event_data) == LEN_UUID_16)
        {
            ...
            case GATT_UUID_HID_REPORT_MAP:
                ...
                kingonhuiton_rc_cb.handle_reportMap = GATT_DISCOVERY_RESULT_CHARACTERISTIC_DESCRIPTOR_VALUE_HANDLE(p_event_data);
                break;
            ...
            case GATT_UUID_CHAR_CLIENT_CONFIG:
            {
                ...
                for (i = 0; i < MAX_HOGP_REPORT; i++)
                {
                    if(!kingonhuiton_rc_cb.hogp_report[i].report_ccc_handle)
                    {
                        kingonhuiton_rc_cb.hogp_report[i].report_ccc_handle = attr_hdl;
                        break;
                    }
                }
            }
...
static void bt_le_proximity_handle_discovery_complete(void)
{
    ...
    if (proximity_monitor_state == HOGP_HOST_STATE_DISCOVERING_SERVICES)
    {
        ...
        if (kingonhuiton_rc_cb.hid_service.shandle)
        {
            proximity_monitor_state = HOGP_HOST_STATE_DISCOVERING_DESCRIPTERS;
            discovery_parameters.s_handle = kingonhuiton_rc_cb.hid_service.shandle;
            discovery_parameters.e_handle = kingonhuiton_rc_cb.hid_service.ehandle;
    ...
    else if (proximity_monitor_state == HOGP_HOST_STATE_DISCOVERING_DESCRIPTERS)
    {
        ...
        if (kingonhuiton_rc_cb.hid_service.shandle)
        {
            ...
            read_parameters.by_handle.handle = kingonhuiton_rc_cb.handle_reportMap;
...
static void bt_le_proximity_handle_operation_complete(wiced_bt_gatt_event_data_t *p_event_data)
{
    ...
    if (proximity_monitor_state == HOGP_HOST_STATE_DISCOVERING_REPORT_DESP_ATTRIBUTE)
    {
        ...
        {
            memcpy(kingonhuiton_rc_cb.hid_descriptor, p_event_data->operation_complete.response_data.att_value.p_data, p_event_data->operation_complete.response_data.att_value.len);
            kingonhuiton_rc_cb.hid_desp_len = p_event_data->operation_complete.response_data.att_value.len;
        }
...
static void bt_le_proximity_start_discovery(void)
{
    ...
    if (proximity_monitor_state == HOGP_HOST_STATE_IDLE)
    {
        ...
        discovery_parameters.uuid.uu.uuid16 = kingonhuiton_rc_cb.hid_service.uuid;
...
static void scan_result_cback (wiced_bt_ble_scan_results_t *p_scan_result, uint8_t *p_adv_data)
{
    ...
    if(!connection_initiated && p_scan_result->remote_bd_addr[0] == 0x11 && p_scan_result->remote_bd_addr[1] == 0x00 &&
        p_scan_result->remote_bd_addr[2] == 0x54 && p_scan_result->remote_bd_addr[3] == 0x45 &&
        p_scan_result->remote_bd_addr[4] == 0x45 && p_scan_result->remote_bd_addr[5] == 0x4B)
    {
        /* Stop scanning */
