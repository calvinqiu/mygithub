...
#define GPS_debug(arg)              DBG_MSG arg
#else
...
ER GPSRec_Open(UART_BAUDRATE BaudRate)
{
    ...
    #if (_GPS_RECEIVE_FROM_ == RECEIVE_FROM_UART2)
    if (uart2_open() == E_OK)
    {
        DBG_DUMP("\r\nUART2: Open success\r\n");
    ...
    #elif (_GPS_RECEIVE_FROM_ == RECEIVE_FROM_UART3)
    if (uart3_open() == E_OK)
    {
        DBG_DUMP("\r\nUART3: Open success\r\n");
...
void GPSRecTsk(void)
{
    ...
    UINT32  YuanEdog_uiLen;
    CHAR    YuanEdog_RecSentence[NMEA_SENTENCE_SIZE];
#endif
    ...
    while(GPSRECEIVE_ID)
    {
        ...
        #endif
            DBG_ERR("UART2: parameter error!\r\n");

    #if defined(YQCONFIG_YUANEDOG_SUPPORT)
        YuanEdog_uiLen = uiLen;
        strncpy(YuanEdog_RecSentence, RecSentence, sizeof(YuanEdog_RecSentence));
    #endif
		...
		#if defined(YQCONFIG_YUANEDOG_SUPPORT)
			YuanEdog_RecSentence[YuanEdog_uiLen]='\r';
			YuanEdog_RecSentence[YuanEdog_uiLen+1]='\n';			
			YuanEdog_RecSentence[YuanEdog_uiLen+2]='\0';
			gps_String_RxCallback(YuanEdog_uiLen+2,YuanEdog_RecSentence);//added jacky lan for Yuan Edog
		#endif
