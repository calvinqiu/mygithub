...
*/
//@{
/*Used in MI format-3 RGB666 8bits bus*/
...
const tLCD_PARAM tModeMI[] =
{
    /***********       MI Format 3 - 8bits bus RGB666   *************/
    {
        ...
        {
            /* Old prototype */
            PINMUX_LCD_SEL_PARALLE_MI_8BITS,//!< pinmux_select_lcd;
            ICST_CCIR601,                   //!< icst;
...
{
	if(tCmdStandbyMI != NULL)
	{
		//Already determine if(tCmdStandbyMI != NULL)
    	//coverity[bad_sizeof]
...
{
    mi_setCmd1(WrCMD, ((addr&0xff00)>>8)<<0);
    mi_setCmd2(WrCMD, (addr&0x00ff)<<0);
    mi_setCmd3(WrDATA,((value&0xff00)>>8)<<0);
    mi_setCmd4(WrDATA,(value&0x00ff)<<0);
    mi_setCmdCnt(4);
    mi_waitCmdDone();
}
...
#if (_FPGA_EMULATION_)
    FLOAT   MiTargetClk = 12.00; // Max is 60MHz
#else
...
#else
    mi_setCtrl(PARALLEL_M68, MI_AUTOFMT3, pMiConfig->DataFmt, FALSE, FALSE, bTeSyncEn, FALSE);
#endif
...
{
    /* Enable AUTO Mode */
    mi_setCmd1(WrCMD, 0x00<<0);
    mi_setCmd2(WrCMD, 0x21<<0);
    //mi_setCmd3(WrDATA,0x9F<<0);
    mi_setCmd3(WrDATA,0xA0<<0);
    mi_setCmd4(WrDATA,0x00<<0);
    mi_setCmd5(WrCMD, 0x00<<0);
    mi_setCmd6(WrCMD, 0x22<<0);
    mi_setCmdCnt(6);
    mi_waitCmdDone();
}