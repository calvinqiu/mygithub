...
#define _IDE_fDCLK      55.00f //24.00f// must > ((((DSI_TARGET_CLK / 8) * 2) / 3)) ->2: lane number
#else
...
const tLCD_PARAM tModeDSI[] =
{
    /***********       MI Serial Format 1      *************/
    {
        // tPANEL_PARAM
        {
            ...
            (0xF0+PANEL_WIDTH+2), //744,//746,                            //!< uiHSyncTotalPeriod(HTOTAL)
            #else
            744,                            //!< uiHSyncTotalPeriod(HTOTAL)
            #endif
            PANEL_WIDTH,                    //!< uiHSyncActivePeriod(HACT)
            HVALIDST,                           //!< uiHSyncBackPorch(HBP)
          0x20 + PANEL_HEIGHT,// 1280,// 1304,                           //!< uiVSyncTotalPeriod(VTOTAL)
            PANEL_HEIGHT, //1270,// PANEL_HEIGHT,                   //!< uiVSyncActivePeriod
