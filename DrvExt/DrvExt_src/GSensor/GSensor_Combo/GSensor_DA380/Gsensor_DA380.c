...
#define __MODULE__          GSensor_GMA380
...
*/
	...
	res |= mir3da_register_mask_write(NSA_REG_INT_LATCH, 0x8F, 0x85);//0x8F
...
int mir3da_open_interrupt(int num)
{
	...
	res = mir3da_register_write(NSA_REG_ACTIVE_THRESHOLD,0X10);//0X20);//0x26 );////DEBUG//38//DF//ÉèÖÃGSensorÁéÃô¶È
	...
	switch(num){
      ...
      debug_msg("--mir3da_open_interrupt= %x-----\r\n",res); 
	return res;
}
...
BOOL GSensor_DA380_ParkingMode(void)
{
	int   res = 0;
      ...
      res=mir3da_open_interrupt(0);//add auto start
      debug_msg("GSensor_DA380_ParkingMode\r\n");
}
...
{
       debug_msg((">>>>GSensor_DA380_SetInterrupt:%d-----\r\n",En));
	if(En==TRUE)
