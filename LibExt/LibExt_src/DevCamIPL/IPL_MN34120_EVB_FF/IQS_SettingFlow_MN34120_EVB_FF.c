#ifndef _IQS_SETTINGFLOW_MN34120_EVB_FF_C
#define _IQS_SETTINGFLOW_MN34120_EVB_FF_C
#include "IPL_MN34120_EVB_FF_Int.h"

#if CAL_ECS
#include "IPL_Cal_DataPassing_MN34120_EVB_FF_Int.h"
#endif
#include "NOTE.h"
...
UINT32 IQS_GetISORange(UINT32 Id, IQS_FLOW_MSG MsgID, UINT32 *IsoValue, UINT32 *IsoStart, UINT32 *IsoEnd, UINT32 *IQIdxL, UINT32 *IQIdxH)
{
    ...
    IPL_GetSensorStatus(Id, IPL_AlgGetUIInfo(Id, IPL_SEL_PRVSENMODE), &SensorInfo);
    ...
    *IsoValue = ISO;
    ...
    else if ( SenId_Value == SENSOR_ID_1 )
    {
        if ( IQS_IsCapFlow(MsgID)==FALSE )  // movie mode
        {
            if(SensorInfo.Mode->FrameRate >= 2400)
            {
                if ( ISO >=IQS_PRV1B8_ISOEND_VALUE )
                {
                    *IQIdxL = IQS_PRV1B8_ISOEND;
                    *IQIdxH = IQS_PRV1B8_ISOEND;
                    *IsoStart = IQS_PRV1B8_ISOEND_VALUE;
                    *IsoEnd = IQS_PRV1B8_ISOEND_VALUE;
                    return FALSE;
                }
                ISOMap = &(IQS1B8_ISOMapTab[0][0]);
            }
            else if(SensorInfo.Mode->FrameRate >= 1200)
            {
                if ( ISO >=IQS_PRV1B4_ISOEND_VALUE )
                {
                    *IQIdxL = IQS_PRV1B4_ISOEND;
                    *IQIdxH = IQS_PRV1B4_ISOEND;
                    *IsoStart = IQS_PRV1B4_ISOEND_VALUE;
                    *IsoEnd = IQS_PRV1B4_ISOEND_VALUE;
                    return FALSE;
                }
                ISOMap = &(IQS1B4_ISOMapTab[0][0]);
            }
            else if(SensorInfo.Mode->FrameRate >= 600)
            {
                if ( ISO >=IQS_PRV1B2_ISOEND_VALUE )
                {
                    *IQIdxL = IQS_PRV1B2_ISOEND;
                    *IQIdxH = IQS_PRV1B2_ISOEND;
                    *IsoStart = IQS_PRV1B2_ISOEND_VALUE;
                    *IsoEnd = IQS_PRV1B2_ISOEND_VALUE;
                    return FALSE;
                }
                ISOMap = &(IQS1B2_ISOMapTab[0][0]);
            }
            else
            {
                if ( ISO >=IQS_PRV1_ISOEND_VALUE )
