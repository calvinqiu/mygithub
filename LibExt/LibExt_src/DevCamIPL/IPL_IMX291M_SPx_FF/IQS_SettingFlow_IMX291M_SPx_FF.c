#ifndef _IQS_SETTINGFLOW_IMX291M_SPx_FF_C
#define _IQS_SETTINGFLOW_IMX291M_SPx_FF_C
...
static ER SIEx_IQparam(UINT32 Id, IQS_FLOW_MSG MsgID, SIE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    Sensor_GetCurMode(SenId_Value, &senMode);


    if (senMode == SENSOR_MODE_1)
       pOBsub = IQS_OBSub30;
    else //if(senMode == SENSOR_MODE_2)
    {
      if(ExpT>16670)
        pOBsub = IQS_OBSub60_33;
      else
        pOBsub = IQS_OBSub;
    }
...
//#NT#2016/06/15#Ted -end
            ...
            // SIE-OB Substration
            ////////////////////
            if(MsgID == IQCB_PREIME_D2D_SIE_PRI||MsgID == IQCB_PRV_SIE_INIT)
            {
                 group->sieUpdate |= (
                                       SIE_OBOFS_          |   ///< update ob offset
                                       SIE_DGAIN_          |   ///< update Digital gain
                 ...
                 if (ISO>1599)
                 {
                    UINT32 Dgain = (ISO*128)/1599;
                    subf->DGain.DigitalGain=(Dgain>1023)?1023:Dgain;
                 }
                 else
                    subf->DGain.DigitalGain=128;
            }
                 //subf->DGain.DigitalGain=2*128;
            ////////////////////
            // SIE-LA GAMMA
            ////////////////////
