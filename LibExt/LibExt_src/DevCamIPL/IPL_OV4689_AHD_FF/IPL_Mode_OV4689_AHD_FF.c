...
BOOL Enable_SIE2_656pPatch = FALSE;//TRUE;

UINT32 IPL_SetSensorMode(IPL_PROC_ID Id, IPL_MODE Mode, BOOL SetFlg)
{
    UINT32 Video_FPS, Video_SIZE, SenMode = SENSOR_MODE_UNKNOWN;

    if (Id == IPL_ID_1 || (Id == IPL_ID_2 && IPL_CtrlGetInfor(IPL_ID_1, IPLCTRL_EXT_FUNC_EN) == ICF_FUNC_SHDR) )    //OV4689 Sensor Mode
    {
        if (Id == IPL_ID_2)
        {
        ...
        switch(Mode)
        {
            case IPL_MODE_INIT:
                if (IPL_AlgGetUIInfo(IPL_ID_1, IPL_SEL_SHDR) == SEL_SHDR_ON)
                    SenMode = SENSOR_MODE_1;    //HDR Mode
                else
                    SenMode = SENSOR_MODE_3;    //Linear Mode
                break;
            ...
            case IPL_MODE_VIDEOREC:
                ...
                {
                    if ( Video_FPS == SEL_VIDEOFPS_24 )
                        SenMode = SENSOR_MODE_7;
                    else if ( Video_FPS == SEL_VIDEOFPS_60 )
                        SenMode = SENSOR_MODE_4;
                    else if ( Video_FPS == SEL_VIDEOFPS_120 )
                        SenMode = SENSOR_MODE_5;
                    else if ( Video_FPS == SEL_VIDEOFPS_240 )
                        SenMode = SENSOR_MODE_9;
                    else
                        SenMode = SENSOR_MODE_3;
                }
    ...
    else if (Id == IPL_ID_2)
    {
        ...
        // Read AHD register to determine mode information
        SenObj = DrvSensor_GetObj(Id);
        SenTab = DrvSensor_GetTab(Id);

        Cmd.uiAddr = 0xFF;
        Cmd.uiDataLen = 0x01;
        Cmd.uiData[0] = 0x00;
        Cmd.uiData[1] = 0x00;
        SenTab->fpWrite(Id, &Cmd);
        Cmd.uiAddr = 0xD0;
        Cmd.uiDataLen = 0x01;
        Cmd.uiData[0] = 0x00;
        Cmd.uiData[1] = 0x00;
        SenTab->fpRead(Id, &Cmd);
        //DBG_ERR("AHD regD0 = 0x%02X, 0x%02X\r\n", Cmd.uiData[0], Cmd.uiData[1]);

        switch(Cmd.uiData[0])
        {
        case 0x04:
        case 0x08:
            DBG_ERR("720P camera found.\r\n");
		SenMode = SENSOR_MODE_1;
            break;
        case 0x40:
        case 0x80:
            DBG_ERR("1080P camera found.\r\n");
            SenMode = SENSOR_MODE_2;
            break;
        default:
            DBG_ERR("Unknown camera. Set to default mode (720P).\r\n");
            SenMode = SENSOR_MODE_1;
        }
...
static ER IPL_Init(IPL_CHGMODE_DATA *ChgMode, IPL_CHGMODE_DATA *PrvMode)
{
    ...
    //IPL data initial
    UINT32 Video_SIZE = IPL_AlgGetUIInfo(ChgMode->ProcSenId, IPL_SEL_VIDEO_SIZE);
    IPL_AlgSetUIInfo(ChgMode->ProcSenId, IPL_SEL_VIDEO_SIZE, SEL_VIDEOSIZE_2880x2160);
    ZoomTab = SenMode2Tbl(ChgMode->ProcSenId, IPL_AlgGetUIInfo(ChgMode->ProcSenId, IPL_SEL_PRVSENMODE), &ZoomTabMax);
    IPL_AlgSetUIInfo(ChgMode->ProcSenId, IPL_SEL_VIDEO_SIZE, Video_SIZE);
    ...
    //#NT#2016/08/10#Jeffrey Teng -begin
    //#NT# // AHD must set this parameter
    sie2_656pPatch_Enable(Enable_SIE2_656pPatch);
    //#NT#2016/08/10#Jeffrey Teng -end
