...
void ExtFunc_Init(UINT32 Id, ENG_SUBFUNC *Info, IPL_HAL_GROUP* group)
{
    ...
    else
    {
		...
		//#NT#2017/03/02#Wendy Liao -begin
		//#NT# modify for pre_WDR_mode
        if( pre_WDR_mode[Id] != IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) )
            ExtFunc_Reset(Id);
		//#NT#2017/03/02#Wendy Liao -end
		...
		//#NT#2017/03/02#Wendy Liao -begin
		//#NT# modify for pre_WDR_mode
        pre_WDR_mode[Id] = IPL_AlgGetUIInfo(Id, IPL_SEL_WDR);
		//#NT#2017/03/02#Wendy Liao -end
    }
}
...
void ExtFunc_Process(UINT32 Id)
{
	...
	//#NT#2017/01/06#Jarkko Chang -begin
    //#NT# modify for wdr_defog
    else if ((IPL_CtrlGetInfor(Id, IPLCTRL_EXT_FUNC_EN) == ICF_FUNC_WDR) && (IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_ON || IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_AUTO || IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_DEFOG))
	//#NT#2017/01/06#Jarkko Chang -end
    {
        ...
        if (gWdrStep[Id] == 0)
        {
			...
			if(IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_AUTO)
			{
...
void ExtFunc_SetWDRLumaLevel(UINT32 Id, UINT32 Level)
{
    ...
    if(IPL_CtrlGetInfor(Id, IPLCTRL_WDR_VER) == WDR_VERSION_2_5)
    {
        ...
        else if( IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_ON )
        {
            UINT32 tmpLevel = 0;
