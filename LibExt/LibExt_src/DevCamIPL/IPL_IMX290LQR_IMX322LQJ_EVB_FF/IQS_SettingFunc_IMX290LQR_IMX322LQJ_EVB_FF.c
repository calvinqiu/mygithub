#ifndef _IQS_SETTINGFUNC_IMX290LQR_IMX322LQJ_EVB_FF_C
#define _IQS_SETTINGFUNC_IMX290LQR_IMX322LQJ_EVB_FF_C
#include "IPL_IMX290LQR_IMX322LQJ_EVB_FF_Int.h"
#include "HwMem.h"
...
void IQS_DCE_ParamSetting(UINT32 Id, UINT32 MsgID, UINT32 ZoomIdx, DCE_Distortion* pDistortion, DIST* pDce_Dist, Fact_Norm* pRadious)
{
    ...
    if ( IQS_IsCapFlow(MsgID) ) //capture
        pDistortion->Fov.Gain = 1024;//DCE_GetFovGain(Id, TRUE, pDistortion->GeoLUT.pGGeoLut);//lydia for IMX291
    else                        //preview
        pDistortion->Fov.Gain = 1024;//DCE_GetFovGain(Id, FALSE, pDistortion->GeoLUT.pGGeoLut);//lydia for IMX291
...
UINT32 g3DNR[IPL_ID_MAX_NUM]={0}, g3DNR_Counter[IPL_ID_MAX_NUM]={0}, IQS_MCTF_KeepCnt[IPL_ID_MAX_NUM]={0};
//#NT#2016/10/24#Wendy Liao -begin
//#NT#Add 3DNR control uart command
INT32 UI_3DNR[IPL_ID_MAX_NUM] = {-1};
//#NT#2016/10/24#Wendy Liao -end
INT32 IQS_MCTF_AE_AVG_old[IPL_ID_MAX_NUM]={0}; //12bits
...
UINT32 IQS_GetMCTFLevel(UINT32 Id)
{
    ...
    else
    {
        ...
        if(Id == 0)
        {
            if     (uiISO1 <= 100)   ui3DNRLevel = H264_3DNR_LV0;
            else if(uiISO1 <= 150)   ui3DNRLevel = H264_3DNR_LV1;
            else if(uiISO1 <= 300)   ui3DNRLevel = H264_3DNR_LV2;
            else if(uiISO1 <= 600)   ui3DNRLevel = H264_3DNR_LV3;
            else if(uiISO1 <= 850)   ui3DNRLevel = H264_3DNR_LV4;
            else if(uiISO1 <= 1200)  ui3DNRLevel = H264_3DNR_LV5;
            else if(uiISO1 <= 3200)  ui3DNRLevel = H264_3DNR_LV5;
            else if(uiISO1 <= 5400)  ui3DNRLevel = H264_3DNR_LV5;
            else if(uiISO1 <= 10000) ui3DNRLevel = H264_3DNR_IPCAM_LV8;
            else                     ui3DNRLevel = H264_3DNR_IPCAM_LV9;
        }
        if(Id == 1)
        {
            if     (uiISO1 <= 100)   ui3DNRLevel = H264_3DNR_LV0;
            else if(uiISO1 <= 151)   ui3DNRLevel = H264_3DNR_LV1;
            else if(uiISO1 <= 300)   ui3DNRLevel = H264_3DNR_LV2;
            else if(uiISO1 <= 600)   ui3DNRLevel = H264_3DNR_LV3;
            else if(uiISO1 <= 850)   ui3DNRLevel = H264_3DNR_LV4;
            else if(uiISO1 <= 1200)  ui3DNRLevel = H264_3DNR_LV5;
...
if (IQS_IPCAM_ENABLE)
{
    ...
    ui3DNRLevel = T_CLAMP((INT32)((INT32)ui3DNRLevel+IQUC_CtrlItem[Id][IQUC_CtrlIndex[Id]].ADJ_ITEM[ADJ_TNR]), H264_UI_3DNR_OFF, H264_UI_3DNR_LV6);
#endif

    if(IQS_IPCAM_ENABLE)
        g3DNR[Id] = IQS_CLAMP(ui3DNRLevel, H264_UI_3DNR_OFF, H264_UI_3DNR_LV6);
    else
        g3DNR[Id] = IQS_CLAMP(ui3DNRLevel, H264_UI_3DNR_OFF, H264_UI_3DNR_LV6);
...
UINT32 IQS_Antifog(void)
{
   ...
   IPL_AlgSetUIInfo(0, IPL_SEL_IPC_MODE, SEL_IPC_MODE_ON);
    ...
    else level = IPL_AlgGetUIInfo(0, IPL_SEL_GAMMA_ANTI_FOG_LEVEL);
  //  DBG_ERR("Gin:%x,Gout:%x\r\n",IQS_selUserGamma(IPL_AlgGetUIInfo(0, IPL_SEL_USER_GAMMA_MODE)),IQS_UserGAMMA);
    IQS_ToneCurveAdj(IQS_selUserGamma(IPL_AlgGetUIInfo(0, IPL_SEL_USER_GAMMA_MODE)),IQS_UserGAMMA,level);
...
{
    if (IPL_AlgGetUIInfo(0, IPL_SEL_ANTI_FOG)==SEL_ANTI_FOG_OFF)
       // memcpy(IQS_selUserGamma(IPL_AlgGetUIInfo(0, IPL_SEL_USER_GAMMA_LEVEL)), IQS_UserGAMMA , sizeof(UINT32)*130);
       return IQS_selUserGamma(IPL_AlgGetUIInfo(0, IPL_SEL_USER_GAMMA_MODE));
    else
        //IQS_ToneCurveAdj(IQS_selUserGamma(IPL_AlgGetUIInfo(0, IPL_SEL_USER_GAMMA_LEVEL)),IQS_UserGAMMA,IPL_AlgGetUIInfo(0, IPL_SEL_GAMMA_ANTI_FOG_LEVEL));
    return IQS_UserGAMMA;
}
#endif
