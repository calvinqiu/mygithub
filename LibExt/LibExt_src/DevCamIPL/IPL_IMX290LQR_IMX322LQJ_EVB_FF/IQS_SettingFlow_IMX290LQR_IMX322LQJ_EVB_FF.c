#ifndef _IQS_SETTINGFLOW_IMX290LQR_IMX322LQJ_EVB_FF_C
#define _IQS_SETTINGFLOW_IMX290LQR_IMX322LQJ_EVB_FF_C
#include "IPL_IMX290LQR_IMX322LQJ_EVB_FF_Int.h"

#if CAL_ECS
#include "IPL_Cal_DataPassing_IMX290LQR_EVB_FF_Int.h"
#endif
#include "NOTE.h"
...
static ER IFE_IQparam(UINT32 Id, IQS_FLOW_MSG MsgID, IFE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    switch((UINT32)MsgID)
    {
        ...
        case IQS_AWB_END:
            ...
            IQS_CGain[SenId_Value][1] = GGain*4095/(4095-IQS_OBSub_IFE_Buf[SenId_Value][0]);//lydia GGain;
            IQS_CGain[SenId_Value][2] = GGain*4095/(4095-IQS_OBSub_IFE_Buf[SenId_Value][0]);//lydia GGain;
    ...
    switch((UINT32)MsgID)
    {
        ...
        case IQCB_PREIME_D2D_IPE_SUB:
            ...
            if ( IPL_AlgGetUIInfo(Id, IPL_SEL_IPPEDGE) == SEL_IPPEDGE_OFF )
              ...
              if(1)
              {
                 UINT32 autoWDRLevel = 0;
		        autoWDRLevel = SEL_WDR_LV4;
                IPL_AlgSetUIInfo(SenId_Value, IPL_SEL_WDR_LEVEL, autoWDRLevel);
                ExtFunc_SetWDRLumaLevel(SenId_Value, IPL_AlgGetUIInfo(Id, IPL_SEL_WDR_LEVEL));
              }
            //////////////////
            // Dynamic Gamma & CC
            //////////////////
