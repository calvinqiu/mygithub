#ifndef _IQS_SETTINGFLOW_IMX123LQT_IPCAM_FF_C
#define _IQS_SETTINGFLOW_IMX123LQT_IPCAM_FF_C
#include "IPL_IMX123LQT_IPCAM_FF_Int.h"

#if CAL_ECS
#include "IPL_Cal_DataPassing_IMX123LQT_IPCAM_FF_Int.h"
#endif
#include "NOTE.h"
...
static ER SIEx_IQparam(UINT32 Id, IQS_FLOW_MSG MsgID, SIE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    if((IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_AUTO && AE_GetIRCutEnable(Id) == FALSE) || IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)
        IQS_OBSub_SIE_Buf[SenId_Value] = 160;
...
static ER IFE_IQparam(UINT32 Id, IQS_FLOW_MSG MsgID, IFE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    {
        if((IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_AUTO && AE_GetIRCutEnable(Id) == FALSE) || IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)
            IQS_OBSub_IFE_Buf[SenId_Value][0] = 160;
    }
        ...
        case IQCB_PREIME_D2D_IPE_SUB:
            ...
            if ( group->ipeUpdate & IPE_RGBGAMMA )
            {
                if(IPL_AlgGetUIInfo(Id, IPL_SEL_IPC_MODE)==SEL_IPC_MODE_OFF)
                {
                    ...
                    #define IQS_LV_DAY_END     50
                    #define IQS_LV_NIGHT_START 30
            ...
            //////////////////
            if((IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_AUTO && AE_GetIRCutEnable(Id) == FALSE) || IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)
                IQS_ColorCorrection[SenId_Value].CCcoef_TblAddr = (UINT32)IQS_CC_INIT[SenId_Value];
            else
