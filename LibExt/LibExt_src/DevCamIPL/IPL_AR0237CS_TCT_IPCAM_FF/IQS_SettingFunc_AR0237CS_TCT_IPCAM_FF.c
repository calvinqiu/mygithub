#ifndef _IQS_SETTINGFUNC_AR0237CS_TCT_IPCAM_FF_C
#define _IQS_SETTINGFUNC_AR0237CS_TCT_IPCAM_FF_C
...
#if 0
	// Adrian's update 20160818
    if(IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)
    {
        if (uiISO1 <= 100) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV0;//H264_3DNR_IPCAM_LV3;
        else if(uiISO1 <= 200) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV0;//H264_3DNR_IPCAM_LV3;
        else if(uiISO1 <= 600) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV0;//H264_3DNR_IPCAM_LV3;
        else if(uiISO1 <= 1200) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV0;//H264_3DNR_IPCAM_LV4;
        else if(uiISO1 <= 4000) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV1;
        else if(uiISO1 <= 6400) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV1;
        else if(uiISO1 <= 8000) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV1;
        else if(uiISO1 <= 18000) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV1;
        else if(uiISO1 <= 30000) ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV1;
        else ui3DNRLevel = H264_3DNR_IPCAM_IR_HighISO_LV1;
      }
    else
    {
        if (uiISO1 <= 100) ui3DNRLevel = H264_3DNR_IPCAM_LV3;
        else if(uiISO1 <= 200) ui3DNRLevel = H264_3DNR_IPCAM_LV3;
        else if(uiISO1 <= 600) ui3DNRLevel = H264_3DNR_IPCAM_LV3;
        else if(uiISO1 <= 2000) ui3DNRLevel = H264_3DNR_IPCAM_LV4;
        else if(uiISO1 <= 4000) ui3DNRLevel = H264_3DNR_IPCAM_LV5;
        else if(uiISO1 <= 6400) ui3DNRLevel = H264_3DNR_IPCAM_LV6;
        else if(uiISO1 <= 8000) ui3DNRLevel = H264_3DNR_IPCAM_LV7;
        else if(uiISO1 <= 18000) ui3DNRLevel = H264_3DNR_IPCAM_LV7;
        else if(uiISO1 <= 30000) ui3DNRLevel = H264_3DNR_IPCAM_LV8;
        else ui3DNRLevel = H264_3DNR_IPCAM_LV9;
    }
#else
...
if (IQS_IPCAM_ENABLE)
{
    ...
    ui3DNRLevel = T_CLAMP((INT32)((INT32)ui3DNRLevel+IQUC_CtrlItem[Id][IQUC_CtrlIndex[Id]].ADJ_ITEM[ADJ_TNR]), H264_UI_3DNR_OFF, H264_UI_3DNR_LV16);
#endif

    if(IQS_IPCAM_ENABLE)
        g3DNR[Id] = IQS_CLAMP(ui3DNRLevel, H264_UI_3DNR_OFF, H264_UI_3DNR_LV16);
    else
        g3DNR[Id] = IQS_CLAMP(ui3DNRLevel, H264_UI_3DNR_OFF, H264_UI_3DNR_LV16);
...
{
    if(id<3 && IQS_User_CC !=NULL)
    {
        UINT16 i;
        for (i=0; i<9; i++)
            IQS_UserDefine_CC[id][i] = IQS_User_CC[i];
    }else
        return 0;
...
UINT32 IQS_getUserDefineCC(UINT16 id,UINT16* IQS_User_CC)
{
    if(id<3 && IQS_User_CC !=NULL)
    {
        UINT16 i;
        for (i=0; i<9; i++)
             IQS_User_CC[i]= IQS_UserDefine_CC[id][i];
    }else
        return 0;
...
{
    if(IQS_User_gamma !=NULL)
    {
        UINT16 i;
        for (i=0; i<129; i++)
             IQS_User_gamma[i]= IQS_GammaUserDefine1Lut[i];
    }else
        return 0;
...
UINT32 IQS_setUserDefineGamma(UINT16* IQS_User_gamma)
{
    if(IQS_User_gamma !=NULL)
    {
        UINT16 i;
        for (i=0; i<129; i++)
             IQS_GammaUserDefine1Lut[i]= IQS_User_gamma[i];
    }else
        return 0;
...
static UINT32* IQS_selUserGamma(SEL_USER_GAMMA_MODE level)
{
    switch (level)
    {
        case SEL_USER_GAMMA_LINEAR:
            return IQS_UserGAMMALinear;
            break;

        case SEL_USER_GAMMA_CURVE_MODE_16:
            return IQS_Gamma16Lut;
            break;

        case SEL_USER_GAMMA_CURVE_MODE_18:
            return IQS_Gamma18Lut;
            break;

        case SEL_USER_GAMMA_CURVE_MODE_20:
            return IQS_Gamma20Lut;
            break;


        case SEL_USER_GAMMA_CURVE_MODE_22:
            return IQS_Gamma22Lut;
            break;

        case SEL_USER_GAMMA_CURVE_MODE_24:
            return IQS_Gamma24Lut;
            break;

        case SEL_USER_GAMMA_CURVE_MODE_USER:
            return IQS_GammaUserDefine1Lut;
            break;

        default:
            return IQS_GammaLut_Day[0];
            break;
...
UINT32 IQS_Antifog(void)
{
    UINT16 level=0;
    static UINT16 cnt = 0,flag = 0;
...
//    IPL_AlgSetUIInfo(0, IPL_SEL_ANTI_FOG,SEL_ANTI_FOG_AUTO);
//   IPL_AlgSetUIInfo(0, IPL_SEL_IPC_MODE, SEL_IPC_MODE_ON);
    if (IPL_AlgGetUIInfo(0, IPL_SEL_ANTI_FOG)==SEL_ANTI_FOG_AUTO) level =7;
       else level = IPL_AlgGetUIInfo(0, IPL_SEL_ANTI_FOG_LEVEL);
  //  DBG_ERR("Gin:%x,Gout:%x\r\n",IQS_selUserGamma(IPL_AlgGetUIInfo(0, IPL_SEL_USER_GAMMA_MODE)),IQS_UserGAMMA);
    IQS_ToneCurveAdj(IQS_UserGAMMALinear,IQS_antiFog_Y,AE_GetCurRawLum(0, 10),IQS_GetInvGammaValue(0,40<<2)>>2,IQS_GetInvGammaValue(0,208<<2)>>2,level);
   // DBG_WRN("cur_raw:%d Y:%d \r\n",AE_GetCurRawLum(0, 10),IQS_GetGammaValue(0,AE_GetCurRawLum(0, 12)));

   return flag;
}

UINT32* IQS_getUserGamma(void)
{

       return IQS_selUserGamma(IPL_AlgGetUIInfo(0, IPL_SEL_USER_GAMMA_MODE));
