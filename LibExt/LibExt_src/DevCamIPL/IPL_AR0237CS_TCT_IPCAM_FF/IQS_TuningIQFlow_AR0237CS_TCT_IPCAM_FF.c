#ifndef _IQS_TUNINGIQFLOW_AR0237CS_TCT_IPCAM_FF_C_
#define _IQS_TUNINGIQFLOW_AR0237CS_TCT_IPCAM_FF_C_
...
if (IQS_IPCAM_ENABLE)
{
    ...
    if( IQSExtSetIQInfo.LSC.isSet )
    {
        ...
        if(IQS_IPCAM_RatioTable[SenId_Value].LSC.isEnable == ENABLE)
        {
            ...
            for( i=0; i<17; i++ )
                IQS_Vig_Tab_Temp[SenId_Value][i] = (UINT32)IQS_IQ_Cal(IQS_IPCAM_RatioTable[SenId_Value].LSC.ratio, IQS_Vig_Set[SenId_Value].Tab[i], 0, 1023, IQS_CAL_MULTIPLY);
...
void IQS_CalValue(UINT32 SenId_Value, UINT32 IQIdxL, UINT32 IQIdxH)
{
    ...
    for ( FilterSize=0; FilterSize<17; FilterSize++ )
    {
        ...
        {
		;
        //    IQS_Vig_Tab_Temp[SenId_Value][FilterSize]  = IQS_Vig_Tab_Temp[SenId_Value][FilterSize];
        //    IQS_Vig_Tab_Temp[SenId_Value][FilterSize]  = IQS_Vig_Tab_Temp[SenId_Value][FilterSize];
        }
        else
        {
            IQS_Vig_Tab_Temp[SenId_Value][FilterSize]  = IQS_Vig_Set[SenId_Value].Tab[FilterSize];
            IQS_Vig_Tab_Temp[SenId_Value][FilterSize]  = IQS_Vig_Set[SenId_Value].Tab[FilterSize];
        }
    }
...
// Without ratio adjusting
    // IFE
    IQS_IntplTbl(IQS_Vig_Tab_Temp[SenId_Value], IQS_Vig_Set_Init[SenId_Value].Tab, 17, IQS_Vig_Tab_Buf[SenId_Value], IsoValue, 400, 1600);
