...
void ExtFunc_Init(UINT32 Id, ENG_SUBFUNC *Info, IPL_HAL_GROUP* group)
{
    ...
    else
    {
        ...
        if (IPL_CtrlGetInfor(Id, IPLCTRL_WDR_VER) == WDR_VERSION_2_5)
        {
            ...
            //#NT#2017/01/06#Jarkko Chang -begin
            //#NT# modify for wdr_defog
            if(IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_DEFOG)
            {
                ChgParams.uiChgSel = SR_WDR_CHG_LUMA_DARK_LEVEL | SR_WDR_CHG_LUMA_BRIGHT_LEVEL | SR_WDR_CHG_BLACK_LEVEL | SR_WDR_CHG_LOCAL_GAIN;
                ChgParams.uiBrightLumaLevel = 100;
                ChgParams.uiDarkLumaLevel = 100;
                ChgParams.uiBlackLevel = SR_WDR_BLACK_LEVEL08;
                ChgParams.puiLocalGain = &IQS_WDR_LocalGain[Id][0];
            }
            else if( (IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_ON)||(IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_AUTO) )
            {
                ChgParams.uiChgSel = SR_WDR_CHG_LUMA_DARK_LEVEL | SR_WDR_CHG_LUMA_BRIGHT_LEVEL | SR_WDR_CHG_LOCAL_GAIN|SR_WDR_CHG_LUMA_WEIGHT_MAP|SR_WDR_CHG_CONTRAST_OB_MAP;
                ChgParams.uiBrightLumaLevel = 105;
                ChgParams.uiDarkLumaLevel = 250;
                ChgParams.puiLocalGain = &IQS_WDR_LocalGain[Id][0];
            }
            //#NT#2017/01/06#Jarkko Chang -end
            IQS_setWDR(TRUE);
...
void ExtFunc_Process(UINT32 Id)
{
	...
	//#NT#2017/01/06#Jarkko Chang -begin
    //#NT# modify for wdr_defog
    else if ((IPL_CtrlGetInfor(Id, IPLCTRL_EXT_FUNC_EN) == ICF_FUNC_WDR) && (IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_ON || IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_DEFOG))
	//#NT#2017/01/06#Jarkko Chang -end
    {
        ...
        if (gWdrStep[Id] == 0)
        {
            ...
            wdrInfo.uiVdoFrameRate = ExtFunc_getVideoFps(Id, IPL_AlgGetUIInfo(Id, IPL_SEL_VIDEO_FPS));
            TrigInt[Id] = ExtFunc_getTrigInterval(Id, IPL_AlgGetUIInfo(Id, IPL_SEL_VIDEO_FPS));
...
void ExtFunc_SetWDRLumaLevel(UINT32 Id, UINT32 Level)
{
    ...
    if(IPL_CtrlGetInfor(Id, IPLCTRL_WDR_VER) == WDR_VERSION_2_5)
    {
        ...
        if(IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_DEFOG)
        {
            ChgParams.uiChgSel = SR_WDR_CHG_LUMA_DARK_LEVEL | SR_WDR_CHG_LUMA_BRIGHT_LEVEL | SR_WDR_CHG_BLACK_LEVEL;
            ChgParams.uiBrightLumaLevel = 100;
            ChgParams.uiDarkLumaLevel = 100;
            ChgParams.uiBlackLevel = SR_WDR_BLACK_LEVEL08;
            SceneRender_WDR_ChgParams(WDRMapId[Id], &ChgParams);
        ...
        else if( (IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_ON)||(IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_AUTO) )
        {
            ...
            ChgParams.uiChgSel = SR_WDR_CHG_LUMA_DARK_LEVEL|SR_WDR_CHG_LUMA_BRIGHT_LEVEL;
            ChgParams.uiBrightLumaLevel = 105;
            ChgParams.uiDarkLumaLevel = tmpLevel;
            SceneRender_WDR_ChgParams(WDRMapId[Id], &ChgParams);
        }
        //#NT#2017/01/06#Jarkko Chang -end
    }
