...
void IPL_CtrlPrvInitFlowD(UINT32 Id, IPLCTRL_INFO *CtrlInfo)
{
    ...
    if (CtrlInfo->ChgMode->FuncEn & IPL_FUNC_VR360) // Force to enable VR360 mode
    {
		...
		DBG_DUMP("VR360 Enabled: ori[0] %d ori[1] %d HeightAdj %d (offset %d %d)\r\n", CtrlInfo->ChgMode->VR360Info.ori[0],
		    CtrlInfo->ChgMode->VR360Info.ori[1],
		    CtrlInfo->ChgMode->VR360Info.ImgHeightAdj,
		    CtrlInfo->ChgMode->VR360Info.IME_P1_Start_Offset,
		    CtrlInfo->ChgMode->VR360Info.IME_P2_Start_Offset);
    }
    else
    {
