...
//#NT#2016/02/02#Jarkko Chang -end
    else
    {
        ...
        IPL_CtrlInitIpc_SIEIn(Id, &SenInfo, &IpcInfo->ICF_FC.BaseInfo.SIE_IN);
        if(Id == IPL_ID_1)
            IPL_CtrlInitIpc_SIESignal(Id, TRUE, TRUE, _RISING, _RISING, _RISING, &IpcInfo->ICF_FC.BaseInfo.SenSorIn);
        else if(Id == IPL_ID_2)
            IPL_CtrlInitIpc_SIESignal(Id, TRUE, TRUE, _RISING, _RISING, _FALLING, &IpcInfo->ICF_FC.BaseInfo.SenSorIn);
        ...
        if (IPL_CtrlGetInfor(Id, IPLCTRL_SIE_CH0_RAW_EN) == ENABLE)
        {
            ...
            else
            {
				...
				/*DBG_ERR("SIE_PPB0 %dx%d SIE_IN.Crop %dx%d\r\n"
					, IpcInfo->ICF_FC.BaseInfo.SIE_PPB0.Width, IpcInfo->ICF_FC.BaseInfo.SIE_PPB0.Height
					, IpcInfo->ICF_FC.BaseInfo.SIE_IN.Crop.SizeH, IpcInfo->ICF_FC.BaseInfo.SIE_IN.Crop.SizeV);*/
            }
        }
        else
        {
