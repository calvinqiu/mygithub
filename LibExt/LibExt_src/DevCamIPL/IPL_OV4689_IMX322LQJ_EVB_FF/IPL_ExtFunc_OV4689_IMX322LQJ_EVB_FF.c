...
void ExtFunc_Process(UINT32 Id)
{
    if (IPL_CtrlGetInfor(Id, IPLCTRL_EXT_FUNC_EN) == ICF_FUNC_SHDR)
    {
        ...
        AE_BOUNDARY LvBound;
        UINT32 AeLVTor = 15; // 1 LV
        SHDR_AE_LV_INFO AeLVInfo = {0};
...
//        AE_GetWin(Id, &AEWin[0]);
        ...
        HdrCalInfo.uiGlobalToneGain = AE_GetGTGain(Id); ///< long line * 100 / shot line
