#ifndef _IQS_SETTINGFLOW_IMX258_OV7251_EVB_FF_C
#define _IQS_SETTINGFLOW_IMX258_OV7251_EVB_FF_C
#include "IPL_IMX258_OV7251_EVB_FF_Int.h"

#if CAL_ECS
#include "IPL_Cal_DataPassing_IMX258_OV7251_EVB_FF_Int.h"
#endif
#include "NOTE.h"
...
UINT32 IQS_WDREdgeGammaLut_Buf[Total_SENIDNum][65];
...
UINT32 IQS_GetISORange(UINT32 Id, IQS_FLOW_MSG MsgID, UINT32 *IsoValue, UINT32 *IsoStart, UINT32 *IsoEnd, UINT32 *IQIdxL, UINT32 *IQIdxH)
{
    ...
    *IsoValue = ISO;
    ...
    if ( SenId_Value == SENSOR_ID_1 && CFMode == IPL_CF_2STRP && IPL_Mode == IPL_MODE_VIDEOREC)
    {
        if ( IQS_IsCapFlow(MsgID)==FALSE )  // movie mode
        {
...
static ER IPE_IQparam(UINT32 Id, IQS_FLOW_MSG MsgID, IPE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    if ( Id == 1 && (IPL_AlgGetUIInfo(Id, IPL_SEL_PRVSENMODE) == SENSOR_MODE_5 || IPL_AlgGetUIInfo(Id, IPL_SEL_PRVSENMODE) == SENSOR_MODE_6 || IPL_AlgGetUIInfo(Id, IPL_SEL_PRVSENMODE) == SENSOR_MODE_9 ))
    {
        if ( IPL_AlgGetUIInfo(Id, IPL_SEL_PRVSENMODE) != SENSOR_MODE_9 )
        {
            IQS_EdgeEnh_Buf[SenId_Value].EnhP = IQS_CLAMP(IQS_EdgeEnh_Buf[SenId_Value].EnhP, 0, 64);
            IQS_EdgeEnh_Buf[SenId_Value].EnhN = IQS_CLAMP(IQS_EdgeEnh_Buf[SenId_Value].EnhN, 0, 32);
        }
        IQS_CFAInter[SenId_Value].GC.En = FALSE;
        IQS_CFAInter[SenId_Value].RBC.En = FALSE;
    }
    else
    {
        IQS_CFAInter[SenId_Value].GC.En = TRUE;
        IQS_CFAInter[SenId_Value].RBC.En = TRUE;
    }
    ...
    switch((UINT32)MsgID)
    {
        ...
        case IQCB_PREIME_D2D_IPE_SUB:
            ...
            if (IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) != SEL_WDR_OFF)
                memcpy(IQS_EdgeGammaLut_Buf[SenId_Value], IQS_WDREdgeGammaLut_Buf[SenId_Value], sizeof(UINT32)*65);
            ...
            IQS_EdgeExt[SenId_Value].ToneMapLutTabAddr = (UINT32)IQS_EdgeGammaLut_Buf[SenId_Value];
