...
ER IPL_TrigFCB(IPC_CTRL_CMD *ProcCmd)
{
    ...
    IPC_CTRL_CMD RunCmd[IPL_ID_MAX_NUM] = {IPC_IGNORE, IPC_IGNORE, IPC_IGNORE, IPC_IGNORE};
    IPC_CTRL_CMD StopCmd[IPL_ID_MAX_NUM] = {IPC_IGNORE, IPC_IGNORE, IPC_IGNORE, IPC_IGNORE};
    ...
    for (Id = IPL_ID_1; Id< IPL_ID_MAX_NUM ; Id++)
    {
        if (ProcCmd[Id] == IPC_STOP_ALL)
            StopCmd[Id] = IPC_STOP_ALL;
        else
            RunCmd[Id] = ProcCmd[Id];
    }
    IPL_CTRLMode(RunCmd[IPL_ID_1],RunCmd[IPL_ID_2],RunCmd[IPL_ID_3],RunCmd[IPL_ID_4]);
    IPL_CTRLMode(StopCmd[IPL_ID_1],StopCmd[IPL_ID_2],StopCmd[IPL_ID_3],StopCmd[IPL_ID_4]);
