...
UINT32 IPL_SetSensorMode(IPL_PROC_ID Id, IPL_MODE Mode, BOOL SetFlg)
{
    ...
    DBG_WRN("always setting sensor mode 1 8 bits for FPGA\r\n");

    if (SetFlg == TRUE && Mode != IPL_MODE_CAP)
    {
        IPL_AlgSetUIInfo(Id, IPL_SEL_CAPSENMODE, SENSOR_MODE_1);
        IPL_AlgSetUIInfo(Id, IPL_SEL_PRVSENMODE, SENSOR_MODE_1);
        IPL_AlgSetUIInfo(Id, IPL_SEL_RAW_BITDEPTH_PRV, SEL_RAW_BITDEPTH_8);
    }
    return SenMode;
}
...
static ER IPL_Init(IPL_CHGMODE_DATA *ChgMode, IPL_CHGMODE_DATA *PrvMode)
{
    ...
    //get mode status
    rt = IPL_GetSensorStatus(IPL_UTI_CONV2_SEN_ID(ChgMode->ProcSenId), IPL_AlgGetUIInfo(ChgMode->ProcSenId, IPL_SEL_PRVSENMODE), &Info);
    if (rt != E_OK)
        return rt;
...
static ER IPL_Off2Cap(IPL_CHGMODE_DATA *ChgMode, IPL_CHGMODE_DATA *PrvMode)
{
    ...
    BitNum3 = AppInfo->IQSimPriInfo.RawInfo.BitNum;
    Width3 = IPL_UTI_SIESUB_ADJ_HSIZE(AppInfo->IQSimPriInfo.RawInfo.Size.w, IPL_CtrlGetInfor(Id, IPLCTRL_SIE_CH3_CAP_SUBRATIO), 4);
    Height3 = IPL_UTI_SIESUB_ADJ_VSIZE(AppInfo->IQSimPriInfo.RawInfo.Size.h, IPL_CtrlGetInfor(Id, IPLCTRL_SIE_CH3_CAP_SUBRATIO), 4);

    IPL_CtrlSetInfor(Id, IPLCTRL_SIE_CH3_RAW_BUFNUM, 1);
    IPL_CtrlSetInfor(Id, IPLCTRL_SIE_CH3_CAP_RAW_SIZE_H, Width3);
    IPL_CtrlSetInfor(Id, IPLCTRL_SIE_CH3_CAPBIT, BitNum3);
    IPL_CtrlSetInfor(Id, IPLCTRL_SIE_CH3_CAP_RAW_SIZE_HLOS, IPL_UTI_RAWH_CONV2_LOFS(Width3, BitNum3));
    IPL_CtrlSetInfor(Id, IPLCTRL_SIE_CH3_CAP_RAW_SIZE_V, Height3);


    IPL_CtrlSetInfor(Id, IPLCTRL_CAPFLOW, IPLCTRL_CAP_FLOW_SIM);

    UINT32 *ZoomTab, ZoomTabMax;
    ZoomTab = SenMode2Tbl(Id, IPL_AlgGetUIInfo(Id, IPL_SEL_CAPSENMODE), &ZoomTabMax);
    ZoomTab[0] = ZoomTab[2] = ZoomTab[4] = ZoomTab[6] = Width0;
    ZoomTab[1] = ZoomTab[3] = ZoomTab[5] = ZoomTab[7] = Height0;
...
static IPL_CMD_CHGMODE_FP ChgModeAct[IPL_MODE_NUM][IPL_MODE_NUM] =
{
    ...
    {NULL,              IPL_Init,           IPL_Off2Stream,     IPL_Off2Stream,     IPL_Off2Stream,     IPL_Off2Cap},       /*IPL_MODE_INIT*/
