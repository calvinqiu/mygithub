...
void* IPL_AdaptSettingCBFlowC(UINT32 Id, IPL_RAW_IMG_INFO IplInInfo, IPL_YCC_CH_INFO *ChInfo)
{
    ChInfo->Width  = IPL_UTI_IPESUB_ADJ_HSIZE(IplInInfo.Width, IQS_GetChromaAdaptRatio(Id), 4);
    if (ChInfo->Width < 80)
    {
        ChInfo->Width = 80;
    }

    ChInfo->Height = IPL_UTI_IPESUB_ADJ_VSIZE(IplInInfo.Height, IQS_GetChromaAdaptRatio(Id), 4);
    if (ChInfo->Height < 60)
    {
        ChInfo->Height = 60;
    }
...
void IPL_CtrlPrvInitFlowC(UINT32 Id, IPLCTRL_INFO *CtrlInfo)
{
    ...
    if (CtrlInfo->Info->Mode->ModeType == SENSOR_MODE_CCIR || CtrlInfo->Info->Mode->ModeType == SENSOR_MODE_CCIR_INTERLACE)
    {
        ...
        if (CtrlInfo->Info->Mode->ModeType == SENSOR_MODE_CCIR_INTERLACE)
        {
            ...
            IPL_CtrlSetInfor(Id, IPLCTRL_CFMODEEXT, IPL_CFEXT_DEINTERLACE);   //special case for CCIR_INTERLACE
    ...
    else
    {
        ...
        else
        {
            ...
            {
                Width  = IPL_UTI_IPESUB_ADJ_HSIZE(IPL_CtrlGetInfor(Id, IPLCTRL_IPL_IN_SIZE_H), IQS_GetChromaAdaptRatio(Id), 4);
                Height = IPL_UTI_IPESUB_ADJ_VSIZE(IPL_CtrlGetInfor(Id, IPLCTRL_IPL_IN_SIZE_V), IQS_GetChromaAdaptRatio(Id), 4);
