...
SIE_EDGETHRE Eth_Thres = {8, 32,128};
...
void IPL_CtrlInitIpc_DispPxPPB(UINT32 Id, UINT32 PathEnIdx)
{
    ...
    for (i = 0; i < Buf_num; i ++)
    {
        ...
        if ((IPL_CtrlGetInfor(Id, IPLCTRL_CFMODEEXT) == IPL_CFEXT_DEINTERLACE) || (PathEnIdx == IPLCTRL_SIE_CH1_Y_EN))
        {
...
void IPL_CtrlInitCapIpcInfo(IPL_GET_CAP_RAW_DATA *CapInfo, IPC_SIE_Info *Info, IPLCTRL_CAP_EXT_INFO *ExtInfo)
{
    ...
    {
        IPL_HAL_GROUP group = {0};
        ENG_SUBFUNC IQSInfo = {0};
        IQSInfo.pSie_sub = Info->psie->pSubFunc;
        IQCB_SettingFlowFunc(CapInfo->Id, IQCB_PREIME_D2D_SIE_PRI, &IQSInfo, &group);
    ...
    else
    {
        ...
        Info->sieUpdate |= (SIE_CH0Out_SRC|SIE_CH3Out_SRC|SIE_SUBFEN);
    }
    ...
    if (CapInfo->OutCh3En == ENABLE)
    {
        ...
        Info->sieUpdate |= (SIE_OUTADDR_CH3|SIE_IOSIZE_CH3);
    }
}
...
//#NT#2016/03/30#Harry Tsai -end
    ...
    Width = IPL_UTI_SENCROPRATIO_ADJ_HSIZE(IPL_CtrlGetInfor(Id, IPLCTRL_SEN_CAPCROPRATIO), IPL_UTI_CONV_SEN_RATIO(CtrlInfo->Info->Mode->Ratio.RatioHV), Width, 4);
