...
void IPL_GetCapRawFCB(IPL_GET_CAP_RAW_DATA *Info)
{
    ...
    {
        Info->res[0] = 0;//output to raw buffer 0
        ExtInfo.Param[0] = Info->res[0];
        IPL_CtrlInitCapIpcInfo(Info, &SIEInfo, &ExtInfo);
        IPL_FCB_GetCapRaw(Info->Id, &SIEInfo, &ExtInfo);

        Info->res[0] = 1;//output to raw buffer 1
        ExtInfo.Param[0] = Info->res[0];
        IPL_CtrlInitCapIpcInfo(Info, &SIEInfo, &ExtInfo);
        IPL_FCB_GetCapRaw(Info->Id, &SIEInfo, &ExtInfo);
    }
...
void IPL_SetIPLStopFCB(IPL_CHGMODE_DATA **IPLModeInfo, IPL_CHGMODE_DATA *ChgMode, UINT32 *TrigFlg)
{
    ...
    for (i = 0 ; i < IPL_ID_MAX_NUM ; i++)
    {
        if ((IPLModeInfo[i]->Mode == IPL_MODE_PREVIEW) || (IPLModeInfo[i]->Mode == IPL_MODE_VIDEO) || (IPLModeInfo[i]->Mode == IPL_MODE_VIDEOREC))
        {
            ...
            {
                //get mode status
                rt = IPL_GetSensorStatus(IPL_UTI_CONV2_SEN_ID(i), IPL_AlgGetUIInfo(i, IPL_SEL_PRVSENMODE), &Info);
                if (rt != E_OK)
                    return;
                ...
                else
                {
                    if (ChgMode[i].EventId == IPL_FLOW_VID_GETRAW)
                    {
                        ...
                        rt = IPL_GetSensorStatus(IPL_UTI_CONV2_SEN_ID(ChgMode[i].ProcSenId), IPL_AlgGetUIInfo(ChgMode[i].ProcSenId, IPL_SEL_PRVSENMODE), &Info);
                        if (rt != E_OK)
                            return;
...
void IPL_SetTrigInfoFCB(IPL_CHGMODE_DATA *ChgMode, UINT32 *TrigFlg)
{
    ...
    static IPL_TRIG_TYPE TrigType =  IPL_TRIG_VD;
    static UINT32 TrigInterval[2] = {0};
    static IPL_MODE PRV_MODE[IPL_ID_MAX_NUM] = {IPL_MODE_NONE,IPL_MODE_NONE,IPL_MODE_NONE,IPL_MODE_NONE};   //Previous IPL Mode
    BOOL CapFlg = FALSE;
    UINT32 i;
    ER rt = E_OK;
        ...
        if (ChgMode[i].ActFlg == TRUE)
        {
            switch (ChgMode[i].Mode)
            {
                case IPL_MODE_INIT:
                    ...
                    if (IPL_CtrlGetInfor(IPL_ID_1, IPLCTRL_CFMODEEXT) == IPL_CFEXT_SYNCTRIG)
                    {
                    ...
                    else
                    {
                        if (PRV_MODE[IPL_ID_1] == IPL_MODE_INIT && PRV_MODE[IPL_ID_2] == IPL_MODE_INIT)
                        {
                            ...
                            {
                                TrigType = IPL_TRIG_VD;     //HDR Sensor
                            }
                            else
                            {
                                TrigType = IPL_TRIG_TIMER;  //Dual Sensor
                            }
                        }
                        else
                        {
                            TrigType = IPL_TRIG_VD;     //Single Sensor
                        }
                    }
                ...
                case IPL_MODE_VIDEOREC:
                    ...
                    if (ChgMode[i].ProcSenId == IPL_ID_1 || ChgMode[i].ProcSenId == IPL_ID_2)
                    {
                        ...
                        //Set Trigger Type
                        if ((ChgMode[i].ImePath1.ImgEn == ENABLE) && (ChgMode[i].ImePath1.Direct2H264 == TRUE))
                        {
                            //Set to H264 Trigger when D2H264 Enable
                            NewTrigInfo.type = IPL_TRIG_H264;
