#ifndef _IQS_SETTINGFLOW_AR0330P_IPCAM_FF_C
#define _IQS_SETTINGFLOW_AR0330P_IPCAM_FF_C
#include "IPL_AR0330P_IPCAM_FF_Int.h"
...
UINT32 IQS_BuildInHdrMode = FALSE;
...
UINT32 IQS_GetISORange(UINT32 Id, IQS_FLOW_MSG MsgID, UINT32 *IsoValue, UINT32 *IsoStart, UINT32 *IsoEnd, UINT32 *IQIdxL, UINT32 *IQIdxH)
{
    ...
    if (SenInfo.Mode->ModeType == SENSOR_MODE_BUILTIN_HDR)
    {
        ...
        ISO = ISO * (RGain + BGain) / 512;
        IQS_BuildInHdrMode = TRUE;
    }
#endif

    *IsoValue = ISO;

    if ( SenId_Value == SENSOR_ID_1)
    {
        if ( IQS_IsCapFlow(MsgID)==FALSE )  // movie mode
        {
            if(IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)   // IRcut off
            {
...
//#NT#2016/06/15#Ted -end
            ...
            // SIE-OB Substration
            ////////////////////
            if (IQS_BuildInHdrMode == TRUE)
                IQS_OBSub_SIE_Buf[SenId_Value] = IQS_OBSub_BHDR[SenId_Value];
            ...
            {
                //if ( IQS_BuildInHdrMode == TRUE )
                //    subf->La.GmaTbl = SIE_xfer2CAGamma(Id, (UINT32 *)IQS_GammaLut_Linear);
                //else
            ...
            if ( group->sieUpdate & SIE_ECS_ )
            {
                ...
                #else
                {
                    ...
                    if(IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)
                        subf->Ecs.Inaddr = (UINT32)IQS_GetECStab(Id, IQS_IsCapFlow(MsgID), (UINT32*)IQS_ECSTab_Init[SenId_Value], 65,  &Info);
                    else
                        subf->Ecs.Inaddr = (UINT32)IQS_GetECStab(Id, IQS_IsCapFlow(MsgID), (UINT32*)IQS_ECSTab[SenId_Value], 65,  &Info);
...
//OB CAL
    ...
    group->sieUpdate &= ~SIE_ECS_;
        ...
        case IQS_AWB_END:
            ...
            if ( IQS_BuildInHdrMode == TRUE )
            {
                subf->Ca.CA_TH_INFO.Rth.Upper = 4095;
                subf->Ca.CA_TH_INFO.Rth.Lower = 0;
                subf->Ca.CA_TH_INFO.Gth.Upper = (240<<8);
                subf->Ca.CA_TH_INFO.Gth.Lower = (2<<8);
                subf->Ca.CA_TH_INFO.Bth.Upper = 4095;
                subf->Ca.CA_TH_INFO.Bth.Lower = 0;
                subf->Ca.CA_TH_INFO.Pth.Upper = 4095;
                subf->Ca.CA_TH_INFO.Pth.Lower = 0;
                subf->La.LA_CG_INFO.RGain = 128;
                subf->La.LA_CG_INFO.GGain = 128;
                subf->La.LA_CG_INFO.BGain = 128;
            }
            else
            {
...
static ER IFE_IQparam(UINT32 Id, IQS_FLOW_MSG MsgID, IFE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    switch((UINT32)MsgID)
    {
        ...
        case IQS_AWB_END:
            ...
            if (IQS_BuildInHdrMode == TRUE)
                IQS_OBSub_IFE_Buf[SenId_Value][0] = IQS_OBSub_BHDR[SenId_Value];
...
//                group->ipeUpdate &= ~IPE_RGBGAMMA;
            ...
            {
                if(1)//if(IPL_AlgGetUIInfo(Id, IPL_SEL_IPC_MODE)==SEL_IPC_MODE_OFF)
                {
                        AWBStatus AWBStatus;
                    ...
                    if(1)
                    {
                        ...
                        {
                            if ((AWBStatus.ColorTemperature<4700)&&(AWBStatus.ColorTemperature>0))
                                IQS_CC_Intpl(IQS_UserDefine_CC[0], IQS_UserDefine_CC[2], 3300, 4700, AWBStatus.ColorTemperature, Normal_CC[SenId_Value]);
                            //else if(AWBStatus.ColorTemperature<5000)
                            //    IQS_CC_Intpl(IQS_UserDefine_CC[1], IQS_UserDefine_CC[2], 3800, 5000, AWBStatus.ColorTemperature, Normal_CC[SenId_Value]);
                            else
                                memcpy(Normal_CC[SenId_Value], IQS_CC_Day[SenId_Value], sizeof(UINT16)*9);

                            //DBG_DUMP("ColorTemperature=%d\r\n",AWBStatus.ColorTemperature);
                            //DBG_DUMP("IQS_CC=%d\r\n",Normal_CC[0][0]);
                        }
                    }
                }else
                {
                    static UINT32* oldgamma = NULL;
                    if(oldgamma == NULL)
                    {
                        oldgamma = IQS_getUserGamma();
                        Gamma_update[SenId_Value] = TRUE;
                    }else if(oldgamma!= IQS_getUserGamma())
                    {
                       oldgamma = IQS_getUserGamma();
                       Gamma_update[SenId_Value] = TRUE;
                    }
                    if( Gamma_update[SenId_Value] == TRUE )
                    {
                        memcpy(IQS_GammaLut[SenId_Value], oldgamma,  sizeof(UINT32)*130);
                        IQS_GammaAddr_Buf[SenId_Value] = (UINT32)IPE_GammaReorgon(SenId_Value, (UINT32 *)IQS_GammaLut[SenId_Value], (UINT32 *)IQS_GammaLut[SenId_Value], (UINT32 *)IQS_GammaLut[SenId_Value]);
                    }

                    if ( IPL_AlgGetUIInfo(Id, IPL_SEL_USER_CC) == SEL_USER_CC_AUTO)
                    {

                     memcpy(Normal_CC[SenId_Value], IQS_CC_Day[SenId_Value], sizeof(UINT16)*9);
                    }
                    else if ( IPL_AlgGetUIInfo(Id, IPL_SEL_USER_CC) == SEL_USER_CC_MANUAL)
                    {
                        //get color tempture
                        AWBStatus AWBStatus;
                        AWB_GetStatus(Id, &AWBStatus);

                        if (AWBStatus.ColorTemperature<5500)
                            IQS_CC_Intpl(IQS_UserDefine_CC[0], IQS_UserDefine_CC[1], 2700, 5500, AWBStatus.ColorTemperature, Normal_CC[SenId_Value]);
                        else
                            IQS_CC_Intpl(IQS_UserDefine_CC[1], IQS_UserDefine_CC[2], 5500, 8000, AWBStatus.ColorTemperature, Normal_CC[SenId_Value]);
                    ...
                    if (IPL_AlgGetUIInfo(Id, IPL_SEL_ANTI_FOG)!=SEL_ANTI_FOG_OFF &&Ycurve_update[SenId_Value]==TRUE)
                    {
                        group->ipeUpdate &= IPE_YCURVE;
            ...
            if ( group->ipeUpdate & IPE_YCURVE )
            {
                if(Ycurve_update[SenId_Value] == TRUE)
                    ...
                    {
                        if (IPL_AlgGetUIInfo(Id, IPL_SEL_ANTI_FOG)!=SEL_ANTI_FOG_OFF &&
                            IPL_AlgGetUIInfo(Id, IPL_SEL_IPC_MODE)==SEL_IPC_MODE_ON &&
                            IPL_AlgGetUIInfo(Id, IPL_SEL_WDR)==SEL_WDR_OFF)
                        {
                                IQS_YCurveAddr_Buf[SenId_Value] = (UINT32)IPE_YCurveReorgon(SenId_Value, (UINT32 *)IQS_antiFog_Y);
                        }
                        else
                        {
            ...
            IQS_EdgeExt[SenId_Value].PowerSav = FALSE; // TRUE: DIRCONEN = DIRCON2EN = 0; FALSE: DIRCONEN = DIRCON2EN = 1
            ...
            if ((AWBStatus.ColorTemperature<4700)&&(AWBStatus.ColorTemperature>0))
                IQS_IntplTblUint8(IQS_hueTab_L[SenId_Value], IQS_hueTab_M[SenId_Value], 24, IQS_hueTab[SenId_Value], AWBStatus.ColorTemperature, 3300, 4700);
            IQS_Cctrl[SenId_Value].HueTab_TblAddr = (UINT32)IQS_hueTab_Buf[SenId_Value];

            /*UINT32 i;
            for(i =0; i<24; i++)
            {
                IQS_satTab_3000K[SenId_Value][i]=IQS_satTab_3000K[SenId_Value][i]+IQS_satTab_off[SenId_Value][i];
                IQS_satTab[SenId_Value][i]=IQS_satTab[SenId_Value][i]+IQS_satTab_off[SenId_Value][i];
                IQS_intTab[SenId_Value][i]=IQS_satTab[SenId_Value][i]+IQS_intTab_off[SenId_Value][i];
                IQS_hueTab[SenId_Value][i]=IQS_satTab[SenId_Value][i]+IQS_hueTab_off[SenId_Value][i];
            }*/

        //    if ((AWBStatus.ColorTemperature<4700)&&(AWBStatus.ColorTemperature>0))
        //        IQS_IntplTbl(IQS_satTab_3000K[SenId_Value], IQS_satTab_5000K[SenId_Value], 24, IQS_satTab[SenId_Value], AWBStatus.ColorTemperature, 4500, 4700);

            if(IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)
            {
                            memcpy(IQS_satTab[SenId_Value], IQS_satTab_M[SenId_Value], sizeof(UINT32)*24);
            }else
            {
                if ((AWBStatus.ColorTemperature<4700)&&(AWBStatus.ColorTemperature>0))
                {
                   int i;
                   for ( i=0; i<24; i++ )
                        IQS_satTab[SenId_Value][i] = IQS_Intpl(AWBStatus.ColorTemperature, IQS_satTab_L[SenId_Value][i], IQS_satTab_M[SenId_Value][i], 4500, 4700);
                //IQS_IntplTbl(IQS_satTab_3000K[SenId_Value], IQS_satTab_5000K[SenId_Value], 24, IQS_satTab[SenId_Value], AWBStatus.ColorTemperature, 4500, 4700);
                }
            }
            ...
            // David said: patch it.
            //(IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_AUTO && AE_GetIRCutEnable(Id) == FALSE) || IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)
            if(IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)
            {
                Saturation_Buf[SenId_Value] = 0;
...
static ER IQS_SetWDRParam(UINT32 Id,IPL_HAL_PARASET  HalId)
{
    ...
    {
            if ((IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_ON)||( IPL_AlgGetUIInfo(Id, IPL_SEL_WDR) == SEL_WDR_AUTO ))
            {
...
ER IQS_SettingFlowFunc(UINT32 Id, IQS_FLOW_MSG MsgID)
{
    ...
    switch((UINT32)MsgID)
    {
        ...
        case IQS_AWB_END:
            ...
            {
                static SEL_ANTI_FOG o_selAntiFog = SEL_ANTI_FOG_OFF;
                SEL_ANTI_FOG selAntiFog = IPL_AlgGetUIInfo(0, IPL_SEL_ANTI_FOG);
                if(selAntiFog != SEL_ANTI_FOG_OFF)
                {
                    flag_ycurve[SenId_Value] = TRUE;
                    Ycurve_update[Id] = IQS_Antifog();
                    flag_ycurve[SenId_Value] = FALSE;
                }else if((o_selAntiFog != SEL_ANTI_FOG_OFF)&&
                          (selAntiFog == SEL_ANTI_FOG_OFF))
                {
                    Ycurve_update[Id] = TRUE;
                }
                o_selAntiFog = selAntiFog;
            }
            IQS_SetUpdateBit(Id, (IQS_ENG_IFE | IQS_ENG_IPE | IQS_TBL_GAMMA | IQS_TBL_YCURVE | IQS_TBL_VIG));
...
void IQS_Init(UINT32 Id)
{
    ...
    //memcpy(IQS_DCE_GLUT_Buf[SenId_Value],  IQS_DCE_GLUT_Init,     sizeof(UINT32)*65);
    memcpy(IQS_hueTab[SenId_Value],    IQS_hueTab_M[SenId_Value],       sizeof(UINT8)*24);
    memcpy(IQS_hueTab_Buf[SenId_Value],    IQS_hueTab[SenId_Value],       sizeof(UINT8)*24);
    memcpy(IQS_satTab[SenId_Value],    IQS_satTab_M[SenId_Value],       sizeof(UINT8)*24);
    memcpy(IQS_YCurveLut_Buf[SenId_Value], IQS_YCurveLut[SenId_Value], sizeof(UINT32)*130);
...
void iqs_param_init(UINT32 id)
{
    ...
    static const UINT32 isoidx_offset[Total_SENIDNum] = {0, PRV2ISO0100};
    const UINT32 idxRange[Total_SENIDNum] = {PRV2ISO0100, Total_ISONum-PRV2ISO0100};
