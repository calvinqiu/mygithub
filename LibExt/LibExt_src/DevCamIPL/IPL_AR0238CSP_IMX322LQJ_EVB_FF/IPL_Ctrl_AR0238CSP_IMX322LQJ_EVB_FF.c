...
void IPL_SIESetLA_FCB(UINT32 Id, SieFuncEn *FuncEn, SIE_INTE_EN *InteEn, SIE_UPDATE *Update, SIE_LACC *lacc, SIE_STATS_PATH_INFO *pPath)
{
    ...
    if (IPL_CtrlGetInfor(Id, IPLCTRL_SIE_LA_EN) == ENABLE)
    {
        ...
        if ((SenInfo.Mode->ModeType == SENSOR_MODE_STAGGER_HDR) || (SenInfo.Mode->ModeType == SENSOR_MODE_BUILTIN_HDR))
        {
...
void IPL_CtrlInitIpc_SIEIn(UINT32 Id, SENSOR_INFO *SenInfo, SIE_WINDOW *Win)
{
   ...
   else if(Id == IPL_ID_1)//20170828 fying
    {
        Win->Act.StartPix.x = SenInfo->Mode->TransHD[0].DataStart+1;
...
void IPL_CtrlInitCapIpcInfo(IPL_GET_CAP_RAW_DATA *CapInfo, IPC_SIE_Info *Info, IPLCTRL_CAP_EXT_INFO *ExtInfo)
{
   ...
   //         IPL_CtrlInitIpc_SIESignal_Cap(CapInfo->Id, &Info->sieUpdate, FALSE, FALSE, _RISING, _RISING, _RISING, &Info->psie->Sie_Signal);//modify by qiuhan on 20170816 from NTK method
	     IPL_CtrlInitIpc_SIESignal_Cap(CapInfo->Id, &Info->sieUpdate, TRUE, FALSE, _RISING, _RISING, _RISING, &Info->psie->Sie_Signal);
...
void IPL_CtrlSIExSetDGain(UINT32 Id, UINT32 Gain, UINT32 EnFlag);
...
void IPL_CtrlSIExSetDGain(UINT32 Id, UINT32 Gain, UINT32 EnFlag)
{
    ...
    if (EnFlag == ENABLE)
    {
        sieSubf.sieFuncEn |= DGainEn;
        group.sieUpdate |= SIE_SUBFEN;

        sieSubf.DGain.DigitalGain = Gain;
        group.sieUpdate |= SIE_DGAIN_;
    }
    else
    {
        sieSubf.sieFuncEn &= ~DGainEn;
