#ifndef _IQS_SETTINGFUNC_AR0238CSP_IMX322LQJ_EVB_FF_C
#define _IQS_SETTINGFUNC_AR0238CSP_IMX322LQJ_EVB_FF_C
...
void IQS_DCE_ParamSetting(UINT32 Id, UINT32 MsgID, UINT32 ZoomIdx, DCE_Distortion* pDistortion, DIST* pDce_Dist, Fact_Norm* pRadious)
{
    ...
    if ( IQS_IsCapFlow(MsgID) ) //capture
        pDistortion->Fov.Gain = 1024;//DCE_GetFovGain(Id, TRUE, pDistortion->GeoLUT.pGGeoLut);//lydia for AR0238
    else                        //preview
        pDistortion->Fov.Gain = 1024;//DCE_GetFovGain(Id, FALSE, pDistortion->GeoLUT.pGGeoLut);//lydia for AR0238
