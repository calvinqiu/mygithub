...
static IPH_CLK _SIEClkCB(UINT32 Id)
{
    ...
    switch (SenInfo.DataType)
    {
        ...
        case SENSOR_DATA_PARALLEL:
            clk.SIEclk = 240000000;//SenInfo.Mode->Pclk * 2;
        break;
...
_INLINE UINT32 _SieInitCB(UINT32 Id,ENG_SUBFUNC *Info,IPL_HAL_GROUP* group)
{
    ...
    *SIE_Update |= SIE_SUBFEN;
    ...
    Info->pSie_sub->OBSub.Offset = 0x96;
...
static void Prv_CtrlInit(IPL_PROC_ID Id,pIPLC_INFO pIplObj,pCTRL_PTR pCtrlptr)
{
    ...
    if (CAL_TYPE_USB == CalApi()->fpTypeEvent()->fpGetType())
    {
        USIZE usb_disp_size = {0};
        CalUSB_GetDispSize(&usb_disp_size.w, &usb_disp_size.h);
        disp_size.w = (INT32)usb_disp_size.w;
        disp_size.h = (INT32)usb_disp_size.h;
    }
    else
    {
