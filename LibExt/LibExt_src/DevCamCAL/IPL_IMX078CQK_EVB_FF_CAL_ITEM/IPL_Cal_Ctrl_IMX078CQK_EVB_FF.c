#include "Sensor.h"
#include "IPL_Cal_Ctrl_IMX078CQK_EVB_FF_Int.h"
...
_INLINE void _SIEIn(IPL_PROC_ID Id, SENSOR_INFO *pSenInfo, Cal_SENSOR_INFO *pCalSenInfo, SIE_WINDOW *Win)
{
    ...
    {
        #if 1 // for sony sensor
        Win->Crop.StartPix.x = (Win->Act.SizeH - Win->Crop.SizeH);
        Win->Crop.StartPix.y = (Win->Act.SizeV - Win->Crop.SizeV);
        #else // for other sensor (crop center same as active center)
...
_INLINE void _SIEAct(IPL_PROC_ID Id, SENSOR_INFO *pSenInfo, SieACTMode *ActMode)
{
    ...
    {
        if (pSenInfo->Mode->ModeType == SENSOR_MODE_LINEAR)
        {
        ...
        else if (pSenInfo->Mode->ModeType == SENSOR_MODE_CCIR)
        {
            *ActMode = SIE_IN_PARA_MSTR_SNR;
        }
        else
...
{
    sie1_chgBurstLengthByMode(BURST_MODE_BURST_CAP);
}
...
_INLINE UINT32 _SieInitCB(UINT32 Id,ENG_SUBFUNC *Info,IPL_HAL_GROUP* group)
{
    ...
    *SIE_Update |= SIE_SUBFEN;
    ...
    Info->pSie_sub->OBSub.Offset = 0x21;
