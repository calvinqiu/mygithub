...
#include "awb_api.h"
#include "awb_task.h"
#include "awb_OV4689_TWIN360_FF_int.h"
#include "ae_api.h"
#include "NOTE.h"
...
void AWB_GetStatus(UINT32 Id, AWBStatus *pAWBStatus)
{
	...
	//#NT#2016/11/30#Silvia Wu -begin
    //#NT# update awb stitch flow
	if ( AwbAppMode == AWB_APPMODE_STITCH)
    {
        Id = AWB_ID_1;
    }
	//#NT#2016/11/30#Silvia Wu -end
...
void AWB_Process(UINT32 id, UINT32 CurrentStatus)
{
    ...
    else if(Mode == AWB_MODE_AUTO)
    {
        ...
        //#NT#2017/01/05#Silvia Wu -begin
        //#NT# update awb stitch flow
        if(AWB_APPMODE_STITCH == AwbAppMode)
        {
            AwbInfo[id].Id = IPL_ID_2;
            AWB_setGain(&AwbInfo[IPL_ID_2], AwbInfo[id].AWBGain.RGain, AwbInfo[id].AWBGain.GGain, AwbInfo[id].AWBGain.BGain);
        }
        //#NT#2017/01/05#Silvia Wu -end
    }
    else
...
void AWB_GetColorGain(UINT32 id, UINT32 *Rg,UINT32 *Gg,UINT32 *Bg)
{
	...
	//#NT#2016/11/30#Silvia Wu -begin
    //#NT# update awb stitch flow
    if (( AwbAppMode == AWB_APPMODE_SHDR )||( AwbAppMode == AWB_APPMODE_STITCH))
    {
        AwbId = AWB_ID_1;
    }
	//#NT#2016/11/30#Silvia Wu -end
...
static void AWB_getCA(UINT32 id, UINT16 *R, UINT16 *G, UINT16 *B, UINT16 *Cnt, UINT32 Size)
{
	...
	//#NT#2016/11/30#Silvia Wu -begin
    //#NT# update awb stitch flow
    else if (AwbAppMode == AWB_APPMODE_STITCH)
	//#NT#2016/11/30#Silvia Wu -end
    {
