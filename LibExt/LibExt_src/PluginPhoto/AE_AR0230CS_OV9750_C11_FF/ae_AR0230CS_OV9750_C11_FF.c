...
#include "ae_alg.h"
#include "ae_AR0230CS_OV9750_C11_FF_int.h"
#include "ae_task.h"
#include "IPL_Cmd.h"
#include "iqs_api.h"
#include "aeafd_alg.h"
...
void AE_SetPLINEInfo(UINT32 Id, AE_EXT_SET_INFO SetInfo)
{
    if (SetInfo.PLineMode == AE_PLINE_AUTOGEN)
    {
        ...
        {
            //#NT#2016/05/24#Jarkko Chang#[0060622] -begin
            //#NT# remove unused code
            //SetInfo.IrisSetting.IrisMode = AE_IRIS_MODE_OPEN;
            //#NT#2016/05/24#Jarkko Chang#[0060622] -end
...
AE_RESET AE_getPrvTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_INFO *AeInfo)
{
    ...
    if (AeExtSetDefaultParamInfo[AeInfo->Id].PLineMode == AE_PLINE_AUTOGEN)
    {
        if (nFlicker == AE_FLICKER_50HZ)
        {
            AeCurveGenInfo[AeInfo->Id].ExpT[AE_GEN_IDX_MIN] = AE_MIN_EXPT_30;
            if (CurFps == SEL_VIDEOFPS_60)
            {
                AeCurveGenInfo[AeInfo->Id].ExpT[AE_GEN_IDX_MAX] = AE_MAX_EXPT_30000;
            }
            ...
            AeCurveGenInfo[AeInfo->Id].Iso[AE_GEN_IDX_MIN] = AeParamInfo[AeInfo->Id].PrvISO.l;
            AeCurveGenInfo[AeInfo->Id].Iso[AE_GEN_IDX_MAX] = AeParamInfo[AeInfo->Id].PrvISO.h;

            AeCurveGenInfo[AeInfo->Id].FnoRat = AE_FNO_RATIO;
            AeCurveGenInfo[AeInfo->Id].FnoIdx = 0;
            AeCurveGenInfo[AeInfo->Id].Freq = AE_GEN_FREQ_50;

            if (CurFps == SEL_VIDEOFPS_60)
            {
                AeCurveGenInfo[AeInfo->Id].FreqNode = AE_50FREQ_NODE;
                AeCurveGenInfo[AeInfo->Id].Node1stExpT = AE_MAX_EXPT_10000;
                AeCurveGenInfo[AeInfo->Id].Node2ndISO = AeParamInfo[AeInfo->Id].PrvISO.h/2;

            }
            else if (CurFps == SEL_VIDEOFPS_30)
            {
                AeCurveGenInfo[AeInfo->Id].FreqNode = AE_50FREQ_NODE;
                AeCurveGenInfo[AeInfo->Id].Node1stExpT = AE_MAX_EXPT_30000;
                AeCurveGenInfo[AeInfo->Id].Node2ndISO = (AeParamInfo[AeInfo->Id].PrvISO.l * 2);
            }
            else
            {
            ...
            //#NT# fixed warning
            rt = AEAlg_GenCurve(&AeCurveGenInfo[AeInfo->Id], &Curve, &AeInfo->EV.h, &AeInfo->EV.l);
            if (rt != E_OK)
        ...
        else if (nFlicker == AE_FLICKER_60HZ)
        {
            ...
            if (CurFps == SEL_VIDEOFPS_60)
            {
                AeCurveGenInfo[AeInfo->Id].ExpT[AE_GEN_IDX_MAX] = AE_MAX_EXPT_33333;
            }
            else if (CurFps == SEL_VIDEOFPS_30)
            {
                AeCurveGenInfo[AeInfo->Id].ExpT[AE_GEN_IDX_MAX] = AE_MAX_EXPT_33333;
            ...
            AeCurveGenInfo[AeInfo->Id].FnoRat = AE_FNO_RATIO;
            AeCurveGenInfo[AeInfo->Id].FnoIdx = 0;
            AeCurveGenInfo[AeInfo->Id].Freq = AE_GEN_FREQ_60;
            if (CurFps == SEL_VIDEOFPS_60)
            {
                AeCurveGenInfo[AeInfo->Id].FreqNode = AE_60FREQ_NODE;
                AeCurveGenInfo[AeInfo->Id].Node1stExpT = AE_MAX_EXPT_16666;
                AeCurveGenInfo[AeInfo->Id].Node2ndISO = AeParamInfo[AeInfo->Id].PrvISO.h/2;
            }
            else if (CurFps == SEL_VIDEOFPS_30)
            {
                AeCurveGenInfo[AeInfo->Id].FreqNode = AE_60FREQ_NODE;
                AeCurveGenInfo[AeInfo->Id].Node1stExpT = AE_MAX_EXPT_33333;
                AeCurveGenInfo[AeInfo->Id].Node2ndISO = (AeParamInfo[AeInfo->Id].PrvISO.l * 2);
            }
            else
            {
        ...
        {
            DBG_ERR("nFlicker setting error\r\n");
        }
    }
    else
    {
