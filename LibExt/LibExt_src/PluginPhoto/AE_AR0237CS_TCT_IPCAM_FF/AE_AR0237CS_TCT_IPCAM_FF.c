...
#include "ae_alg.h"
#include "AE_AR0237CS_TCT_IPCAM_FF_int.h"
#include "ae_task.h"
#include "IPL_Cmd.h"
#include "iqs_api.h"
#include "aeafd_alg.h"
...
#if CAL_ISO
#include "ipl_cal_datapassing_AR0237CS_TCT_EVB_FF_int.h"
#endif
...
void AE_Init (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    //#NT#2017/04/21#Jarkko Chang -begin
    //#NT# add support new hdr library
    UINT32 expround = 0;
    //#NT#2017/04/21#Jarkko Chang -end
    ...
    IPL_AlgSetUIInfo(Id, IPL_SEL_AEMODE,SEL_AEMODE_CENTERWEIGHTED);
    ...
    if (AeAppMode == AE_APPMODE_SHDR)
    {
        ...
        AeInfo[Id].ExtSet.OverExp.AdjLumTab = ExptY_Photo_OverExpoAdj_HDR[Id];

        if(AeInfo[Id].ExtSet.ExpLumDiv > SHDR_RATIO_GAIN_1X)
                    expround = (1 << (AeInfo[Id].ExtSet.ExpLumDiv - 1));
                else
                    expround = 0;

        ExpoTime[0] = ((PrvNewAeArg[Id][0].ExpoTime + expround) >> AeInfo[Id].ExtSet.ExpLumDiv);
        ExpoTime[1] = PrvNewAeArg[Id][0].ExpoTime;//bright image
    ...
    {
        AeInfo[Id].ExtSet.ExpLumDiv = 0;   // for linear mode
...
void AE_Process (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    if ( uiISOGain[Id]!=0 && uiExpoTime[Id]!=0 )
    {
        ...
        {
            DBG_ERR("Manual_AE Id: %d, Exp: %d, ISO: %d, Iris: %d, Lum: %d\r\n", Id, uiExpoTime[Id], uiISOGain[Id], uiIris[Id], AeInfo[Id].PrvAvgLum.y);
        }
        ...
        //#NT#2016/08/05#XL_Wei -begin
        //#NT#get statisic info for debug usage
        AE_getStatisticInfo(&AeInfo[Id], &AeBlkInfo[Id]);
        //#NT#2016/08/05#XL_Wei -end
    ...
    if (AEStep[Id] == 0)
    {
        ...
        if (AeAppMode == AE_APPMODE_SHDR)
        {
            ...
            {
                ExpoTime[1] = PrvNewAeArg[Id][i].ExpoTime;  //bright image
                if(AeInfo[Id].ExtSet.ExpLumDiv > SHDR_RATIO_GAIN_1X)
                    expround = (1 << (AeInfo[Id].ExtSet.ExpLumDiv - 1));
                else
                    expround = 0;

                ExpoTime[0] = ((PrvNewAeArg[Id][i].ExpoTime + expround) >> AeInfo[Id].ExtSet.ExpLumDiv);    //dark image
	            ...
	            //update gt gain
    	        SHDR_GT_Gain = gExpTSetting[Id][InStrPos[Id]+i+AeIntCtrlInfo[Id].CalSetShiftNum].LineRatio;
            }
        }
        else
        {
    ...
    if (uiAEOSDCmd[Id] == ENABLE)
    {
        ...
        snprintf((char *)msg, sizeof(msg), "%ld %ld %ld", AeInfo[Id].CurLum.y, PrvNewAeArg[Id][AEStep[Id]].ExpoTime, PrvNewAeArg[Id][AEStep[Id]].ISOGain);
    ...
    if (AEUartMsg == ENABLE || uiAEUartCmd[Id] == ENABLE)
    {
        DBG_DUMP("%d: %3d %3d %3d %8d %8d %3d %3d %3d %3d %3d %3d %d\r\n", Id, AeInfo[Id].PrvAvgLum.y, AeInfo[Id].CurExpectLum.y, AeInfo[Id].CurLum.y, AeInfo[Id].PrvAvgEV.EVValue, PrvNewAeArg[Id][AEStep[Id]].ExpoTime, PrvNewAeArg[Id][AEStep[Id]].ISOGain, IPL_AlgGetUIInfo(Id, IPL_SEL_WDR), g3DNR[Id], AeInfo[Id].Counter, AeInfo[Id].CounterRF, AEStep[Id], uiAEManualCmd[Id]);
    }

    AEStep[Id] = (AEStep[Id]+1)%AeIntCtrlInfo[Id].AEPeroid;  //AE Peroid

    if(!AE_firstStable[Id] && AeInfo[Id].Counter > AE_STABLE_THRESHOLD){
        AE_firstStable[Id] = TRUE;
        AE_stableLum[Id] = AeInfo[Id].PrvAvgLum.raw;
        AE_stableEV[Id] = AeInfo[Id].PrvAvgEV.EVValue;
    }
}
...
{
    if(uiISOGain[Id]!=0 && uiExpoTime[Id]!=0)
        return AEAlg_Conv2LV(AE_stableEV[Id] * AeInfo[Id].PrvAvgLum.raw / AE_stableLum[Id]);
    else
...
{
    //#NT#2016/08/08#XL_Wei -begin
    //#NT#Calculate EV when manual mode
    if(uiISOGain[Id]!=0 && uiExpoTime[Id]!=0){
        return AE_stableEV[Id] * AeInfo[Id].PrvAvgLum.raw / AE_stableLum[Id];
    }
    else{
...
UINT32 AE_getStatisticInfo(AE_INFO *AeInfo, AE_BLK_INFO *AeBlkInfo)
{
    ...
    else
    {
        ...
        //#NT# add support new hdr library
        if (AeAppMode == AE_APPMODE_SHDR)
	        AE_GetLA(AE_ID_2, AeBlkInfo->Gacc, AeBlkInfo->Yacc, AeInfo->histogram.y);
		else
...
void AE_getCtrlInfo(AE_INFO *AeInfo, AE_BLK_INFO *AeBlkInfo)
{
    ...
    if (AeAppMode == AE_APPMODE_SHDR)
    {
        ...
        if ((Info.Mode == IPL_MODE_PREVIEW)||(Info.Mode == IPL_MODE_CAP))
        {
            ...
            AeInfo->ExtSet.OverExp.AdjLumTab = ExptY_Photo_OverExpoAdj_HDR[AeInfo->Id];
        }
        else
        {
            ...
            AeInfo->PrvExpectLum.raw = IQS_GetInvGammaValue(AeInfo->Id, AeInfo->PrvExpectLum.y);
            AeInfo->ExtSet.OverExp.AdjLumTab = ExptY_Mov_OverExpoAdj_HDR[AeInfo->Id];
        }
    }
    else
    {
        ...
        else
        {
            AeBlkInfo->BLKLum_En = DISABLE;
            AeInfo->ExtSet.AdjHistoEn = DISABLE;
            if(IPL_AlgGetUIInfo(AeInfo->Id, IPL_SEL_IRCUT) == SEL_IRCUT_NIGHT)   // IRcut off
                AeInfo->ExtSet.PrvLumRatioTab = ExptY_Mov_IRoff_Prv[AeInfo->Id];
            else
                AeInfo->ExtSet.PrvLumRatioTab = ExptY_Mov_Prv[AeInfo->Id];

            //*2016.08.11 Wendy -start
            if ( IPL_AlgGetUIInfo(AeInfo->Id, IPL_SEL_WDR) != SEL_WDR_OFF )
            {
                AeInfo->ExtSet.OverExp.AdjLumTab = ExptY_Mov_OverExpoAdj_WDR[AeInfo->Id];
            }
            else
            {
        ...
        AeInfo->ExtSet.ExpLumDiv = 0;   // for linear mode
    }
    //#NT#2017/04/21#Jarkko Chang -end
...
static void AE_GetFaceWin(UINT32 Id)
{
    ...
    reSize.x = reSize.y = 0;
    reSize.w = LA_Info.WinXNum * LA_Info.CropSizeH;
    reSize.h = LA_Info.WinYNum * LA_Info.CropSizeV;
...
void AE_SetPLINEInfo(UINT32 Id, AE_EXT_SET_INFO SetInfo)
{
    if (SetInfo.PLineMode == AE_PLINE_AUTOGEN)
    {
        ...
        if (IPL_AlgGetUIInfo(Id, IPL_SEL_FREQUENCY) == SEL_FREQUENCY_50HZ)
        {
            ...
            //#NT#2016/10/27#Wendy Liao -begin
            //#NT# fixed ipcam ExpBound error
            if (SetInfo.ExpTBound.h >= 40000)
                AeCurveGenInfo[Id].Node1stExpT = 40000;
            else
                AeCurveGenInfo[Id].Node1stExpT = SetInfo.ExpTBound.h;

            if (SetInfo.ExpTBound.l >= 40000)
                AeCurveGenInfo[Id].Node1stExpT = SetInfo.ExpTBound.l;
            //#NT#2016/10/27#Wendy Liao -end
        }
        else
        {
            ...
            //#NT#2016/10/27#Wendy Liao -begin
            //#NT# fixed ipcam ExpBound error
            if (SetInfo.ExpTBound.h >= 33333)
                AeCurveGenInfo[Id].Node1stExpT = 33333;
            else
                AeCurveGenInfo[Id].Node1stExpT = SetInfo.ExpTBound.h;

            if (SetInfo.ExpTBound.l >= 33333)
                AeCurveGenInfo[Id].Node1stExpT = SetInfo.ExpTBound.l;
            //#NT#2016/10/27#Wendy Liao -end
        }
        AeCurveGenInfo[Id].Node2ndISO = SetInfo.IsoBound.h/2;
        //#NT#2016/06/30#Jarkko Chang -end

        //#NT#2016/10/27#Wendy Liao -begin
        //#NT# fixed ipcam IsoBound error
        if (SetInfo.IsoBound.l*2 <= SetInfo.IsoBound.h)
            AeCurveGenInfo[Id].Node2ndISO = SetInfo.IsoBound.l*2;
        else
...
AE_RESET AE_setWin(UINT32 Id, AE_WIN mode)
{
    ...
    switch(mode)
    {
        ...
        case AE_WIN_USERDEFINE:
            AeWin = (UINT8(*)[AE_WIN_X])AE_Win_UserDef;
            break;
...
AE_RESET AE_getPrvTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_INFO *AeInfo)
{
	...
	static UINT32 shdr_ratio = SHDR_RATIO_GAIN_1X;
    ...
    else
    {
        if ((Flicker[AeInfo->Id] == nFlicker)&&
			(shdr_ratio == SensorHDR_GetAeRatio())&&
            (Iso[AeInfo->Id] == nISO)&&
    ...
    //#NT#2016/08/10#XL_Wei -end
    {
        if (AeAppMode == AE_APPMODE_SHDR)
        {
		    //#NT#2017/04/21#Jarkko Chang -begin
            //#NT# add support new hdr library
			switch (SensorHDR_GetAeRatio())
			{
				case SHDR_RATIO_GAIN_16X:
	                 AeInfo->PrvCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_HDR16X_AntiFlicker60;
					break;

				case SHDR_RATIO_GAIN_8X:
	                 AeInfo->PrvCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_HDR8X_AntiFlicker60;
					break;
				...
				case SHDR_RATIO_GAIN_2X:
	                 AeInfo->PrvCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_HDR2X_AntiFlicker60;
					break;
...
UINT32 AE_Get_IrcutInfo(void* data)
{
    AE_IRCUT_INFO* info = (AE_IRCUT_INFO*)data;
    UINT32 Id = info->Id;

    info->avg_y = AeInfo[Id].PrvAvgLum.y;
    info->ev = AeInfo[Id].PrvAvgEV.EVValue;
    info->iso = PrvNewAeArg[Id][AEStep[Id]].ISOGain;
    info->expt= PrvNewAeArg[Id][AEStep[Id]].ExpoTime;
    return 0;
}
