...
#include "ae_alg.h"
#include "ae_IMX123LQT_IPCAM_FF_int.h"
#include "ae_task.h"
#include "IPL_Cmd.h"
#include "iqs_api.h"
#include "aeafd_alg.h"
...
#if CAL_ISO
#include "ipl_cal_datapassing_IMX123LQT_EVB_FF_int.h"
#endif
...
void AE_Process (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    if ( uiISOGain[Id]!=0 && uiExpoTime[Id]!=0 )
    {
        ...
        //#NT#2016/09/06#XL_Wei -begin
        //#NT#support auto ircut
        AE_getStatisticInfo(&AeInfo[Id], &AeBlkInfo[Id]);
        if (IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_AUTO){
            UINT32 ev = AE_stableEV[Id] * AeInfo[Id].PrvAvgLum.raw / AE_stableLum[Id];
            AE_SetIRCut(Id, AEAlg_Conv2LV(ev));
        }
        //#NT#2016/09/06#XL_Wei -end
...
void AE_getCtrlInfo(AE_INFO *AeInfo, AE_BLK_INFO *AeBlkInfo)
{
    ...
    else
    {
        ...
        {
            AeBlkInfo->BLKLum_En = DISABLE;
            AeInfo->ExtSet.AdjHistoEn = DISABLE;// ENABLE; // 160318
            AeInfo->ExtSet.PrvLumRatioTab = ExptY_Mov_Prv[AeInfo->Id];
            AeInfo->ExtSet.OverExp.AdjLumTab = ExptY_Mov_OverExpoAdj[AeInfo->Id];
...
//    memset((void *)AeInfo, 0, sizeof(AE_INFO));
    ...
    AeInfo->ExtSet.OverExp.UserDefRatio = FALSE;    // = TRUE;
