...
//extern void AWB_setGain(UINT32 RG, UINT32 GG, UINT32 BG); //ipl tmp
...
void flash_RedEyeReduct(FLASH_PARA_SET FlashSetting_api)
{
    ...
    FlashMode = IPL_AlgGetUIInfo(IPL_ID_1, IPL_SEL_FLASHMODE);
    ...
    if(FlashParam.Enable && ((FlashMode == SEL_FLASHMODE_AUTOREDEYE)||(FlashMode == SEL_FLASHMODE_FORCEONREDEYE)))
    {
        ...
        CurSensorMode = (SENSOR_MODE)IPL_AlgGetUIInfo(IPL_ID_1, IPL_SEL_PRVSENMODE);
        SensorInfo.Mode = NULL;
        ErrCode = Sensor_GetStatus(SENSOR_ID_1, CurSensorMode, &SensorInfo);
        if(ErrCode != E_OK)
        {
            DBG_ERR("Sensor Info error!!\r\n");
            return;
        }

        if(SensorInfo.Mode != NULL)
...
{
    switch(IPL_AlgGetUIInfo(IPL_ID_1, IPL_SEL_FLASHMODE))
    {
        case SEL_FLASHMODE_AUTO:
        case SEL_FLASHMODE_AUTOREDEYE:
            if(AE_GetPrvLVValue(IPL_ID_1)<FLASH_TRIGGERPOINT) // LV5.0
...
static FLASH_INFOR flash_PreFlashFlow_IGBT(FLASH_PARA_SET FlashSetting_api)
{
    ...
    UINT32 FlashMode;
    FLASH_INFOR Flashlight_Setting = {0}, Pre_FlashSetting = {0};
    ...
    UINT32 PrvISO[SEN_MFRAME_MAX_NUM] = {0}, PrvExpT[SEN_MFRAME_MAX_NUM] = {0}, PrvIris[SEN_MFRAME_MAX_NUM] = {0}, Ratio[SEN_MFRAME_MAX_NUM] = {0};
    UINT32 CapISO, CapExpT, CapIris;
    ...
    //Sensor_GetCurMode(SENSOR_ID_1, &CurSensorMode);
    CurSensorMode = (SENSOR_MODE)IPL_AlgGetUIInfo(IPL_ID_1, IPL_SEL_PRVSENMODE);
    CurSensorInfo.Mode = NULL;
    ErrCode = Sensor_GetStatus(SENSOR_ID_1, CurSensorMode, &CurSensorInfo);
    if((CurSensorInfo.Mode == NULL) || (ErrCode != E_OK))
    {
        DBG_ERR("Sensor mode error!!\r\n");
        return Flashlight_Setting;
    }

    CapSensorMode = (SENSOR_MODE)IPL_AlgGetUIInfo(IPL_ID_1, IPL_SEL_CAPSENMODE);
    CapSensorInfo.Mode = NULL;
    ErrCode = Sensor_GetStatus(SENSOR_ID_1, CapSensorMode, &CapSensorInfo);
    if((CapSensorInfo.Mode == NULL) || (ErrCode != E_OK))
    {
        DBG_ERR("Sensor mode error!!\r\n");
        return Flashlight_Setting;
    }
    ...
    if(FlashParam.Enable)
    {
        ...
        AE_GetCapAEArg(IPL_ID_1, &CapISO,&CapExpT,&CapIris);
        ...
        PrvExpT[IPL_ID_1] = FlashInputParam.PreviewExpoTime;
        Sensor_GetExpoSetting(SENSOR_ID_1, &PrvExpT[0], CurSensorMode, &ExpTSetting, &Ratio[0]);

        AE_GetCapAEArg(IPL_ID_1, &PrvISO[0],&PrvExpT[0],&PrvIris[0]);

        if(PrvISO[0] > 400)
            PrvISO[0] = 400;
         else if(PrvISO[0] < 100)
            PrvISO[0] = 50;
         else
            PrvISO[0]/=2;

        Sensor_GetGainSetting(SENSOR_ID_1, &PrvISO[0], &Gain);
        ...
        IPC_WaitVD(IPL_ID_1, TRUE);
        IPC_WaitVD(IPL_ID_1, TRUE);
//        Img_GetCA_Result(IPL_ID_1,&NoFlash_R[0][0],&NoFlash_G[0][0],&NoFlash_B[0][0],Flash_CAWinX*Flash_CAWinY);  //ipl tmp
        ...
        IPC_WaitVD(IPL_ID_1, TRUE);
        IPC_WaitVD(IPL_ID_1, TRUE);
//        Img_GetCA_Result(IPL_ID_1,&PreFlash_R[0][0],&PreFlash_G[0][0],&PreFlash_B[0][0],Flash_CAWinX*Flash_CAWinY);   //ipl tmp
        ...
        FlashParam.ISO_MainFlash = PrvISO[0]; // Modify ISO by LV
...
//        AWB_GetColorGian(&NoFlashRgain,&NoFlashGgain,&NoFlashBgain);  //ipl tmp
...
//        AWB_setGain(FlashWB_CapRgain,FlashWB_CapGgain,FlashWB_CapBgain);  //ipl tmp
...
{
    FLASH_INFOR Flashlight_Setting = {0};
    ...
    CapSensorMode = (SENSOR_MODE)IPL_AlgGetUIInfo(IPL_ID_1, IPL_SEL_CAPSENMODE);
    SensorInfo.Mode = NULL;
    ErrCode = Sensor_GetStatus(SENSOR_ID_1, CapSensorMode, &SensorInfo);
    if((SensorInfo.Mode == NULL) || (ErrCode != E_OK))
    {
        DBG_ERR("Sensor mode error!!\r\n");
        return Flashlight_Setting;
        //return;
    ...
    if(AE_GetPrvLVValue(IPL_ID_1)< 60)
        FlashParam.ISO_MainFlash = 200; // Modify ISO by LV
    else if(AE_GetPrvLVValue(IPL_ID_1)<70)
        FlashParam.ISO_MainFlash = 100; // Modify ISO by LV
    else if(AE_GetPrvLVValue(IPL_ID_1)<80)
...
FLASH_INFOR gMainFlash_Infor = {0}; //ipl temp, flash not ready
FLASH_INFOR flash_PreFlashFlow(FLASH_PARA_SET FlashSetting_api)
{
    ...
    if(0)//(FLASH_LIGHT ==   _FLASH_SCR_)
        gMainFlash_Infor = flash_PreFlashFlow_SCR();
    else
        gMainFlash_Infor = flash_PreFlashFlow_IGBT(FlashSetting_api);
    //#NT#2016/05/25#Harry_Tsai -end
    // To be modified
    //SetFlashFireStatus(FlashParam.Enable);

    //IPL_temp Flash not ready
    MainFlash_Infor = gMainFlash_Infor;
    MainFlash_Infor.TriggerEnable = DISABLE;
...
void flash_PreFlashFlow_Test(void)
{
    ...
    EXPOSURE_SETTING ExpTSetting;
    GAIN_SETTING Gain;
    UINT32 PrvISO[SEN_MFRAME_MAX_NUM] = {0}, PrvExpT[SEN_MFRAME_MAX_NUM] = {0}, Ratio[SEN_MFRAME_MAX_NUM] = {0};
    ...
    for(Cnt=0; Cnt<(sizeof(FlashTestTbl)/4);Cnt++)
    {
        ...
        FlashInputParam.PreviewExpoTime = CurSensorInfo.Mode->SenHD->Period*(CurSensorInfo.Mode->SenVD->Period)/(CurSensorInfo.Mode->Pclk/1000000);//UNIT is ms
        PrvExpT[0] = FlashInputParam.PreviewExpoTime;
        Sensor_GetExpoSetting(SENSOR_ID_1, &PrvExpT[0], CurSensorMode, &ExpTSetting, &Ratio[0]);
        PrvISO[0] = 200;
        Sensor_GetGainSetting(SENSOR_ID_1, &PrvISO[0], &Gain);
...
#if 0
    UINT32 i,j;
    Addr += snprintf((char *)Addr, 200, "\r\nFlash Info------------------------------------------------------------------------------------------------------------------------\r\n");
    Addr += snprintf((char *)Addr, 200, "Flash Enable = %d, Redeye = %d, ET = %d, ISO = %d\r\n",FlashParam.Enable,FlashParam.RedeyeEnable,FlashParam.ExpoTime_MainFlash,FlashParam.ISO_MainFlash);
    Addr += snprintf((char *)Addr, 200, "NoFlashY = %d, PreflashY = %d, PreflashWidth = %d, Mainflash = %d, CapBackLum = %d, CapFlashCompensation = %d\r\n",FlashParam.YNoFlash, FlashParam.YPreFlash, FlashParam.WidthPreFlash, FlashParam.WidthMainFlash,FlashInputParam.FlashReturnInfo.CaptureBackLum,FlashInputParam.FlashReturnInfo.CaptureFlashCompensateLum);
    Addr += snprintf((char *)Addr, 200, "NoflashRgain = %d, NoflashBgain = %d, PreFlashRgain = %d, PreflashBgain = %d, CapflashRgain = %d, CapflashBgain = %d\r\n",NoFlashRgain,NoFlashBgain,FlashWB_PrvRgain,FlashWB_PrvBgain,FlashWB_CapRgain,FlashWB_CapBgain);
    Addr += snprintf((char *)Addr, 200, "Noflash R = %d, G = %d, B = %d, CapFlashR = %d, CapFlashG = %d, CapFlashB = %d\r\n",NoFlashR,NoFlashG,NoFlashB,CapFlashR,CapFlashG,CapFlashB);

    Addr += snprintf((char *)Addr, 50, "Noflash CA:\r\n");
    for(j=0;j<Flash_CAWinY;j++)
    {
        for(i=0;i<Flash_CAWinX;i++)
        {
            Addr += snprintf((char *)Addr, 50, "%4d %4d %4d\t", NoFlash_R[j][i]>>4,NoFlash_G[j][i]>>4,NoFlash_B[j][i]>>4);
        }
        Addr += snprintf((char *)Addr, 10, "\r\n");
    }

    Addr += snprintf((char *)Addr, 50, "Preflash CA:\r\n");
    for(j=0;j<Flash_CAWinY;j++)
    {
        for(i=0;i<Flash_CAWinX;i++)
        {
            Addr += snprintf((char *)Addr, 50, "%4d %4d %4d\t", PreFlash_R[j][i]>>4,PreFlash_G[j][i]>>4,PreFlash_B[j][i]>>4);
        }
        Addr += snprintf((char *)Addr, 10, "\r\n");
    }
#endif
