...
static UINT8 (*AeWin[AE_PROC_CNT])[AE_WIN_Y][AE_WIN_X];
...
void AE_Init (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    if (FALSE == Init_Flag[Id])
    {
        ...
        //#NT# add for support ae curve auto gen
        if (AeExtSetDefaultParamInfo[Id].PLineMode == AE_PLINE_AUTOGEN)
        {
...
UINT32 AE_getStatisticInfo(AE_INFO *AeInfo, AE_BLK_INFO *AeBlkInfo)
{
    ...
    if ((AeAppMode == AE_APPMODE_STITCH) && (Sensor_IsOpen(IPL_ID_2) == TRUE)) //Stitch Mode only Run AE_ID_1
    {
        ...
        for (j = 0; j < LA_WIN_Y; j ++)
        {
            for (i = 0; i < LA_WIN_X; i ++)
            {
                GTotal += ((*AeWin[AeInfo->Id])[j/Ratio_Y][i/Ratio_X] * AeBlkInfo->Gacc[j * LA_WIN_X + i]);
                YTotal += ((*AeWin[AeInfo->Id])[j/Ratio_Y][i/Ratio_X] * AeBlkInfo->Yacc[j * LA_WIN_X + i]);
                WTotal += (*AeWin[AeInfo->Id])[j/Ratio_Y][i/Ratio_X];
            }
        }
        ...
        for (j = 0; j < LA_WIN_Y; j ++)
        {
            for (i = 0; i < LA_WIN_X; i ++)
            {
                GTotal += ((*AeWin[AeInfo->Id])[j/Ratio_Y][AE_WIN_X/2+i/Ratio_X] * AeBlkInfo->Gacc[j * LA_WIN_X + i]);
                YTotal += ((*AeWin[AeInfo->Id])[j/Ratio_Y][AE_WIN_Y/2+i/Ratio_X] * AeBlkInfo->Yacc[j * LA_WIN_X + i]);
                WTotal += (*AeWin[AeInfo->Id])[j/Ratio_Y][i/Ratio_X];
            }
        }
    ...
    else
    {
        ...
        for (j = 0; j < LA_WIN_Y; j ++)
        {
            for (i = 0; i < LA_WIN_X; i ++)
            {
                ...
                YTotal += ((*AeWin[AeInfo->Id])[j/Ratio_Y][i/Ratio_X] * AeBlkInfo->Yacc[j * LA_WIN_X + i]);
                WTotal += (*AeWin[AeInfo->Id])[j/Ratio_Y][i/Ratio_X];
                weighting_histogram.y[(AeBlkInfo->Yacc[j * LA_WIN_X + i]/HistIntervalY)] += AE_OverExpo_Win_Normal[AeInfo->Id][j/Ratio_Y][i/Ratio_X];
                weighting_histogram.data_num += (AE_OverExpo_Win_Normal[AeInfo->Id][j/Ratio_Y][i/Ratio_X]);
            }
        }
...
AE_RESET AE_setWin(UINT32 Id, AE_WIN mode)
{
    ...
    switch(mode)
    {
        case AE_WIN_CENTERWEIGHT:
            AeWin[Id] = (UINT8 (*)[8][8])AE_Win_CenterWeighted;
            break;

        case AE_WIN_SPOT:
            AeWin[Id] = (UINT8 (*)[8][8])AE_Win_Spot;
            break;

        case AE_WIN_MARTIX:
            AeWin[Id] = (UINT8 (*)[8][8])AE_Win_Multi;
            break;

        case AE_WIN_USERDEFINE:
            AeWin[Id] = (UINT8 (*)[8][8])AE_Win_UserDef;
            break;

        default:
        case AE_WIN_EVALUATIVE:
            AeWin[Id] = (UINT8 (*)[8][8])AE_Win_Evaluative[Id];
            break;
    }
