/**
    ...
    @file       awb_AR0238CS_OV9750_C11_FF.c
    @ingroup    mISYSAlg
...
#include "awb_api.h"
#include "awb_task.h"
#include "awb_AR0238CS_OV9750_C11_FF_int.h"
#include "ae_api.h"
#include "NOTE.h"
...
void AWB_Init(UINT32 id, UINT32 CurrentStatus)
{
    ...
    if ( Init_Flag[id] == FALSE )
    {
        ...
        if (IPL_AlgGetUIInfo((IPL_PROC_ID)id, LENS_TYPE) == 0)
        {
            //old lens
        ...
        {
            //new lens
            AwbInfo[id].DefGain.RGain = AWB_DEF_RGAIN2*AwbInfo[id].KGainInfo.KGain[0].KRGain/AWB_BASE_KGAIN;
            AwbInfo[id].DefGain.GGain = 256;
            AwbInfo[id].DefGain.BGain = AWB_DEF_BGAIN2*AwbInfo[id].KGainInfo.KGain[0].KBGain/AWB_BASE_KGAIN;
        }
    ...
    {
        //new lens
        AwbInfo[id].CTParam = &AwbCTParam2[id];
        AwbInfo[id].PostParam = &AwbPostParam2[id];
        AwbInfo[id].Tab = AwbWhiteTAB2[id];
    }
