...
#if CAL_ISO
#include "ipl_cal_datapassing_OV5658M_EVB_FF_int.h"
#endif
...
UINT32 AE_stableLum[AE_PROC_CNT] = {1000, 1000};
UINT32 AE_stableEV[AE_PROC_CNT] = {1000, 1000};
...
void AE_Process (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    if (AEStep[Id] == 0)
    {
        ...
        else
        {
            ...
            //#NT#2016/07/22#XL_Wei -begin
            //#NT#Disable Ircut control, this will be controled in linux
            /*
            if (IPL_AlgGetUIInfo(Id, IPL_SEL_IRCUT) == SEL_IRCUT_AUTO)
                AE_SetIRCut(Id, AeInfo[Id].PrvAvgEV.LV);
            */
            //#NT#2016/07/22#XL_Wei -end
        }
    }
    else if (AEStep[Id] == 1)
    {
...
AE_RESET AE_getPrvTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_INFO *AeInfo)
{
    ...
    //#NT#2016/08/10#XL_Wei -end
    {
        if (AeAppMode == AE_APPMODE_SHDR)
        {
            ...
            {
                AeInfo->PrvCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_HDR16X;
            }
            else if  (SensorHDR_GetAeRatio() == SHDR_RATIO_GAIN_4X)
            {
                AeInfo->PrvCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_HDR4X;
            }
            else    //N.S. Ratio or 1X Ratio
            {
                AeInfo->PrvCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_HDR1X;
            }
