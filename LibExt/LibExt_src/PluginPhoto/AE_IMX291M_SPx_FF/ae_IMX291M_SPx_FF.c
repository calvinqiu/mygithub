...
#include "ae_alg.h"
#include "ae_IMX291M_SPx_FF_int.h"
#include "ae_task.h"
#include "IPL_Cmd.h"
#include "iqs_api.h"
#include "aeafd_alg.h"
...
#if CAL_ISO
#include "ipl_cal_datapassing_IMX291M_SPx_FF_int.h"
#endif
...
static AEALG_ARG PrvIQAeArg[AE_PROC_CNT][3];
static AEALG_ARG PrvLastAeArg[AE_PROC_CNT][3] = {0};
static UINT32 AECnt[AE_PROC_CNT] = {0};
static UINT32 AEStep[AE_PROC_CNT] = {0};
static UINT32 AEIQStep[AE_PROC_CNT] = {0};
...
void AE_Process (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    if ( uiISOGain[Id]!=0 && uiExpoTime[Id]!=0 )
    {
        ...
        PrvIQAeArg[Id][0] = PrvNewAeArg[Id][0];
        PrvIQAeArg[Id][1] = PrvNewAeArg[Id][1];
        PrvIQAeArg[Id][2] = PrvNewAeArg[Id][2];
        if (AEUartMsg == ENABLE || uiAEUartCmd[Id] == ENABLE)
        {
            DBG_ERR("Manual_AE Id: %d, Exp: %d, ISO: %d, Iris: %d %d\r\n", Id, uiExpoTime[Id], uiISOGain[Id], uiIris[Id],AeInfo[Id].PrvAvgLum.raw);
        }
    ...
    if (AEStep[Id] == 0)
    {
        ...
        else
        {
            ...
            for (i = 0; i < AeIntCtrlInfo[Id].PredictPoint; i ++)
            {
                    ...
                    PrvNewAeArg[Id][i].ISOGain = (PrvNewAeArg[Id][i].ISOGain * Ratio[0] / 100);
                    ISOGain[0] = PrvNewAeArg[Id][i].ISOGain;
                    if (ISOGain[0]>1599)ISOGain[0]=1599;
                    Sensor_GetGainSetting(IPL_UTI_CONV2_SEN_ID(Id), &ISOGain[0], &gGain[Id][(InStrPos[Id] + i + SenInfo.SyncTiming+AeIntCtrlInfo[Id].CalSetShiftNum)%AE_TOTAL_PARAMNUM]);
            }
    ...
    {
        PrvIQAeArg[Id][1] = PrvNewAeArg[Id][0];
        PrvIQAeArg[Id][2] = PrvNewAeArg[Id][1];
        PrvIQAeArg[Id][0] = PrvNewAeArg[Id][2];
        if (AEUartMsg == ENABLE || uiAEUartCmd[Id] == ENABLE)
            AE_getStatisticInfo(&AeInfo[Id], &AeBlkInfo[Id]);
    }
    else if (AEStep[Id] == 2)
...

    {
        ...
        snprintf((char *)msg, sizeof(msg), "%ld %3d %3d 06.16 ", g3DNR[Id],  PrvNewAeArg[Id][AEStep[Id]].ISOGain,PrvNewAeArg[Id][AEStep[Id]].ExpoTime);
//vNewAeArg[Id][AEStep[Id]].ExpoTime,
    ...
    if (AEUartMsg == ENABLE || uiAEUartCmd[Id] == ENABLE)
    {
        DBG_DUMP("%d: %3d %3d %3d %8d %8d %8d %8d %3d  %3d\r\n",
                  Id, AeInfo[Id].PrvAvgLum.y,AeInfo[Id].PrvAvgLum.raw,
                   AeInfo[Id].CurExpectLum.y, AeInfo[Id].CurLum.y,
                   PrvNewAeArg[Id][AEStep[Id]].ExpoTime, PrvNewAeArg[Id][AEStep[Id]].ISOGain,
                   PrvIQAeArg[Id][AEStep[Id]].ExpoTime, PrvIQAeArg[Id][AEStep[Id]].ISOGain,  AEStep[Id]);
    }
...
UINT32 AE_getStatisticInfo(AE_INFO *AeInfo, AE_BLK_INFO *AeBlkInfo)
{
    ...
    else
    {
        ...
        if(PrvNewAeArg[AeInfo->Id][AEStep[AeInfo->Id]].ISOGain>8800)
          Histo_Prv[0][0] = 76;
        else if(PrvNewAeArg[AeInfo->Id][AEStep[AeInfo->Id]].ISOGain>4700)
          Histo_Prv[0][0] =40+(35*(PrvNewAeArg[AeInfo->Id][AEStep[AeInfo->Id]].ISOGain-4699))/4100;
        else
          Histo_Prv[0][0]=40;
