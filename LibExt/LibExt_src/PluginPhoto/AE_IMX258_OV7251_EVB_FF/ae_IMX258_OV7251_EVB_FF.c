/**
    ...
    @file       ae_IMX258_OV7251_EVB_FF.c
    @ingroup    mISYSAlg
...
#include "ae_alg.h"
#include "ae_IMX258_OV7251_EVB_FF_int.h"
#include "ae_task.h"
#include "IPL_Cmd.h"
#include "iqs_api.h"
#include "aeafd_alg.h"
...
static AE_INT_CTRL_INFO AeIntCtrlInfo[AE_PROC_CNT] = {{AE_NOR_CAL_SET_SHIFT, AE_NOR_PREDICT_POINT_1, AE_NOR_PEROID},
                                                      {AE_NOR_CAL_SET_SHIFT, AE_NOR_PREDICT_POINT_2, AE_NOR_PEROID}};
...
static UINT32 testEn = FALSE;
static UINT32 gainTest = 0;
static UINT32 isoTest = 50;
static UINT32 cntTest = 0;
static UINT32 cntThs = 1;
void AE_Process (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    if ( uiISOGain[Id]!=0 && uiExpoTime[Id]!=0 )
    {
        if(uiISOGain[Id]==1){
            ...
            cntTest = 0;
            testEn = TRUE;
            DBG_DUMP("gainTest Enable cnt threshold=%d\r\n", cntThs);
        }
        else if(uiISOGain[Id]==2 && uiExpoTime[Id]==2){
            uiExpoTime[Id] = 3000;
            uiISOGain[Id] = 50;
            testEn = FALSE;
            DBG_DUMP("gainTest Disable\r\n");
        }

        if(testEn == TRUE){
            uiISOGain[Id] = isoTest;
            cntTest ++;
            if(cntTest == cntThs){
                if(isoTest == 1000)
                    gainTest = 1;
                ...
                isoTest = gainTest==0? isoTest+5: isoTest-5;
                cntTest = 0;
            }
        }
        ...
        {
            DBG_DUMP("Manual_AE Id: %d, Lum: %d, Exp: %d, ISO: %d, Gain: %x\r\n", Id, AeInfo[Id].PrvAvgLum.y, uiExpoTime[Id], uiISOGain[Id], gGain[Id][0].data1[0]);
        }
...
void AE_getCtrlInfo(AE_INFO *AeInfo, AE_BLK_INFO *AeBlkInfo)
{
    ...
    AE_INT_CTRL_INFO AeCtrlInfo = {AE_NOR_CAL_SET_SHIFT, AE_NOR_PREDICT_POINT_1, AE_NOR_PEROID};
    if(AeInfo->Id==AE_ID_2)
        AeCtrlInfo.PredictPoint = AE_NOR_PREDICT_POINT_2;
...
AE_RESET AE_getPrvTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_INFO *AeInfo)
{
    ...
    // Test
    //AeInfo->PrvCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_Fix_Expt;
    //AeInfo->PrvCurve = (UINT32(*)[AEALG_INFO_MAX])AeCurve_Fix_Gain;
    //AeInfo->EV = AeParamInfo[AeInfo->Id].EV;
    //AeInfo->ISO = AeParamInfo[AeInfo->Id].PrvISO;
