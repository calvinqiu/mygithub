...
#include "ae_alg.h"
#include "ae_AR0230CS_OV9750_C10_FF_int.h"
...
static UINT32 gSIEGain[AE_PROC_CNT][AE_TOTAL_PARAMNUM];  //Keep 6 frames setting.
...
static AE_PLINEMODE AePLINEMode[AE_PROC_CNT] = {AE_PLINE_PREDEF, AE_PLINE_PREDEF};
...
extern void IPL_CtrlSIExSetDGain(UINT32 Id, UINT32 Gain, UINT32 EnFlag);
...
//MovieStamp
#if AE_STAMPLOG
extern UINT32 debug_AWB_PPROC_Rratio;
extern UINT32 debug_AWB_PPROC_Bratio;
extern UINT32 g3DNR;
...
{
    INT32 lum_min = 0, lum_tag = 0;
    UINT32 CurveFactor = 0, ContrastLevel = 0;;
    UINT32 RGain, GGain, BGain;
    AWB_GetColorGain(Id, &RGain, &GGain, &BGain);
    INT32 uiParam[8];
    H264Test_GetEncInfo(uiParam);

    lum_min = SceneRender_WDR_GetLumaMin();
    lum_tag = SceneRender_WDR_GetLumaShiftTag();
    CurveFactor = SceneRender_WDR_GetCurveFactors(SR_WDR_PROCID1);
    ContrastLevel = SceneRenderWDR_GetContrastLevel();

    snprintf(Buf, BufLen, "0222 %3d %3d %3d %3d %3d %3d %3d %5d %4d %d %d %3d %3d %3d %3d %1d %2d %d %d %d %d\0",
        ...
        g3DNR, IPL_AlgGetUIInfo(Id, IPL_SEL_WDR),
        RGain, BGain,
        debug_AWB_PPROC_Rratio, debug_AWB_PPROC_Bratio,
        uiParam[0],
        uiParam[2],
        CurveFactor, ContrastLevel, lum_min, lum_tag
    );
}
#endif

void AE_SetTrig2Event(void)
{
    set_flg(AEFlagID, 0x10000000);
}

void AE_ClearTrig2Event(void)
{
    clr_flg(AEFlagID, 0x10000000);
}

void AE_waitTrig2Event(void)
{
    FLGPTN uiFlag;
    wai_flg(&uiFlag, AEFlagID, 0x10000000, TWF_CLR);
}
...
void AE_Init (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    memset(gSIEGain[Id], 0xff, sizeof(gSIEGain[Id]));
    AeSyncCnt[Id] = 0;
...
void AE_Process (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    if ( uiISOGain[Id]!=0 && uiExpoTime[Id]!=0 )
    {
        ...
        gSIEGain[Id][0] = (gGain[Id][0].data2[0] * 128 / 1000);
        ...
        {
            IPL_CtrlSIExSetDGain(Id, gSIEGain[Id][0], ENABLE);
            gSIEGain[Id][0] = 0xffff;
        }
    ...
    if (AEStep[Id] == 0)
    {
        ...
        else
        {
            ...
            for (i = 0; i < AeIntCtrlInfo[Id].PredictPoint; i ++)
            {
                ...
                if (SenInfo.Mode->ModeType == SENSOR_MODE_BUILTIN_HDR)
                {
                    gSIEGain[Id][(InStrPos[Id] + i + 1)%AE_TOTAL_PARAMNUM] = (gGain[Id][(InStrPos[Id] + i + SenInfo.SyncTiming)%AE_TOTAL_PARAMNUM].data2[0] * 128 / 1000);
                }
            }
#if (ALTM_VERSION == ALTM_VER_2)
            if (SenInfo.Mode->ModeType == SENSOR_MODE_BUILTIN_HDR)
            {
                AR0230_ALTM_FW_Control_1();
            }
#endif
        }
    }
    ...
    if (SenInfo.Mode->ModeType == SENSOR_MODE_BUILTIN_HDR)
    {
        if (AEStep[Id] == 0)
        {
            AR0230_ALTM_FW_Control_2();
            AR0230_ALTM_FW_Control_3();
        }
        else if (AEStep[Id] == 1)
        {
            AE_waitTrig2Event();
            AR0230_SetSensorRigister();
        }
    }
#endif
    ...
    if (uiAEOSDCmd[Id] == ENABLE)
    {
        ...
        snprintf((char *)msg, sizeof(msg), "%3d %3d %3d %3d %3d", AeInfo[Id].PrvAvgEV.EVValue, AeInfo[Id].PrvAvgEV.LV, PrvNewAeArg[Id][AEStep[Id]].ISOGain,PrvNewAeArg[Id][AEStep[Id]].ExpoTime, AeInfo[Id].Counter);
...
UINT32 AE_getStatisticInfo(AE_INFO *AeInfo, AE_BLK_INFO *AeBlkInfo)
{
    ...
    else
    {
        ...
        rt = Sensor_GetStatus(IPL_UTI_CONV2_SEN_ID(AeInfo->Id), IPL_AlgGetUIInfo(AeInfo->Id, IPL_SEL_PRVSENMODE), &SenInfo);
        if (rt != E_OK || SenInfo.Mode == NULL)
            return 0;

        if (SenInfo.Mode->ModeType == SENSOR_MODE_BUILTIN_HDR)
        {
#if ((ALTM_VERSION == ALTM_VER_1) || (ALTM_VERSION == ALTM_VER_2))
            SENSOR_CMD Cmd;

            //Cmd.uiAddr = 0x315E;
            //[0811][Joyce]reference whole HDR mean (T1 +T2) before ALTM
            Cmd.uiAddr = 0x3152;
            Sensor_ReadReg(AeInfo->Id, &Cmd);

            AeInfo->PrvAvgLum.raw = Cmd.uiData[0];
            AeInfo->PrvAvgLum.y = IQS_GetGammaValue(AeInfo->Id, AeInfo->PrvAvgLum.raw);
...
void AE_SetPLINEInfo(UINT32 Id, AE_EXT_SET_INFO SetInfo)
{
    ...
    {
        AePLINEMode[Id] = AE_PLINE_AUTOGEN;
        ...
        //SetInfo.FnRatio.h
        //SetInfo.FnRatio.l
        AeCurveGenInfo[Id].FnoIdx = 0;
        AeCurveGenInfo[Id].FnoRat = 100;
    ...
    {
        AePLINEMode[Id] = AE_PLINE_MANUAL;
    ...
    {
        AePLINEMode[Id] = AE_PLINE_PREDEF;
        AeCurveUpdate[Id] = TRUE;
...
AE_RESET AE_getPrvTab(AE_ISO nISO, AE_FLICKER nFlicker, AE_INFO *AeInfo)
{
    ...
    static UINT32 CurSenModeType = 0xFF;
    SENSOR_INFO NexSenInfo = {0};
    IPL_MODE_INFOR Info = {0};
    ...
    rt = Sensor_GetStatus(IPL_UTI_CONV2_SEN_ID(AeInfo->Id), IPL_AlgGetUIInfo(AeInfo->Id, IPL_SEL_PRVSENMODE), &NexSenInfo);
    if (rt != E_OK || NexSenInfo.Mode == NULL)
        return AE_RESET_TRUE;
    ...
    if ((Flicker[AeInfo->Id] == nFlicker)&&(Iso[AeInfo->Id] == nISO)&&(CurSenModeType == NexSenInfo.Mode->ModeType)&&(CurFps == Fps[AeInfo->Id]))
    {
        return AE_RESET_FALSE;
    }

    CurSenModeType = NexSenInfo.Mode->ModeType;
    if ( NexSenInfo.Mode->ModeType == SENSOR_MODE_STAGGER_HDR)
    {
        DBG_ERR("not support\r\n");
    }
    ...
    else if(nFlicker == AE_FLICKER_50HZ)
    {
        switch(nISO)
        {
            ...
            default:
                    ...
                    GenInfo.Iso[AE_GEN_IDX_MIN] = AeParamInfo[AeInfo->Id].PrvISO.l;
                    GenInfo.Iso[AE_GEN_IDX_MAX] = AeParamInfo[AeInfo->Id].PrvISO.h;
                    ...
                    if (CurFps == SEL_VIDEOFPS_60)
                    {
                        ...
                        GenInfo.Node2ndISO = AeParamInfo[AeInfo->Id].PrvISO.h;
                    ...
                    else if (CurFps == SEL_VIDEOFPS_30)
                    {
                        ...
                        GenInfo.Node2ndISO = (AeParamInfo[AeInfo->Id].PrvISO.l * 2);
                    }
                    else
                    {
                        DBG_ERR("CurFps setting error 22\r\n");
                    }
                    //#NT#2016/08/22#Jarkko Chang -begin
                    //#NT# fixed warning
                    rt = AEAlg_GenCurve(&GenInfo, &CurveAdr, &AeInfo->EV.h, &AeInfo->EV.l);
                    if (rt != E_OK)
                        DBG_ERR("AE Curve Gen Fail\r\n");
                    //#NT#2016/08/22#Jarkko Chang -end
...
//    UINT32 step = (AeInfo[AE_ID_1].ExtSet.OverExp.CurThreshold - (((1 << 10) / weighting_histogram.bin_num) >> 1)) / ((1 << 10) / weighting_histogram.bin_num);
    UINT32 step = AeInfo[AE_ID_1].ExtSet.OverExp.CurThreshold / ((1 << 10) / weighting_histogram.bin_num);
