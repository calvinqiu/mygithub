...
#if caf_tmp
static UINT32 VACnt = 0
#endif
static UINT32 LastAF_Value = 0;
...
---------------------------------------------------------------------------*/
static void caf_SetSearchRange(UINT32 id, INT16 MinIndex, INT16 MaxIndex)
{
    INT16 IdxSwitch;
...
static void caf_InitParam(UINT32 id)
{
...
static void caf_Initial(UINT32 id)
{
    ...
    /* Setup IPE VA. */
    AF_VA_Setup(id);
    AFdrv_GetCurrentRange(id, &alg_info, SEL_AFMODE_CONTINUEFOCUS);
    caf_SetSearchRange(id, alg_info.iStartIdx, alg_info.iEndIdx);
...
void caf_AutoFocusProcess(UINT32 id)
{
    static UINT32 PrevEV = 99999;
    UINT32 CurEV, CurVA;
    //#NT#2016/05/26#Harry_Tsai -begin
    UINT32 uiG1h = 0, uiG1v = 0, uiG2h = 0, uiG2v = 0;
    //#NT#2016/05/26#Harry_Tsai -end
    ...
    CurEV = AE_GetPrvEVValue(id); // 70 = LV7.0
    #if 0
    CurVA = caf_GetVA();
