...
#include "ae_alg.h"
#include "ae_IMX326LQC_MIO_FF_int.h"
#include "ae_task.h"
#include "IPL_Cmd.h"
#include "iqs_api.h"
#include "aeafd_alg.h"
...
#if CAL_ISO
#include "ipl_cal_datapassing_IMX326LQC_MIO_FF_int.h"
#endif
...
void AE_Process (UINT32 Id, UINT32 CurrentStatus)
{
    ...
    if ( uiISOGain[Id]!=0 && uiExpoTime[Id]!=0 )
    {
        ...
        if (AEUartMsg == ENABLE || uiAEUartCmd[Id] == ENABLE)
        {
            ...
            DBG_ERR("Manual_AE Id: %d, Exp: %d, ISO: %d, PrvAvgLum y %d raw %d ExpT %d ISO %d\r\n", Id, uiExpoTime[Id], uiISOGain[Id], AeInfo[Id].PrvAvgLum.y, AeInfo[Id].PrvAvgLum.raw, uiExpoTime[Id], uiISOGain[Id]);
        }
...
//vNewAeArg[Id][AEStep[Id]].ExpoTime,
    ...
    if (AEUartMsg == ENABLE || uiAEUartCmd[Id] == ENABLE)
    {
        ...
        DBG_DUMP("%d: PrvAvgLum %3d CurExpectLum %3d CurLum %3d PrvAvgEV %8d ExpT %8d ISO %3d %3d %3d %3d %3d\r\n", Id, AeInfo[Id].PrvAvgLum.raw, AeInfo[Id].CurExpectLum.raw, AeInfo[Id].CurLum.raw, AeInfo[Id].PrvAvgEV.EVValue, PrvNewAeArg[Id][AEStep[Id]].ExpoTime, PrvNewAeArg[Id][AEStep[Id]].ISOGain, IPL_AlgGetUIInfo(Id, IPL_SEL_WDR), AeInfo[Id].Counter, AeInfo[Id].CounterRF, AEStep[Id]);
    }
