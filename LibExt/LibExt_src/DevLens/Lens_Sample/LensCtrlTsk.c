...
ER LensCtrl_Open(PLENSCTRL_APPOBJ pLensCtrlObj)
{
    ...
    pLensCtrlObj->pLens->pLensCtrlTskObj->lensctrltsk_ircut_setstate = LensCtrl_IRCUT_SetState;
...
void LensCtrlTsk(void)
{
    ...
    Mask = (FLGLENS_ZOOMFWD|FLGLENS_ZOOMREV|FLGLENS_ZOOMBRK|FLGLENS_ZOOMOFF|
            FLGLENS_ZOOMCHANGE|FLGLENS_ZOOM_CHANGESPPED|FLGLENS_FOCUS_SETSTATE|FLGLENS_APERTURE_SETSTATE|FLGLENS_SHUTTER_SETSTATE|FLGLENS_IRCUT_SETSTATE);

    clr_flg(FLG_ID_LENS, Mask);
    while (LENSCTRLTSK_ID)
    {
...
//Brake move to back
        ...
        if(uiFlag & FLGLENS_IRCUT_SETSTATE)
        {
            gLensCtrlObj.pMotor->ircut_setState(gLensCtrlParamObj.IRCUTState, gLensCtrlParamObj.uiIRCUTParam1);
        }
    }
}
...
void LensCtrl_IRCUT_SetState(MOTOR_IRCUT_ACT state, UINT32 param1)
{
    FLGPTN  Flag;

    gLensCtrlParamObj.IRCUTState = state;
    gLensCtrlParamObj.uiIRCUTParam1 = param1;
    wai_flg(&Flag, FLG_ID_LENS, FLGLENS_IDLE , TWF_ORW|TWF_CLR);
    set_flg(FLG_ID_LENS,FLGLENS_IRCUT_SETSTATE);
    wai_flg(&Flag, FLG_ID_LENS, FLGLENS_IDLE , TWF_ORW);
}
