...
void Lens_Focus_SetFocalLength_um(UINT32 ZoomSection, UINT32 FocalLength)
{
    if(gLensCtrlObj.pLens)
        gLensCtrlObj.pLens->focus_setFocalLength(ZoomSection, FocalLength);
    else
    {
        DBG_ERR("Lens Obj not registered!\r\n");
    }
...
INT32 Lens_IRCUT_Move(IRCUT_POS Position)
{
    if (Lens_Module_GetState() != LENS_STATE_IDLE)
    {
        DBG_ERR("%s, State incorrect, (%d)\r\n",__func__,Lens_Module_GetState());
    ...
    Lens_Module_SetState(LENS_STATE_IRCUTMOVING);
    DBG_MSG("LENS_STATE_IRCUTMOVING\r\n");    // --> LENS_STATE_IRCUTMOVING

    //Callback function
    if(gLensCtrlObj.APICB!=NULL)
    {
        gLensCtrlObj.APICB(LENS_CB_IRCUT_START, NULL);
    }

    gLensCtrlObj.pLens->ircut_setState(MOTOR_IRCUT_NORMAL, Position);

    //Callback function
    if(gLensCtrlObj.APICB!=NULL)
    {
        gLensCtrlObj.APICB(LENS_CB_IRCUT_END, NULL);
    }
