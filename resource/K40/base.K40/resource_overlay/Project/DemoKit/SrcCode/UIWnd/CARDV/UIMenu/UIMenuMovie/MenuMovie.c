...
TMDEF_BEGIN_OPTIONS(MOVIE_CLOUDDOG)
TMDEF_OPTION_TEXT(MOVIE_CLOUDDOG_OFF)
TMDEF_OPTION_TEXT(MOVIE_CLOUDDOG_ON)
TMDEF_END_OPTIONS()
...
TMDEF_ITEM_TEXTID(MOVIE_CLOUDDOG)
...
int Movie_MenuCallback(UINT32 uiMessage, UINT32 uiParam)
{
    ...
    if (uiMessage == TMM_CONFIRM_OPTION)
    {
        ...
        case IDM_MOVIE_PARKING:
            Ux_SendEvent(&CustomMovieObjCtrl, NVTEVT_EXE_PARKING, 1, uwOption);
            break;
        ...
        case IDM_MOVIE_CLOUDDOG:
            Ux_SendEvent(&CustomMovieObjCtrl, NVTEVT_EXE_CLOUDDOG, 1, uwOption);
            break;
