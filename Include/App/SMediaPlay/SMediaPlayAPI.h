...
/*
    Get GPS Data from File

    Get GPS data from file

    param  [in]      TimeInx:  the specific time Index (unit: sec)
    param  [in]      BufAddr:  allocate buffer address
    param  [in/out]  *BufSize:  allocate buffer size / output data size

    @return
     - @b E_OK:     set successfully.
     - @b E_SYS:    function is failed.
*/
extern ER SMediaPlay_GetGPSData(UINT32 TimeInx, UINT32 BufAddr, UINT32 *BufSize);
...
/**
    ...
    @param[in] h264DescAddr   pointer of memory for h264 SPS/PPS
    @param[in] h264DescLen    size of memory for h264 SPS/PPS

    @return
     - @b E_OK:     set successfully.
     - @b E_SYS:    function is failed.
*/
extern ER SMediaPlay_StreamDecodeInit(UINT32 workbuf, UINT32 workbufsize, UINT32 rawWidth, UINT32 rawHeight, UINT32 h264DescAddr, UINT32 h264DescLen);
