...
typedef enum _PB_PRMID
{
    ...
    PBPRMID_SINGLE_DSTREGION	= 0x95, ///< Assign destination region in single view mode. Only support coordinate now.
    PBPRMID_VIDEO_INFO_CALLBACK = 0x96, ///< Callback function for getting video information.
...
/**
    Playback single region display.

    There are three types for single region display.

    @note For PB_SIGDISP_TYPE.
*/
typedef enum _PB_SIGDISP_TYPE
{
    PB_SIGDISP_DISABLE     = 0x00,    ///< Disable single region display.
    PB_SIGDISP_COORDINATE  = 0x01,    ///< Set coordinate(x,y) to top left corner and stretch region to boundary.
    PB_SIGDISP_CUSTOMIZE   = 0x02,    ///< Set coordinate(x,y), width and height to customize display region.
    PB_SIGDISP_CROP		   = 0x03,	  ///< Set crop region of source video. Coordinate(x,y), width and height.
    ENUM_DUMMY4WORD(PB_SIGDISP_TYPE)
} PB_SIGDISP_TYPE;
//------------------------------------------------------------------------------
// Function prototype
//------------------------------------------------------------------------------
...
/**
    Callback function prototype for getting video information.

    The callback function is invoked everytime the video file header is parsed.
    User could get current video information.
*/
typedef BOOL (*PB_VIDEOINFO_CB)(PGXVIDEO_INFO pVidInfo);
//------------------------------------------------------------------------------
// Structures
//------------------------------------------------------------------------------
/**
...
typedef struct
{
    ...
    INT32  iOSDWidth;
    INT32  iOSDHeight;
    INT32  iOutStartX;    //base on OSD coordinate
    INT32  iOutStartY;    //base on OSD coordinate
    INT32  iOutWidth;     //base on OSD coordinate
    INT32  iOutHeight;    //base on OSD coordinate
    PB_SIGDISP_TYPE typeID;
} VIDEO_WIN_ATTR, *PVIDEO_WIN_ATTR;
...
/**
    Get Face numbers and locations in display coordinate.

    @note This API is an Async Function (Means: need to call #PB_WaitCommandFinish() to wait for the operation to finish).

    @param[out] ppRect Pointer to the rectangle of faces.
    @param[out] pNum   Pointer to the number of faces.
    @return void.
