...
extern void     MovRec_SetReadyBuf4RawEnc(UINT32 fileid, MEDIAREC_READYBUF_INFO *pInfo);
...
/**
    ...
    @return void
*/
extern VIDENC_PATH MovRec_GetEncPath(UINT32 uiVidEncId);
...
extern UINT32 MovRec_AddOne(VIDSTRM_VID_INFO *pinfo);

/**
    Pause/resume video path

    @param[in] path, video path number

    @param[in] enable, 1: enable

    @return void
*/
extern void MovRec_PathN_Start_Stop(UINT32 path, UINT32 enable);

/**
    Set max stopflash rollback Keep sec

    @param[in] stop 1:stop encording

    @return void
*/
extern void MovRec_Audio_Start_Stop(UINT32 stop);

/**
    Set valid video path (only for MEDIAREC_VER_VSWA/MEDIAREC_VER_VS)

    @param[in] value: <= 4

    @return void
*/
extern void MovRec_SetValidPathNum(UINT32 value);
...
/**
    Update movrec recorder parameters.

    Update movrec recorder parameters, such as width, height, frame rate, target bit rate.
    Only for MEDIAREC_VER_VS/MEDIAREC_VER_VSWA now
    ...
    @return
        ...
        - @b E_PAR: parameter value error
*/
extern ER MovRec_UpdateParam(UINT32 type, UINT32 p1, UINT32 p2, UINT32 p3);
...
/**
    Get encoding YUV information.

    @param[in] vsID  vidpath id
    @param[in] pInfo  output yuv information
    @return void
*/
extern void MovRec_GetLastV_YUV(UINT32 vsID, MEDIAREC_YUV_INFO* pInfo);
...
/**
    ...
    @return void
*/
extern void MovRec_WaitJpgEnc(UINT32 pathid);
...
/**
    ...
    @return void
*/
extern void MovRec_StartFile(UINT32 pathid);
...
/**
    ...
    @return void
*/
extern void MovRec_StopFile(UINT32 pathid);

/**
    Enable/disable start with filewriting (default: enable)

    @param[in] value (1 as default)

    @return void
*/
extern void MovRec_Enable_RecordStartWithWriting(UINT32 value);
...
/*
    Set video target bitrate (Bytes in second)

    @param  [in] pathid: Path ID
    @param  [in] value: should < Max (1st setting)

    @return E_OK or others
*/
extern ER MovRec_SetVidTBR(UINT32 pathID, UINT32 value);
...
/**
    ...
    @param[in] pathid : 0 = 1st path
    @param[in] filesavetype: MOVREC_FILESAVE_PART1/MOVREC_FILESAVE_PART2
                         (default: MOVREC_FILESAVE_PART1)

    @return void
*/
extern void MovRec_SetEMRsaveType(UINT32 pathid, UINT32 filesavetype);

/**
    Set 2ndpath savetype, PART1:1st storage, PART2:2nd storage.
    ...
    @return void
*/
extern void MovRec_Set2ndPathsaveType(UINT32 filesavetype);

/**
    Enable/disable AAC with Header. MovRec_GetLastA_BS will get AAC bs with/without header.

    @param[in] value: 1 to enable. Default: 0.
    @return void
*/
extern void MovRec_SetAACWithHeader(UINT32 value);

/**
    Enable using last QP as initial QP value.

    @param[in] en: use last QP to encode new frame. (default: off)

    @return void
*/
extern void MovRec_EnUseLastQPtoEncode(UINT32 en);
