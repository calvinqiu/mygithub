...
typedef enum _IPH_SIE_CLKSRC_SEL
{
    ...
    IPH_SIECLKSRC_PLL6    , ///< SIE clock source as PLL6
    IPH_SIECLKSRC_PLL7    , ///< SIE clock source as PLL7
...
*/
typedef enum _SIE_PPB_SW
{
    SIE_BUF_OFF = 0,    ///< -
    SIE_BUF_ON = 1,     ///< -
    ENUM_DUMMY4WORD(SIE_PPB_SW)
}SIE_PPB_SW;
...
{
    NonScaled_Img   = 0,    ///< SIE Dram output source = cropped image
...
typedef struct
{
    UINT32      SizX; ///< horizontal size
    UINT32      SizY; ///< vertical size
} SIE_SRCIMG_WIN_INFO;
...
typedef enum _SIE_INTE_EN
{
    ...
    SIE__INTE_DPCF           = 0x00001000,   ///< enable interrupt: Defect Pixel/Column concealment failure.
    SIE__INTE_RAWENC_OUTOVFL = 0x00002000,   ///< enable interrupt: Raw encoder output buffer full
...
typedef enum _SieFuncEn
{
    ...
    GammaEn     = 0x00100000,   ///< enable Gamma
    ...
    CACCEn      = 0x04000000,   ///< enable Color accumulation      (AWB)
    LACCEn      = 0x08000000,   ///< enable Luminance accumulation  (AE)
    VACCEn      = 0x10000000,   ///< enable Variation accumulation  (AF)
...
typedef struct _SIE_RAWENC
{
    enum
    {
        RAWENC_LINE_MODE  = 0,   ///< fixed rounding
        RAWENC_FRAME_MODE = 1,   ///< random rounding
        ENUM_DUMMY4WORD(BCC_OP_MODE),
    } BCC_OP_MODE;
    BOOL    b2Strp;         ///< Raw encoder - multiple(two) stripe mode
    UINT32  CompRate;       ///< range: 1~100
    UINT32  QInit;          ///< Raw encoder - Q Init Value

    UINT32  ui1stStrpWidth;     ///< valid b2Strp = 1, Raw encoder - 2-stripe line mode - 1st stripe width
    UINT32  ui2ndStrpSpt;       ///< valid b2Strp = 1, Raw encoder - 2-stripe line mode - 2nd stripe start
    UINT32  ui1stStrpLofs;      ///< valid b2Strp = 1, Raw encoder - 2-stripe line mode - 1st stripe output line-offset
    UINT32  ui2ndStrpLofs;      ///< valid b2Strp = 1, Raw encoder - 2-stripe line mode - 2nd stripe output line-offset
}SIE_RAWENC;
...
{
    UINT32  Gma_TblAddr;          ///< tab number:32, Data type: 16bit Four channel use the same table.
}SIE_GAMMA;
...
typedef struct _SIE_SUBFUNC
{
    ...
    SIE_ColumnDef   ColumnDef;  ///< column defect concealment configuration    @SIE1/SIE2
    ...
    SIE_GAMMA   *pGamma;    ///< gamma table                                    @SIE1/SIE2
    struct
    {
