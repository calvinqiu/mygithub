...
{
    IPH_IPECLK0 = 192,  ///< -192 mhz
    IPH_IPECLK1 = 240,  ///< -240 mhz
    IPH_IPECLK2 = 270,  ///< -270 mhz
    IPH_IPECLK3 = 300,  ///< -300 mhz
...
{
    IPE_D2D             = 0,    ///< D2D
    IPE_IPP             = 1,    ///< IFE->DCE->IPE->IME
    IPE_RDE_IPP         = 2,    ///< RDE + IFE->DCE->IPE->IME
...
typedef enum _IPEDMAOutSel
{
    ...
    DMA_SEL_SUBOUT = 1,             ///< subsampled out
    DMA_DIRECT = 2,                 ///< edge direction(for debug)
...
typedef struct _EdgeMap
{
    enum
    {
        EMI_USD = 0,    ///< Select 1st order kernel output for edge mapping input
