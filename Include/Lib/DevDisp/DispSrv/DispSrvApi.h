...
#define DISPSRV_API_VERSION 0x14050910U ///< API version
...
typedef enum _DISPSRV_ER{
    ...
    DISPSRV_ER_DIRECT_SRC_MAP    = -36, ///< error data in DISPSRV_CMD_IDX_SET_DIRECT_SRC_MAP
    DISPSRV_ER_MASK_SRC_NOT_SET  = -37, ///< user set source dual, the DISPSRV_SRC_CFG::uiMaskSrc also need setting.
...
typedef enum _DISPSRV_CMD_IDX{
    ...
    DISPSRV_CMD_IDX_SET_DIRECT_SRC_MAP, ///< set mapping table to select source in direct mode. (In:DISPSRV_DIRECT_SRC_MAP, Out:NULL)
...
typedef enum _DISPSRV_SRC_IDX{
    DISPSRV_SRC_IDX_0 = 0,  ///< primary source
    DISPSRV_SRC_IDX_1,      ///< secondary source
    DISPSRV_SRC_IDX_2,      ///< 3rd source
    DISPSRV_SRC_IDX_3,      ///< 4th source
    DISPSRV_SRC_IDX_4,      ///< 5th source
    DISPSRV_SRC_IDX_T0,     ///< twin - primary source
    DISPSRV_SRC_IDX_T1,     ///< twin - secondary source
    DISPSRV_SRC_IDX_T2,     ///< twin - 3rd source
    DISPSRV_SRC_IDX_T3,     ///< twin - 4th source
    DISPSRV_SRC_IDX_T4,     ///< twin - 5th source
    DISPSRV_SRC_IDX_MAX_NUM,
    DISPSRV_SRC_IDX_DONT_CARE, ///< for DISPSRV_DIRECT_SRC_MAP, if single display, the 2nd source should set to this.
    ENUM_DUMMY4WORD(DISPSRV_SRC_IDX)
}DISPSRV_SRC_IDX;

/**
     @name Definition of backward compatible
*/
//@{
#define DISPSRV_SRC_IDX_PRIMARY DISPSRV_SRC_IDX_0 ///< primary source
#define DISPSRV_SRC_IDX_SECONDARY DISPSRV_SRC_IDX_1 ///< secondary source
//@}
...
{
    BOOL                    bDual;                                  ///< selected-source enable, it also is called 'bEnable_uiMaskSrc'
    UINT32                  uiMaskSrc;                              ///< where bDual=TRUE, use uiMaskSrc&(1<<DISPSRV_SRC_IDX_?) to check source is available
...
/**
    Structure for DISPSRV_FP_GETDRAW_CB
*/
typedef struct _DISPSRV_GETDRAWCB_DATA{
    //[BASIC]
    PIMG_BUF* ppImg;     //DISPSRV_DRAW::pImg
    PIMG_BUF* ppRawImg;  //DISPSRV_DRAW::pRawImg
    MEM_RANGE* pRawMem; //DISPSRV_DRAW::RawRange
}DISPSRV_GETDRAWCB_DATA,*PDISPSRV_GETDRAWCB_DATA;

typedef DISPSRV_ER (*DISPSRV_FP_GETDRAW_CB)(DISPSRV_GETDRAWCB_DATA* pData); /// get draw destination buffer index
/**
    Draw Configuration
*/
typedef struct _DISPSRV_DRAW{
    BOOL                    bDualSrc;                       ///< [R] indicate pDS is "MULTI", user need to check uiMaskSrc to get detail
    UINT32                  uiMaskSrc;                      ///< [R] use uiMaskSrc&(1<<DISPSRV_SRC_IDX_?) to check source is available
    BOOL                    bDualDst;                       ///< [R] indicate pDD is dual
    BOOL                    bDualHandle;                    ///< [R] indiction pImg is dual, and draw callback have to draw these output image
    DISPSRV_SRC_DESC*       pDS[DISPSRV_SRC_IDX_MAX_NUM];   ///< [R] source buffer description for reference
    DISPSRV_DISP_DESC*      pDD[DISPSRV_DISP_IDX_MAX_NUM];  ///< [R] main display buffer description for reference
    PIMG_BUF                pImg[2];                        ///< [R] display buffer for using currently.
    DISPSRV_DISP_IDX        IdxDs[2];                       ///< [R] the index of indicating pImg[] used pDD
    DISPSRV_FP_LOCK_CB      fpLock[DISPSRV_SRC_IDX_MAX_NUM];///< [R] Api for lock source frame
    DISPSRV_FP_UNLOCK_CB    fpUnlock[DISPSRV_SRC_IDX_MAX_NUM];///< [R] Api for unlock source frame
    DISPSRV_FP_GETDRAW_CB   fpGetDraw;                      ///< [R] Api for get next draw buffer
    DISPSRV_FP_FLIP_CB      fpFlip;                         ///< [R] flip frame, set TRUE to wait finish, FALSE without wait finish
    UINT32                  uiFrameCnt;                     ///< [R] Frame count
    UINT32                  uiUserState;                    ///< [R] Reversed for customer using
    //FOR ADVANCE USE to avoid DispSrv inside operation of rotation
    MEM_RANGE               RawMem[2];                      ///< [R] display buffer starting address
    PIMG_BUF                pRawImg[2];                     ///< [R] direct to access corresponding display buffer (for ratation, pRawImg starting address may not equal uiRawStartAddr)
    DISPSRV_FP_FLIP_CB      fpRawFlip;                      ///< [R] flip frame, set TRUE to wait finish, FALSE without wait finish
...
/**
    ...
    To be used in direct to display what source index is.
    SrcIdx[0]: the source index uses to show on primary display
    SrcIdx[1]: the source index uses to show on second display
*/
typedef struct _DISPSRV_DIRECT_SRC_MAP{
    DISPSRV_SRC_IDX SrcIdx[2];
}DISPSRV_DIRECT_SRC_MAP;
