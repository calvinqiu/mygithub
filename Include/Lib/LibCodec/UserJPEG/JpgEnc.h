...
/**
    Re-asign bitstream buffer.

    Revise address for HW limitation.

    @param[in,out] pBuff [in]Original buffer address, [out]revised buffer address
    @param[in,out] pSize [in]Original buffer size, [out]revised buffer size
    @return
        - @b ER_OK:  Succeed.
        - @b ER_SYS:  Failed.
*/
