...
/**
     ...
     Call this API to enroll FDE target(replace one of the 5 default targets).
     when new target enrolled, new detector may also need to enroll(use "nvtfd_enrolldetector").
     when either ptarget, ptgtstat or ptgtparam is NULL, the target will be deleted.

     @param[in] target: the FDE target information.
     ...
     @code
     {
        UINT32 tgt[48] = {...};
        FDE_TGTSTAT tgtstat = {...};
        FDE_TGTPARAM tgtparam = {...};
        
        FD_FDE_TARGET target = {tgt, &tgtstat, &tgtparam, 0, FDE_R000_P000};

        fd_enrolltarget(&target);
     }
     @endcode
*/
extern VOID fd_enrolltarget(FD_FDE_TARGET *target);
