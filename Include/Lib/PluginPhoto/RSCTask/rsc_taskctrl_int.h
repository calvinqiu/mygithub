...
typedef enum _RSC_CTRL_STATUS
{
    ...
    RSC_ACTDURATION    = 0x00000020,    ///< STATUS RSC_ACT (ON ACT DURATION OR NOT).
    RSC_SAVE2DLUT      = 0x00000040,    ///< STATUS RSC_ACT (SAVE 2DLUT INFORMATION AND RAW).
...
typedef enum
{
    ...
    _DIR_NEQ,      ///< negative direction, X: left , Y: down, Z: counter-clockwise
...
extern void RSCCTRL_ClearInit(void);
...
extern ER RSC_HandleGyroData(RSCCTRL_GYRO_TBL* gyrodata);
extern UINT32 RSC_FrameTimeChange(UINT32 ftime,UINT32 expT,SENSOR_FPS_TYPE FPSType);
extern ER RSC_GetSensorInfor(RSCCTRL_SEN_INFO* SensorInfor);
extern ER RSC_SetCtrlStatus(IPL_PROC_ID Id,UINT32 ststus);
extern ER RSC_SaveRaw(void);
#endif
