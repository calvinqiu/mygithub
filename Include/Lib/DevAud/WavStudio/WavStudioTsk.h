...
{
    WAVSTUD_MODE_1P1R = 0,              ///< 1 Play 1 Record
    WAVSTUD_MODE_1P2R,                  ///< 1 Play 2 Record
    WAVSTUD_MODE_1P0R,                  ///< 1 Play 0 Record
    WAVSTUD_MODE_0P1R,                  ///< 0 Play 1 Record
    WAVSTUD_MODE_0P2R,                  ///< 0 Play 2 Record
    WAVSTUD_MODE_MAX
}WAVSTUD_MODE;
...
typedef enum _WAVSTUD_CFG
{
    ...
    WAVSTUD_CFG_PASSIVE_PLAY,           ///< Switch passive play mode
    ...
    WAVSTUD_CFG_PUSH_PLAY_QUE,          ///< Push data to play queue. Only for passive play mode
    WAVSTUD_CFG_RECV_CB,                ///< Callback function to notify the source of play data. Only for passive play mode
    WAVSTUD_CFG_GETSTART_CB,            ///< Callback function to get playback/record start time. For AEC.
    WAVSTUD_CFG_ADDBUFFQUE_CB,          ///< Callback function to notify that buffer is added to audio driver. For AEC.
    WAVSTUD_CFG_STOP_CB,                ///< Callback function to notify that playback/record is stopped. For AEC.
    WAVSTUD_CFG_MAX
}WAVSTUD_CFG;
...
typedef void (*WAVSTUD_GetStartCB)(UINT32 p1, UINT32 p2, UINT32 p3);

typedef void (*WAVSTUD_AddBufCB)(UINT32 p1, UINT32 p2);

typedef void (*WAVSTUD_StopCB)(void);
...
typedef struct _WAVSTUD_AUD_INFO
{
    ...
    WAVSTUD_CODEC       codec;          ///< Codec (e.g. PCM)
...
{
    WAVSTUD_MODE mode;                  ///< WavStudio mode

    WAVSTUD_AUD_INFO playInfo;          ///< Audio info of play
    UINT32 playBufCount;                ///< Play buffer count (Should >= 5)
    WAVSTUD_UNIT_TIME playUnitTime;     ///< Play unit time

    WAVSTUD_AUD_INFO recInfo;           ///< Audio info of record
    UINT32 recBufCount;                 ///< Record buffer count (Should >= 5)
    WAVSTUD_UNIT_TIME recUnitTime;      ///< Record unit time
...
/**
    ...
    @return Open status
        ...
        - @b E_OK: No error
*/
extern ER WavStudio_Open(PWAVSTUD_APPOBJ pWavObj, DX_HANDLE dxSndHdl, PWAVSTUD_INFO_SET pWavInfoSet);
...
*/
extern BOOL WavStudio_Start(WAVSTUD_ACT actType, PWAVSTUD_AUD_INFO info, UINT64 stopcount, BOOL (*DataCB)(PAUDIO_BUF_QUEUE, UINT32));
...
*/
extern BOOL WavStudio_Stop(WAVSTUD_ACT act);
...
*/
extern BOOL WavStudio_Pause(WAVSTUD_ACT act);
...
*/
extern BOOL WavStudio_Resume(WAVSTUD_ACT act);
...
/**
    ...
    @param[in]  actType         Act type.
*/
extern void WavStudio_WaitStart(WAVSTUD_ACT act);
...
/**
    ...
    @param[in]  actType         Act type.
*/
extern void WavStudio_WaitStop(WAVSTUD_ACT act);
