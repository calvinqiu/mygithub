...
typedef enum _FWSRV_CMD_IDX{
    ...
    FWSRV_CMD_IDX_ECOS_WRITE,       ///< Input:FWSRV_ECOS_BUF_DESC
    FWSRV_CMD_IDX_ECOS_READ,        ///< Input:FWSRV_ECOS_BUF_DESC
    FWSRV_CMD_IDX_DSP_WRITE,        ///< Input:FWSRV_DSP_BUF_DESC
    FWSRV_CMD_IDX_DSP_READ,         ///< Input:FWSRV_DSP_BUF_DESC
    FWSRV_CMD_IDX_LINUX_WRITE,      ///< Input:FWSRV_LINUX_BUF_DESC
    FWSRV_CMD_IDX_LINUX_READ,       ///< Input:FWSRV_LINUX_BUF_DESC
    ...
    FWSRV_CMD_IDX_DECOMPRESS,       ///< Input:FWSRV_DECOMPRESS_DESC, decompress compressed uITRON
    FWSRV_CMD_IDX_BIN_UPDATE_ALL_IN_ONE, ///< Input:FWSRV_BIN_UPDATE_ALL_IN_ONE, update all-in-one bin
    FWSRV_CMD_IDX_ECOS_READ_EX,     ///< Input:Read ecos FW for compare
...
typedef struct _FWSRV_DXTYPE_MAP
{
    ...
    UINT32 RootFs;          ///< USER_DX_TYPE_EMBMEM_ROOTFS (not used currently)
...
/**
     ECOS Buffer Description

     A buffer with address and size information.
*/
typedef struct _FWSRV_ECOS_BUF_DESC{
    DX_HANDLE hStrg;///< ecos firmware stroage object handle
    FWSRV_BUF_DESC Buf; ///< buffer description
}FWSRV_ECOS_BUF_DESC,*PFWSRV_ECOS_BUF_DESC;

/**
     DSP Buffer Description

     A buffer with address and size information.
*/
typedef struct _FWSRV_DSP_BUF_DESC{
    DX_HANDLE hStrg;///< dsp firmware stroage object handle
    FWSRV_BUF_DESC Buf; ///< buffer description
    FWSRV_BUF_DESC WorkBuf; ///< if DSP is compressed, FwSrv need temp buffer to decode it.
}FWSRV_DSP_BUF_DESC,*PFWSRV_DSP_BUF_DESC;


/**
     Partial load (Linux)finish callback
*/
typedef void (*FWSRV_CB_TIME_TO_BOOT_LINUX)(void);

/**
     Linux Buffer Description

     A buffer with address and size information.
*/
typedef struct _FWSRV_LINUX_BUF_DESC{
    DX_HANDLE hStrg_uBoot;      ///< uBoot stroage object handle (USER_DX_TYPE_EMBMEM_UBOOT)
    DX_HANDLE hStrg_uImage;     ///< Linux-Kernel (uImage) storage object handle (USER_DX_TYPE_EMBMEM_LINUX)
    DX_HANDLE hStrg_RootFs;     ///< Root-Fs storage object handle( USER_DX_TYPE_EMBMEM_ROOTFS)
    FWSRV_BUF_DESC Buf_uBoot;   ///< uBoot buffer description
    FWSRV_BUF_DESC Buf_uImage;  ///< Linux-Kernel (uImage) buffer description
    FWSRV_BUF_DESC Buf_RootFs;  ///< Root-Fs buffer description
    FWSRV_CB_TIME_TO_BOOT_LINUX fpBootCb; ///< (Optional) after u-boot and first part of linux were loaded, this callback was invoked to speed up linux boot.
}FWSRV_LINUX_BUF_DESC,*PFWSRV_LINUX_BUF_DESC;
...
/**
     Updating All-In-One Bin structure

     For de-packing and updating the all-in-one bin file, for PStore partition data, it callbacks out to users updating themselves.
*/
typedef struct _FWSRV_BIN_UPDATE_ALL_IN_ONE{
    UINT32 uiSrcBufAddr;    ///< all-in-one bin context
    UINT32 uiSrcBufSize;    ///< all-in-one bin size
    UINT32 uiWorkBufAddr;   ///< working buffer address must WORD alignment
    UINT32 uiWorkBufSize;   ///< working buffer size, must larger than FWSRV_ALL_IN_ONE_TMP_SIZE
    FWSRV_BIN_DEPACK_CB fpItemCb; ///< callback for get each PStore item information and context.
    FWSRV_CB_CHECK_SUM_OK fpCheckSumCb; ///< (Optional) check sum result notification
}FWSRV_BIN_UPDATE_ALL_IN_ONE,*PFWSRV_BIN_UPDATE_ALL_IN_ONE;

/**
     Decompress FW

     eCos FW may be compressed. for FWSRV_CMD_IDX_DECOMPRESS and this structure can decode it.
*/
typedef struct _FWSRV_DECOMPRESS_DESC{
    FWSRV_BUF_DESC Buf_Src; ///< source compressed buffer
    FWSRV_BUF_DESC Buf_Dst; ///< target decoded buffer
}FWSRV_DECOMPRESS_DESC, *PFWSRV_DECOMPRESS_DESC;
