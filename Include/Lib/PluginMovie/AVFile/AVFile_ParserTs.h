...
typedef struct {
    ...
    UINT32 size;
	UINT16 key_frame;
	UINT32 frameIdx;} TS_FRAME_ENTRY;
...
extern void TsReadLib_ParseThumbInfo(UINT32 srcAddr, UINT32 readSize, UINT32 dstAddr);extern UINT32 TsReadLib_GetNextIFrame(UINT32 uiFrmIdx, UINT32 uiSkipI);
extern ER TsReadLib_SearchIFrame(UINT32 srcAddr, UINT32 srcSize, UINT32 dstAddr, UINT32* dstSize, UINT32* direction);
extern ER   TsReadLib_ParseNextIFrame();
/**
...
extern unsigned int TsReadLib_ReadBit(void);
extern unsigned int TsReadLib_ReadBits(int n);
extern unsigned int TsReadLib_ReadExponentialGolombCode(void);
extern unsigned int TsReadLib_ReadSE(void);
