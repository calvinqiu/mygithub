...
#define MEDIAREC_VER_VS      33
#define MEDIAREC_VER_VSWA    34
#define MEDIAREC_VER_4_0     40


#define AVI_USERDATA_MAXLEN 128//2011/11/22 Meg adds ///< user data max length for avi file

#define MEDIAREC_H264GOP_IPONLY     0 ///< H264 IP only
#define MEDIAREC_H264GOP_IPB        1 ///< H264 IPB
...
typedef void  (*MEDIAREC_3DNRCB)(UINT32 level, UINT32 config);
...
typedef struct
{
    ...
    UINT32              uiJpgYuvFormat;     ///< jpeg yuv format setting, JPG_FMT_YUV420
    ...
    VIDENC_PATH         uiEncPath[VIDENC_ID_MAX];  ///< H.264 encoding path (D2D, direct, ...)
    ...
    UINT32              uiSensorNum;        ///< input sensor number (for encoding path judgement)
    VIDENC_SVC          uiSVC[VIDENC_ID_MAX];   ///< set SVC disable/enable and GOP number
    BOOL                bRawEncOn;          ///< raw encode enable/disable

    //#NT#2016/09/20#Bob Huang -begin
    //#NT#Support HDMI Display with 3DNR Out
    BOOL                b3DNROut;           ///< 3DNR Out enable/disable
