...
typedef enum
{
  DCE_OPMODE_D2D             = 0,    ///< D2D
  DCE_OPMODE_IPP             = 1,    ///< IFE->DCE->IPE->IME
  DCE_OPMODE_RDE_IPP         = 2,    ///< RDE + IFE->DCE->IPE->IME
  DCE_OPMODE_RHE_IPP         = 3,    ///< RDE + IFE->DCE->IPE->IME
  DCE_OPMODE_SIE_IPP         = 4,    ///< SIE + IFE->DCE->IPE->IME (all direct)
...
typedef enum
{
  DCE_1ST_HSTP_POS_CALC    = 0,    ///< Input starts from DCE calculated position
  DCE_1ST_HSTP_POS_ZERO    = 1,    ///< Input starts from 0
  ENUM_DUMMY4WORD(DCE_HSTP_INST)
} DCE_HSTP_INST;

/**
    DCE H stripe output overlap selection

    Select output overlap pixel number
\n Used for dce_setHstripeOutOvlp().
*/
typedef enum
{
  DCE_HSTP_OUTOVLP_16    = 0,    ///< 16 pixels overlap
  DCE_HSTP_OUTOVLP_24    = 1,    ///< 24 pixels overlap
  DCE_HSTP_OUTOVLP_32    = 2,    ///< 32 pixels overlap
  ENUM_DUMMY4WORD(DCE_HSTP_OUTOVLP)
} DCE_HSTP_OUTOVLP;
...
typedef struct
{
    ...
    DCE_2DLUT_SCALE TwoDLutScaleFact;   ///< 2D lut x y scale factor
} DCE_2DLUT_PARAM;
//@}
...
typedef struct
{
    ...
    DCE_HSTP_INST HStpInStart;          ///< First H stripe input selection
    DCE_HSTP_OUTOVLP HstpOutOvlp;       ///< H stripe output overlap selection
} DCE_HSTP_PARAM;
//@}
...
typedef struct
{
    ...
    DCE_LBUF_MODE LbufMode;         ///< Line buffer mode
    UINT32 uiWidth;                 ///< Input width
    UINT32 uiHeight;                ///< Input height
    UINT32 uiHolap;                 ///< Output H overlap
...
extern ER dce_change2DLut(DCE_2DLUT_PARAM *pInParam);
