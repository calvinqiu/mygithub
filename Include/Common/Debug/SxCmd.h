...
/**
    Command interface of CPU2 mode

    SxCmd provide anoter command system we called CPU2 used in test mode.
    If user need to run CPU2 mode, the CPU2 have to be registered via this API.
    More information about CPU2, can refer to the CPU2 related AppNote or Doxygen.

    @param[in] fpCPU2CB CPU2 command entry function pointer.
*/
extern void SxCmd_RegCPU2CB(SX_CMD fpCPU2CB);

/**
    Command interface of DSP mode

    SxCmd provide anoter command system we called DSP used in test mode.
    If user need to run DSP mode, the DSP have to be registered via this API.
    More information about DSP, can refer to the DSP related AppNote or Doxygen.

    @param[in] fpDSPCB DSP command entry function pointer.
*/
extern void SxCmd_RegDSPCB(SX_CMD fpDSPCB);
