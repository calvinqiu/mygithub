...
#if (UIFRAMEWORK_HANG_DUMP_KER == ENABLE)
void OnNvtUser(UXUSER_CB_TYPE evt, UINT32 p1, UINT32 p2, UINT32 p3)
{
	static BOOL bDumped = FALSE;
	switch(evt) {
	case UXUSER_CB_QFULL:
		if(!bDumped) {
...
int NvtMain(int argc, char *argv[])
{
    ...
    if(RtcDate.s.year < 2000)
    {
        rtc_setDate(2000, 1, 1);
        rtc_triggerCSET();
    }
    ...
    // Start User Event System
    {
		...
		//#NT#[118423], UIFramework got -1 and then perform ker dump
#if (UIFRAMEWORK_HANG_DUMP_KER == ENABLE)
		uiUserObj.pfNvtUserCB = OnNvtUser;
#endif
		//#NT#2017/04/21#Niven Cho -end
        Ux_Open(&uiUserObj);
        //Init UI background
