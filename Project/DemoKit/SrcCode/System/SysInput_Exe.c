...
#if defined(YQCONFIG_YUANEDOG_SUPPORT)
int SX_TIMER_YUANEDOG_SOUND_ID = -1;
#endif
...
#endif
#if defined(YQCONFIG_YUANEDOG_SUPPORT)
SX_TIMER_ITEM(YuanEdog_SoundPlay, YuanEdog_SoundPlay, 1, FALSE)
#endif
...
void System_OnInputInit(void)
{
    ...
    //PHASE-2 : Init & Open Lib or LibExt
    {
        ...
        #endif
        #if defined(YQCONFIG_YUANEDOG_SUPPORT)
        SX_TIMER_YUANEDOG_SOUND_ID = SxTimer_AddItem(&Timer_YuanEdog_SoundPlay);
        #endif
        #if (STATUSKEY_FUNCTION == ENABLE)
        GxKey_DetStatusKey();
        #endif
        ...
        #endif
        #if defined(YQCONFIG_YUANEDOG_SUPPORT)
        SxTimer_SetFuncActive(SX_TIMER_YUANEDOG_SOUND_ID, TRUE);
        #endif
		...
		GxTouch_SetCtrl(GXTCH_MOVE_REPEAT_RATE, 3);
