...
void Load_MenuInfo(void)
{
    ...
    UINT32  uiSbsCalLength=0, uiPoolAddr=0;
    #endif

    UIInfoTraceMsg("Read UIINFO\r\n");

    uiFWUpdate = (gBinInfo.ld.LdCtrl2 & LDCF_UPDATE_FW);
    uiTmpBoot = (gBinInfo.ld.LdCtrl2 & LDCF_BOOT_CARD);
    ...
    #if (SBS_VIEW_FUNC == ENABLE)
    if ((pSection = PStore_OpenSection(PS_SBS_CAL_PARAM, PS_RDWR)) != E_PS_SECHDLER)
    {
        result = PStore_ReadSection((UINT8 *)&uiSbsCalLength, 0, 4, pSection); // read SBS cal length
        if (result != E_PS_OK)
        {
            UIInfoErrMsg("Load side_by_side cal length fail\r\n");
        }
        else
        {
            uiSbsCalLength = UINT32_SWAP(uiSbsCalLength); //SWAP for little endian
            debug_msg("^G%s uiSbsCalLength = %d\r\n", __func__, uiSbsCalLength);

            uiPoolAddr = OS_GetMempoolAddr(POOL_ID_SBS_CAL);

            if ((uiSbsCalLength != 0) && (uiSbsCalLength < OS_GetMempoolSize(POOL_ID_SBS_CAL)))
            {
                result = PStore_ReadSection((UINT8 *)uiPoolAddr, 4, uiSbsCalLength, pSection);
                if (result != E_PS_OK)
                {
                    SysSetFlag(FL_SBS_CAL_PARAM_SIZE, 0);
                    UIInfoErrMsg("Load side_by_side cal param fail\r\n");
                }
                else
                {
                    SysSetFlag(FL_SBS_CAL_PARAM_SIZE, uiSbsCalLength);
                }
            }
            else
            {
                UIInfoErrMsg("SBS cal length > %d\r\n", OS_GetMempoolSize(POOL_ID_SBS_CAL));
            }
        }
        PStore_CloseSection(pSection);
    }
    ...
    {
        UIInfoErrMsg("Open side_by_side calibration section fail\r\n");
    }
    #endif
    //#NT#2016/11/18#KS Hung -begin
...
void Save_MenuInfo()
{
    ...
    UINT32  uiTmpBoot = (gBinInfo.ld.LdCtrl2 & LDCF_BOOT_CARD);
	...
	#if defined(YQCONFIG_UART_TO_MCU_SUPPORT)
	if(Is_validMCUVersion())
	{
		CHKPNT;
		currentInfo.MCU_Ver[0] = gMCUVersion[0];
		currentInfo.MCU_Ver[1] = gMCUVersion[1];	 
	}	
	#endif
...
#if defined(YQCONFIG_ADAS_FUNC_SUPPORT) && defined(YQCONFIG_ANDROID_SYSTEM_SUPPORT)
...
BOOL TPIsOnRange(VControl *pCtrl,UINT32 P1,UINT32 P2)
{
    ...
    if(strcmp(pCtrl->Name, UIFlowWndMovie_Status_Touch_SnapeShotCtrl.Name) == 0)
    {
...
//xc add for uart to mcu @20170210 start
#if defined(YQCONFIG_UART_TO_MCU_SUPPORT)
void UPdataMCUVersion(void)
{
