...
BOOL VidInfoCB(PGXVIDEO_INFO pVidInfo)
{
    IRECT DstRegion = {0};
    USIZE DeviceRatioSize = GxVideo_GetDeviceAspect(DOUT1);
    DstRegion.w=pVidInfo->uiVidWidth;
    DstRegion.h=  DstRegion.w*DeviceRatioSize.h/DeviceRatioSize.w;
    DstRegion.x=0;
    if((INT32)pVidInfo->uiVidHeight > DstRegion.h)
        DstRegion.y=(pVidInfo->uiVidHeight-DstRegion.h)>>1;
    else
        DstRegion.y=0;
    if ((DstRegion.x+DstRegion.w) > (INT32)pVidInfo->uiVidWidth)
    {
        DBG_ERR("Crop width is out of src image, SrcWidth=%d,StartX=%d,Width=%d\r\n", pVidInfo->uiVidWidth, DstRegion.x, DstRegion.w);
        return FALSE;
    }
    else if ((DstRegion.y+DstRegion.h) > (INT32)pVidInfo->uiVidHeight)
    {
        DBG_ERR("Crop height is out of src image, SrcHeight=%d,StartY=%d,Height=%d\r\n", pVidInfo->uiVidHeight, DstRegion.y, DstRegion.h);
        return FALSE;
    }
    else
    {
        MoviePlay_ChangeSingleDstRegion(PB_SIGDISP_CROP, &DstRegion);
    }
    return TRUE;
}
...
//    PB_OpenSpecFileBySeq(DCF_GetDBInfo(DCF_INFO_TOL_FILE_COUNT), TRUE);
    ...
    PB_SetParam(PBPRMID_VIDEO_INFO_CALLBACK, (UINT32)VidInfoCB);
	#endif
            ...
            switch(Tabfocus)
            {
                ...
                case UIFlowWndPlay_TabNavi_ButtonPlay:
                    ...
                    if (uiPBFileFmt & (PBFMT_MOVMJPG | PBFMT_AVI | PBFMT_MP4 | PBFMT_TS))
                    {
                        ...
                        g_UIPlayMediaObj.uiMemSize        = (UINT32)OS_GetMempoolSize(POOL_ID_APP);
...
//        SxTimer_SetFuncActive(SX_TIMER_DET_STRG_ID, TRUE);
	...
	//#NT#2016/11/10#Niven Cho -begin
    //#NT#AUTO_TEST
	MsdcNvtCb_ExamMsg("OnMovieFinish\r\n");
	//#NT#2016/11/10#Niven Cho -end
    //#NT#2016/03/03#Ben Wang -end
