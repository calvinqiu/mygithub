...
BOOL UIStorageCheck(UINT32 uiCheckItem, UINT32 *pFreeCount)
{
    ...
    switch(uiCheckItem)
    {
        ...
        case STORAGE_CHECK_ERROR:
            FileSys_WaitFinish();
            if (UI_GetData(FL_FSStatus) == FS_DISK_ERROR
            || UI_GetData(FL_FSStatus) == FS_UNKNOWN_FORMAT
            || UI_GetData(FL_FSStatus) == FS_UNFORMATTED
            //#NT#2016/12/06#Niven Cho -begin
            //#NT#MULTI_DRIVE
            || UI_GetData(FL_CardStatus) == CARD_REMOVED
