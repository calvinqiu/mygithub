...
UINT32 XML_GetTempMem(UINT32 uiSize)
{
    ...
    return (UINT32)pBuf;
#else
    return SxCmd_GetTempMem(uiSize);
#endif
...
int XML_PBGetData(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    for (i=gIndex;i<=fileCount;i++)
    {
        ...
        {
            len= XML_snprintf(&buf,&xmlBufSize,"<Dir>\n<NAME>\n<![CDATA[%s]]></NAME><FPATH>\n<![CDATA[%s]]></FPATH>",&tempPath[15],tempPath);
            len= XML_snprintf(&buf,&xmlBufSize,"<TIMECODE>%ld</TIMECODE><TIME>%04d/%02d/%02d %02d:%02d:%02d</TIME>\n</Dir>\n",(FileStat.uiModifiedDate <<16) + FileStat.uiModifiedTime,
...
int XML_GetModeData(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    {
        len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_HEAD);
        len= XML_snprintf(&buf,&xmlBufSize,"<FREEPICNUM>%d</FREEPICNUM>\n",UI_GetData(FL_WIFI_PHOTO_FREEPICNUM));
        *bufSize = (HFS_U32)(buf)-bufAddr;
    ...
    {
        len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_HEAD);
        len= XML_snprintf(&buf,&xmlBufSize,"<MAXRECTIME>%d</MAXRECTIME>\n",UI_GetData(FL_WIFI_MOVIE_MAXRECTIME));
        *bufSize = (HFS_U32)(buf)-bufAddr;
        return CYG_HFS_CB_GETDATA_RETURN_OK;
    }
...
int XML_QueryCmd(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    else
    {
        ...
        len= XML_snprintf(&buf,&xmlBufSize,"<Function>\n");
...
int XML_GetThumbnail(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    TempBuf = XML_GetTempMem(POOL_SIZE_TEMP_TS);
    if(!TempBuf)
    {
    ...
    if (XML_CheckSub(pch, TYPE_MOV_MP4) || XML_CheckSub(pch, TYPE_TS))
    {
        if(segmentCount ==0)
        {
            ...
            if(gMovFilehdl)
            {
                ...
                uiBufferNeeded = POOL_SIZE_TEMP_TS;
                {
...
//#NT#2016/02/15#Ben Wang -begin
//#NEW,re-encode thumbnail for the photo that doesn't have EXIF.
                    GXIMGDEC_SCALE_DOWN_LEVEL ScaleDownlevel = GXIMGDEC_SCALE_DOWN_NONE;
...
int XML_GetScreen(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    char*             buf = (char*)bufAddr;
    FST_FILE          filehdl;
    char              tempPath[XML_PATH_LEN];
    ...
    if(!pch)
    {
        XML_DefaultFormat(WIFIAPP_CMD_SCREEN,WIFIAPP_RET_PAR_ERR,bufAddr,bufSize,mimeType);
        return CYG_HFS_CB_GETDATA_RETURN_OK;
    }
            ...
            if(gMovFilehdl)
            {
                ...
                uiBufferNeeded = POOL_SIZE_TEMP;
                {
                    ...
                    else if (VideoInfo.ui1stFrameSize) // Return 1st I frame back to APP.
                    {
                        ...
                        if ((VideoInfo.uiVidWidth == 2880) && (VideoInfo.uiVidHeight == 2160))
                        {
                            IdrToJpg.uiJpgWidth  = 3840;
                            IdrToJpg.uiJpgHeight = 2160;
                            IdrToJpg.uiDAR = VIDENC_DAR_16_9;
                        }
                        else
                        {
                            IdrToJpg.uiJpgWidth  = VideoInfo.uiVidWidth;
                            IdrToJpg.uiJpgHeight = VideoInfo.uiVidHeight;
                            IdrToJpg.uiDAR = VIDENC_DAR_DEFAULT;
                        }
                        IdrToJpg.uiFileSize  = FileStat.uiFileSize;
                        //#NT#2016/02/15#Ben Wang -begin
                        ...
                        if (iResult == E_OK)
                        {
                            ScreenBuf = IdrToJpg.uiJpgBsAddr;
                            remain = IdrToJpg.uiJpgBsSize;
                        }
                        else
                        {
            ...
            if(filehdl)
            {
                ...
                if(EXIF_ER_OK == EXIF_ParseExif(EXIF_HDL_ID_1, &ExifData))
                {
                   //find screennail
                    ...
                    ScreenBuf = XML_GetTempMem(ScreenSize);
                    if(ScreenBuf)
                    {
...
int XML_GetSbsCalFromPstore(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    char*             buf = (char*)bufAddr;
    UINT32            bufflen = *bufSize-512;
    static UINT32     remain =0;
    ...
    // set the data mimetype
    XML_STRCPY(mimeType,"sbs cfg/xml",CYG_HFS_MIMETYPE_MAXLEN);
    ...
    if(segmentCount ==0)
    {
        if(uiPoolAddr)
        {
            remain = SysGetFlag(FL_SBS_CAL_PARAM_SIZE);
            if(!remain)
            {
                DBG_ERR("no cfg file!!!\r\n");
                XML_ValueResult(WIFIAPP_CMD_GET_SBS_CAL, WIFIAPP_RET_NOFILE, bufAddr,bufSize,mimeType);
                return CYG_HFS_CB_GETDATA_RETURN_OK;
            }
        }
        else
        {
            DBG_ERR("no buffer!!!\r\n");
            XML_ValueResult(WIFIAPP_CMD_GET_SBS_CAL, WIFIAPP_RET_NOBUF, bufAddr,bufSize,mimeType);
            return CYG_HFS_CB_GETDATA_RETURN_OK;
        }
    }
    ...
    if(uiPoolAddr)
    {
        ...
        {
            hwmem_open();
            hwmem_memcpy((UINT32)buf, uiPoolAddr+bufflen*segmentCount, bufflen);
            hwmem_close();
            *bufSize = bufflen;
            remain-=bufflen;
            return CYG_HFS_CB_GETDATA_RETURN_CONTINUE;
        ...
        {
            hwmem_open();
            hwmem_memcpy((UINT32)buf, uiPoolAddr+bufflen*segmentCount, remain);
            hwmem_close();
            *bufSize = remain;
            remain=0;
            uiPoolAddr =0;
...
int XML_GetLiveViewFmt(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    if(ImageApp_NetPhoto_GetConfig(NETPHOTO_CFG_MODE)==NETPHOTO_TYPE_HTTP)
        len= XML_snprintf(&buf,&xmlBufSize, "<PhotoLiveViewLink>http://%s:8192</PhotoLiveViewLink>\n", UINet_GetIP());
    else
        len= XML_snprintf(&buf,&xmlBufSize, "<PhotoLiveViewLink>rtsp://%s/xxx.mov</PhotoLiveViewLink>\n", UINet_GetIP());
...
void XML_DefaultFormat(UINT32 cmd,UINT32 ret,HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType)
{
    ...
    len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_HEAD);
    len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_RET,cmd,ret);
    *bufSize = (HFS_U32)(buf)-bufAddr;
...
void XML_StringResult(UINT32 cmd,char *str,HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType)
{
    ...
    XML_STRCPY(mimeType,"text/xml",CYG_HFS_MIMETYPE_MAXLEN);
    len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_HEAD);
    len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_STR,cmd,0,str);//status OK
    *bufSize = (HFS_U32)(buf)-bufAddr;
...
void XML_ValueResult(UINT32 cmd,UINT64 value,HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType)
{
    ...
    XML_STRCPY(mimeType,"text/xml",CYG_HFS_MIMETYPE_MAXLEN);
    len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_HEAD);
    len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_VALUE,cmd,0,value);//status OK
    *bufSize = (HFS_U32)(buf)-bufAddr;
    ...
    else
    {
        ...
        while(appCmd[uiCmdIndex].cmd != 0)
        {
            if(appCmd[uiCmdIndex].UIflag != FL_NULL)
            {
                ...
                len= XML_snprintf(&buf,&xmlBufSize, DEF_XML_CMD_CUR_STS, appCmd[uiCmdIndex].cmd, UI_GetData(appCmd[uiCmdIndex].UIflag));
                #else
                ...
                else
                {
                    ...
                    len= XML_snprintf(&buf,&xmlBufSize, DEF_XML_CMD_CUR_STS, appCmd[uiCmdIndex].cmd, uiRecSizeIdx);
                }
                #endif
...
//#NT#Don't enter Wi-Fi mode again if it's Wi-Fi mode already.
...
int XML_GetPictureEnd(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    if(result == 0)
    {
        len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_HEAD);
        len= XML_snprintf(&buf,&xmlBufSize,"<Function>\n");
        len= XML_snprintf(&buf,&xmlBufSize,DEF_XML_CMD_CUR_STS,WIFIAPP_CMD_CAPTURE,result);
        len= XML_snprintf(&buf,&xmlBufSize,"<File>\n<NAME>%s</NAME>\n<FPATH>%s</FPATH></File>\n",pFileName,pFilePath);
        len= XML_snprintf(&buf,&xmlBufSize,"<FREEPICNUM>%d</FREEPICNUM>\n",FreePicNum);
        len= XML_snprintf(&buf,&xmlBufSize,"</Function>\n");
        *bufSize = (HFS_U32)(buf)-bufAddr;
    }
...
{
    INT32 iStatus = UIFlowWndWiFiMovie_GetStatus();
...
int XML_GetCntModeStatus(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    INT32 iCurStatus = UIFlowWndWiFiMovie_GetStatus();
...
int XML_GetWifiAP(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount){
    ...
    for(i=0; i<UINet_ss_status.number && UINet_ss_status.number!=0xff; i++){
        ...
        DBG_DUMP("%-16s\r\n", tmp2Buf);
        len= XML_snprintf(&buf,&xmlBufSize,"<Auth_type>%d</Auth_type>\n",auth_type);

        DBG_DUMP("rssi %d\r\n", pBss->rssi);
        len= XML_snprintf(&buf,&xmlBufSize,"<RSSI>%d dBm</RSSI>\n",pBss->rssi);
...
int XML_GetSSID_passphrase(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    len= XML_snprintf(&buf,&xmlBufSize, "<PASSPHRASE>%s</PASSPHRASE>\n", UINet_GetPASSPHRASE());
...
INT32 WifiCmd_getQueryData(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    ...
    len= XML_snprintf(&buf,&xmlBufSize,QUERY_FMT,WIFI_APP_MANUFACTURER,1,WIFI_APP_MODLE);
    DBG_DUMP(buf);
...
#if (SBS_VIEW_FUNC == ENABLE)
#define FW_UPLOAD_USAGE_SAVE_PSTORE          0x03 // Save to Card and then store to PStore
#endif
...
int XML_UploadFile(char* path, char* argument, HFS_U32 bufAddr, HFS_U32 bufSize, HFS_U32 segmentCount, HFS_PUT_STATUS putStatus)
{
    ...
    else
    {
        if(segmentCount==0)
        {
            ...
            //#NT#For Side By Side calibration from APP
            #if (SBS_VIEW_FUNC == ENABLE)
            if ((uiUsage == FW_UPLOAD_USAGE_SAVE_AS_FILE) || (uiUsage == FW_UPLOAD_USAGE_SAVE_PSTORE))
            #else
        ...
        if(putStatus == HFS_PUT_STATUS_FINISH)
        {
            ...
            //#NT#For Side By Side calibration from APP
            #if (SBS_VIEW_FUNC == ENABLE)
            else if (uiUsage == FW_UPLOAD_USAGE_SAVE_PSTORE)
            {
                PPSTORE_SECTION_HANDLE  pSection;
                INT32  result;

                 SysSetFlag(FL_SBS_CAL_PARAM_SIZE, uiCopiedBytes);

                if (uiCopiedBytes > OS_GetMempoolSize(POOL_ID_SBS_CAL))
                {
                    DBG_ERR("SBS cal param more than 512KB\r\n");
                    return CYG_HFS_UPLOAD_FAIL_RECEIVE_ERROR;
                }
                debug_msg("^M%s: uiCopiedBytes = 0x%x\r\n", __func__, uiCopiedBytes);
                u8PoolAddr = (UINT8 *)OS_GetMempoolAddr(POOL_ID_SBS_CAL);
                u8PoolAddr[0] = (uiCopiedBytes >> 24) & 0xFF; // Using 4 byte stands for calibration length
                u8PoolAddr[1] = (uiCopiedBytes >> 16) & 0xFF;
                u8PoolAddr[2] = (uiCopiedBytes >> 8) & 0xFF;
                u8PoolAddr[3] = uiCopiedBytes & 0xFF;

                FileSys_SeekFile(fhdl, 0 , FST_SEEK_SET);
                FileSys_ReadFile(fhdl,&u8PoolAddr[4], &uiCopiedBytes, 0, NULL);

                if ((pSection = PStore_OpenSection(PS_SBS_CAL_PARAM, PS_RDWR | PS_CREATE)) != E_PS_SECHDLER)
                {
                    result = PStore_WriteSection(u8PoolAddr, 0, uiCopiedBytes+4, pSection);
                    if (result != E_PS_OK )
                    {
                        DBG_ERR("Write side_by_side calibration param fail\r\n");
                    }
                }
                else
                {
                    DBG_ERR("Open side_by_side calibration section fail\r\n");
                    return CYG_HFS_UPLOAD_FAIL_RECEIVE_ERROR;
                }
