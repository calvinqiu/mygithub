...
{
    if(_g_bFirstWifi){TM_BOOT_BEGIN("network","config");}
    g_Wifi_time = clock();
    DBG_DUMP("^RAP start\r\n");
...
BOOL UINet_CPU2_Init(void)
{
    ...
    UINet_NvtIpc_Init();
    //GxeCos_Init();
    SxCmd_DoCommand("ecos uart");
    SxCmd_DoCommand("ecos open");
    // wait cpu2 power on
    ...
    UINet_NvtIpc_Init();
    //GxLinux_Init();
    SxCmd_DoCommand("linux uart");
    SxCmd_DoCommand("linux open");
    // wait cpu2 power on
    NvtIpc_WaitCPU2PowerOnReady();
#endif
    //Delay_DelayMs(3000); //wait for cpu2 startup finish
#endif

#if 0  //CPU2 mem protect
#include "DbgUtApi.h"
    ...
    Cfg.User.bIncludeCpu = FALSE;
    Cfg.User.bIncludeCpu2 = TRUE;
    Cfg.User.bIncludeDsp = FALSE;
    Cfg.User.uiAddr = 0x8E800000;
    Cfg.User.uiSize = 0x01000000;
    Cmd.In.pData=&Cfg;
...
#if(_NETWORK_ == _NETWORK_SDIO_EVB_WIFI_ && _CPU2_TYPE_!=_CPU2_NONE_)
...
void UINet_StationStatus_CB(char *pIntfName, char *pMacAddr, int status)
{
	...
	DBG_IND("UINet_StationStatus_CB=%d\r\n", status);
    ...
    if (status == NVT_WIFI_STA_STATUS_ASSOCIATED) {
        ...
        if(_g_bFirstWifi){TM_BOOT_END("network","open"); _g_bFirstWifi = FALSE;}
...
#if (ECOS_POWER_OFF_FLOW == ENABLE || WIFI_POWER_OFF_FLOW == ENABLE || _CPU2_TYPE_==_CPU2_LINUX_)
//#NT#2016/04/14#YongChang_Qui -end
    ...
    if(_g_bFirstWifi){TM_BOOT_BEGIN("network","poweron");}
    DrvWiFi_PowerOn();
    if(_g_bFirstWifi){TM_BOOT_END("network","poweron");}

    memset(pwifi,0,sizeof(nvt_wifi_settings));

    if(!bInit)
    {
        if(_g_bFirstWifi){TM_BOOT_BEGIN("network","netdev");}
        result = WiFiIpc_init_wlan0_netdev(0);
        if(_g_bFirstWifi){TM_BOOT_END("network","netdev");}
        if (result!=0 )
        {
...
//#NT#2016/08/25#YongChang_Qui -end
    ...
    if ((mode == NET_AP_MODE) || (mode == NET_P2P_GO_MODE) || (mode == NET_WPS_AP_PBC_MODE))
    {
        memset(gCurrIP,0,NVT_WIFIIPC_IP_MAX_LEN);
        snprintf(gCurrIP, NVT_WIFIIPC_IP_MAX_LEN,"%s",EXAM_NET_AP_IP);
        if(_g_bFirstWifi){TM_BOOT_BEGIN("network","up");}
        #if (_CPU2_TYPE_==_CPU2_ECOS_)
        WiFiIpc_interface_config("wlan0", gCurrIP, "255.255.255.0");
        #else
        WiFiIpc_interface_up("wlan0");
        #endif
        if(_g_bFirstWifi){TM_BOOT_END("network","up");}
        DBG_IND(" setip=%s -\r\n", gCurrIP);
        if(_g_bFirstWifi){TM_BOOT_BEGIN("network","dhcp");}
        #if (_CPU2_TYPE_==_CPU2_ECOS_)
        result = UINet_DHCP_Start(FALSE);
        #endif
        if(_g_bFirstWifi){TM_BOOT_END("network","dhcp");}
    }
    else if (mode == NET_STATION_MODE)
    {
        g_CliConnected = FALSE;
    ...
    if(_g_bFirstWifi){TM_BOOT_BEGIN("network","hfs");}
    if(result==E_OK)
    {
        result=UINet_HFSInit();
    }
    if(_g_bFirstWifi){TM_BOOT_END("network","hfs");}
        //#NT#2016/05/06#Jeah Yen -begin
    //#NT#Support IOTP2P
    #if (_CPU2_TYPE_==_CPU2_LINUX_)
    #if (IOT_P2P_FUNC == ENABLE)
    ...
    if(_g_bFirstWifi){TM_BOOT_END("network","config");}
    if(_g_bFirstWifi){TM_BOOT_BEGIN("network","open");}
...
#else //if (_NETWORK_ == _NETWORK_SPI_EVB_ETHERNET_) //Janice
